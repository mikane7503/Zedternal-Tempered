// ===================================================================
// DKUpgrade_Perk_Omen_Helper - Prophecy Engine
//
...
//
// SKILL INTEGRATION: Iterates ChildActors of DKUpgrade_Skill_OmenBase_Helper
// at key moments. Skills modify prophecy behavior via virtual callbacks.
// ===================================================================
class DKUpgrade_Perk_Omen_Helper extends Info
    transient;
...
function SetUpgradeLevel(int InLevel)
{
    ...
    if ((OldLevel < class'DKConfig_Capstone'.default.Capstone_Rank1Level && InLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level) || (OldLevel < class'DKConfig_Capstone'.default.Capstone_Rank2Level && InLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level))
        BuildProphecyPool();
}
...
{
    local DKUpgrade_Skill_OmenBase_Helper SH;
    local int HPCost;
    ...
    if (Player != None)
    {
        foreach Player.ChildActors(class'DKUpgrade_Skill_OmenBase_Helper', SH)
        {
...
function FailProphecy(string Reason)
{
    ...
    // Level 20: Deny Fate
    if (bDenyFateAvailable && !bDenyFateUsed && CurrentUpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level)
    {
        bDenyFateUsed = true;
        bDenyFateAvailable = false;
        DenyFateWaveCounter = 0;
...
function OnWaveEnd(int upgLevel)
{
    ...
    if ((upgLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level && ProphecyPool.Length < 14) || (upgLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && ProphecyPool.Length < 19))
        BuildProphecyPool();
...
function PlayOmenSound(name SoundID)
{
    ...
    local DKMutator MutObj;

    if (Player == None) return;
    DKPC = DKPlayerController(Player.Controller);
    if (DKPC == None) return;

    MutObj = class'DKSoundManager'.static.GetMutator(WorldInfo);
    if (MutObj == None) return;

    Sound = class'DKSoundManager'.static.GetSound(MutObj, SoundID);
    if (Sound != None)
...
//   from the active locale's [DKHudWrapper] section using helper
//   functions on DKHudWrapper (GetLocalizedOmenName/Desc/Whisper,
...
// localized strings using DKHudWrapper helpers based on the active locale.
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_Omen_Helper"
}
