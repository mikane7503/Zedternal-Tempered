// ===================================================================
// DKUpgrade_Perk_Speedster_Helper - per-player Blink Strike state machine
//
...
// (DKPlayerController.ActivateSpeedster -> TryActivate).
//
...
// (Ready / Active / Cooldown) drawn in DKHudWrapper.DrawSpeedsterCard.
// ===================================================================
class DKUpgrade_Perk_Speedster_Helper extends Info
    transient;
...
function int GetMaxTargets()
{
    ...
    if (PerkLevel >= class'DKConfig_Capstone'.static.GetRank1Level())
        T = class'DKUpgrade_Perk_Speedster'.default.MaxTargetsCapstone10;
    else
        T = class'DKUpgrade_Perk_Speedster'.default.MaxTargetsBase;

    if (OpenSeasonLevel >= 1)
        T += class'DKUpgrade_Skill_OpenSeason'.default.TargetBonus[OpenSeasonLevel - 1];
...
function float GetCooldown()
{
    ...
    CD = class'DKUpgrade_Perk_Speedster'.default.Cooldown;
    if (PerkLevel >= class'DKConfig_Capstone'.static.GetRank1Level())
        CD *= class'DKUpgrade_Perk_Speedster'.default.CooldownCapstone10Mult;

    if (OverclockLevel >= 1)
        CD *= 1.0f - class'DKUpgrade_Skill_Overclock'.default.CooldownReduction[OverclockLevel - 1];
...
function TryActivate()
{
    ...
    {
        class'DKMessageManager'.static.SendMinor(DKPC, "Blink Strike: cannot dash while down.");
        return;
    }
    ...
    if (bOnCooldown)
    {
        class'DKMessageManager'.static.SendMinor(DKPC, "Blink Strike: recharging.");
        return;
    }
...
function StartBlink()
{
    ...
    {
        class'DKMessageManager'.static.SendMinor(DKPC, "Blink Strike: no targets in range.");
        return;
    }
    ...
    PushHUD(HUD_ACTIVE);
    PlayBlinkSound();
    class'DKMessageManager'.static.SendImportant(DKPC, "BLINK STRIKE!");

    // First strike fires after one interval; subsequent strikes each tick.
    SetTimer(class'DKUpgrade_Perk_Speedster'.default.BlinkInterval, true, nameof(BlinkTick));
}
...
function GatherTargets()
{
    ...
    Range = class'DKUpgrade_Perk_Speedster'.default.TagRange;
    if (LongReachLevel >= 1)
        Range *= 1.0f + class'DKUpgrade_Skill_LongReach'.default.RangeBonus[LongReachLevel - 1];
...
function BlinkTick()
{
    ...
    if (IsZero(Dir))
        Dir = vect(1, 0, 0);
    Dest = KFM.Location + Dir * class'DKUpgrade_Perk_Speedster'.default.BlinkStandoff;
...
function StrikeTarget(KFPawn_Monster KFM)
{
    ...
    Pct = class'DKUpgrade_Perk_Speedster'.default.BlinkStrikePctHP;
    bBig = KFM.IsLargeZed() || KFM.IsABoss() || KFInterface_MonsterBoss(KFM) != None;

    if (bBig)
    {
        if (PerkLevel >= class'DKConfig_Capstone'.static.GetRank2Level())
            LargeMult = class'DKUpgrade_Perk_Speedster'.default.BlinkStrikeLargeMultCapstone20;
        else
            LargeMult = class'DKUpgrade_Perk_Speedster'.default.BlinkStrikeLargeMult;

        // Giant-Killer skill: scale up the reduced giant/boss chunk.
        if (GiantKillerLevel >= 1)
            LargeMult *= 1.0f + class'DKUpgrade_Skill_GiantKiller'.default.ChunkBonus[GiantKillerLevel - 1];
    ...
    KFM.TakeDamage(Dmg, DKPC, KFM.Location, Push * 1000.0f, class'DKDT_BlinkStrike', , OwnerPawn);
    ...
    if (ConcussiveLevel >= 1)
        KnockScale = 1.0f + class'DKUpgrade_Skill_ConcussiveBlink'.default.KnockdownBonus[ConcussiveLevel - 1];
...
function Cleave(KFPawn_Monster Center, int BaseDmg)
{
    ...
    Dmg = Round(float(BaseDmg) * class'DKUpgrade_Perk_Speedster'.default.CleaveFrac);
    if (Dmg < 1 || Center == None)
        return;

    Radius = class'DKUpgrade_Perk_Speedster'.default.CleaveRadius;
    ...
    foreach OwnerPawn.WorldInfo.AllPawns(class'KFPawn_Monster', M, Center.Location, Radius)
    {
        ...
        M.TakeDamage(Dmg, DKPC, M.Location, Push * 500.0f, class'DKDT_BlinkStrike', , OwnerPawn);
    }
}
...
function DoShockfront(KFPawn_Monster Center, int StrikeDmg)
{
    ...
    Radius = class'DKUpgrade_Skill_Shockfront'.default.Radius[ShockfrontLevel - 1];
    Dmg = Round(float(StrikeDmg) * class'DKUpgrade_Skill_Shockfront'.default.DamageFrac[ShockfrontLevel - 1]);
    if (Dmg < 1)
        return;
    ...
    foreach OwnerPawn.WorldInfo.AllPawns(class'KFPawn_Monster', M, Center.Location, Radius)
    {
        ...
        M.TakeDamage(Dmg, DKPC, M.Location, Push * 400.0f, class'DKDT_BlinkStrike', , OwnerPawn);
    }
}
...
function MarkZed(KFPawn_Monster KFM)
{
    ...
    Dur = class'DKUpgrade_Skill_HuntersMark'.default.MarkDuration;
...
function PlayBlinkSound()
{
    ...
    Cue = class'DKSoundManager'.static.GetSound(Mut, 'BlinkStrike_Activate');
    if (Cue != None)
...
function FinishBlink()
{
    ...
    SetTimer(class'DKUpgrade_Perk_Speedster'.default.InvulnTail, false, nameof(EndBlink));
}
...
function EndBlink()
{
    ...
    {
        Refund = FMin(float(DashKills) * class'DKUpgrade_Skill_Momentum'.default.RefundPerKill[MomentumLevel - 1], CD * 0.75f);
        CD -= Refund;
    }
...
function UpdateAbility()
{
    ...
    if (bOnCooldown)
    {
        ...
        if (Elapsed >= CD)
        {
            bOnCooldown = false;
            PushHUD(HUD_READY);
            class'DKMessageManager'.static.SendImportant(DKPC, "Blink Strike ready!");
        }
    }
}
...
// and animates the bar in DKHudWrapper.DrawSpeedsterCard.
// ===================================================================
function PushHUD(byte State)
{
    ...
    if (State == HUD_COOLDOWN)
        ...
        Dur = class'DKUpgrade_Perk_Speedster'.default.InvulnTail + 0.3f;
    else
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_Speedster_Helper"
}
