// ===================================================================
// DKAchievementReplicator - Per-Player Achievement Replication
...
// ===================================================================
class DKAchievementReplicator extends ReplicationInfo;

var KFPlayerController OwningPC;
var DKMutator Mutator;
...
{
	`log("DKAchievementReplicator (CLIENT): Loading custom sounds...");
	...
	{
		`log("DKAchievementReplicator (CLIENT): Loaded achievement sound");
	}
	else
	{
		`log("DKAchievementReplicator (CLIENT): Failed to load achievement sound!");
	}
	...
	{
		`log("DKAchievementReplicator (CLIENT): Loaded perk unlock sound");
	}
	else
	{
		`log("DKAchievementReplicator (CLIENT): Failed to load perk unlock sound!");
	}
}
...
function bool ShouldShowProgressNotification(name AchievementID)
{
	...
	local DKAchievementData.AchievementDefinition ThisAchievement, OtherAchievement;
...
function CompleteAchievement(name AchievementID, string AchievementName, string UnlockedPerkClass, Texture2D AchievementIcon)
{
	...
	`log("DKAchievementReplicator: Achievement completed -" @ AchievementName);
}
...
function UnlockPerkFromAchievement(string PerkClassName)
{
	...
	if (PerkIdx != INDEX_NONE)
	{
		...
		`log("DKAchievementReplicator: Unlocked perk" @ PerkClassName @ "from achievement");
	}
}
...
reliable client function ClientQueueAchievement(string AchievementName, string Description, string UnlockedPerkClass, Texture2D AchievementIcon)
{
	...
	`log("DKAchievementReplicator (CLIENT): Received achievement -" @ AchievementName);
...
reliable client function ShowNextAchievementInQueue()
{
	...
	if (AchievementQueue.Length == 0)
	{
		...
		HUDRetryCount = 0;
		`log("DKAchievementReplicator (CLIENT): Queue empty, processing complete");
		return;
	}
	...
	if (CustomHUD == None)
	{
		...
		{
			`log("DKAchievementReplicator (CLIENT): HUD not ready, retry" @ HUDRetryCount $ "/" $ MAX_HUD_RETRIES @ "in 1 second");
			SetTimer(1.0f, false, 'ShowNextAchievementInQueue');
			return;
		}
		else
		{
			`log("DKAchievementReplicator (CLIENT): HUD not available after" @ MAX_HUD_RETRIES @ "retries, skipping achievement popup");
	...
	`log("DKAchievementReplicator (CLIENT): Showing achievement" @ NextAchievement.AchievementName);
	
	// ===================================================================
	// PLAY ACHIEVEMENT UNLOCK SOUND
	// ===================================================================
	DKPC = DKPlayerController(OwningPC);
	if (DKPC != None && AchievementSound != None)
	{
		DKPC.ClientPlayAchievementSound(AchievementSound);
		`log("DKAchievementReplicator (CLIENT): Called ClientPlayAchievementSound");
	}
...
simulated function ShowDelayedPerkUnlock()
{
	...
	if (WMGRI == None)
	{
		`log("DKAchievementReplicator (CLIENT): WMGRI not available for delayed perk unlock");
	...
	for (PerkIdx = 0; PerkIdx < WMGRI.PerkUpgradesList.Length; PerkIdx++)
	{
		if (string(WMGRI.PerkUpgradesList[PerkIdx].PerkUpgrade.Name) ~= DelayedPerkClassName)
		{
			...
			if (CustomHUD != None)
			{
				...
				`log("DKAchievementReplicator (CLIENT): Showed delayed perk unlock popup for" @ PerkName);
			}
			else
			{
				`log("DKAchievementReplicator (CLIENT): Could not get CustomHUD for perk unlock popup");
			}
	...
	`log("DKAchievementReplicator (CLIENT): Could not find perk" @ DelayedPerkClassName);
	DelayedPerkClassName = "";
}
...
defaultproperties
{
	...
	Name="Default__DKAchievementReplicator"
}
