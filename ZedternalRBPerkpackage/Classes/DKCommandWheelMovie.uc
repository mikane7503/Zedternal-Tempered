// ===================================================================
// DKCommandWheelMovie - Standalone GFxMoviePlayer for command wheel
//
// Loads CommandWheel.swf as a full-screen overlay during trader time.
// Pushes slot data from DKConfig_CommandWheel to Flash.
...
// ===================================================================
class DKCommandWheelMovie extends GFxMoviePlayer;
...
function bool OpenWheel(DKPlayerController NewOwner)
{
    ...
    if (MovieInfo == None)
    {
        `log("[DK] CommandWheel: MovieInfo is None - ZedternalRBPerkpackage_Menus.upk not found!");
        return false;
    }
...
function PushAllSlotData()
{
    ...
    for (i = 0; i < 10; i++)
    {
        Label = class'DKConfig_CommandWheel'.static.GetSlotLabel(i);
        Cmd = class'DKConfig_CommandWheel'.static.GetSlotCommand(i);
...
function Callback_SlotSelected(int SlotIndex)
{
    ...
    {
        Cmd = class'DKConfig_CommandWheel'.static.GetSlotCommand(SlotIndex);
        if (Cmd != "")
        {
