// ===================================================================
// DKUpgrade_Skill_DeadEye_Helper - Manages Dead Eye ability state
// ===================================================================
class DKUpgrade_Skill_DeadEye_Helper extends Info
	transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, DKPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'DKUpgrade_Skill_DeadEye'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'DKUpgrade_Skill_DeadEye'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Dead Eye", self, class'DKUpgrade_Skill_DeadEye_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Dead Eye: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Dead Eye: Already active!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'DKMessageManager'.static.SendMinor(DKPC, "Dead Eye: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "DEAD EYE ACTIVATED! +25% headshot damage for 10 seconds!");
	}
	else
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "DEAD EYE (DELUXE) ACTIVATED! +40% headshot damage for 10 seconds!");
	}
...
function Deactivate()
{
	...
	class'DKMessageManager'.static.SendMinor(DKPC, "Dead Eye ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'DKMessageManager'.static.SendImportant(DKPC, "Dead Eye ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Skill_DeadEye_Helper"
}