class DKUpgrade_Perk_Scavenger_Helper extends Info
    transient;
...
function TrackKill(optional KFPawn_Monster KilledMonster, optional int UpgradeLevel = 0)
{
    ...
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level && KFPC != None && OwnerPawn != None)
    {
        if (FRand() <= class'DKUpgrade_Perk_Scavenger'.default.AmmoScavengeChance)
        {
            CurrentWeapon = KFWeapon(OwnerPawn.Weapon);
            if (CurrentWeapon != None)
            {
                // Add spare ammo to current weapon
                CurrentWeapon.AddAmmo(class'DKUpgrade_Perk_Scavenger'.default.AmmoScavengeAmount);
                
                // Show scavenging message
                KFPC.ClientMessage("AMMO SCAVENGED! Found " $ class'DKUpgrade_Perk_Scavenger'.default.AmmoScavengeAmount $ " spare ammo!", 'Event');
                
                // Play scavenge sound
                if (AmmoScavengeSound != None)
                    KFPC.PlayRMEffect(AmmoScavengeSound, ScavengerSoundRTPCName, class'DKUpgrade_Perk_Scavenger'.default.AmmoScavengeAmount);
...
function ApplyFieldAdaptation(KFPlayerController KFPC)
{
    ...
    local DKHudWrapper ScavengerHUD;
    ...
    if (KFPC != None)
    {
        ...
        ScavengerHUD = class'DKHudWrapper'.static.GetReaperHUD(KFPC);
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_Scavenger_Helper"
}