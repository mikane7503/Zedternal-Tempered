...
function AddHeatStack(KFPawn_Monster Target, int SkillLevel)
{
    ...
    // [ZedternalRBPerkpackage.DKUpgrade_Skill_ThermalDrill] MaxStacks
    // (index 0 = standard, 1 = deluxe).
    if (SkillLevel == 1)
    {
        MaxStacks = class'DKUpgrade_Skill_ThermalDrill'.default.MaxStacks[0];
    }
    else
    {
        MaxStacks = class'DKUpgrade_Skill_ThermalDrill'.default.MaxStacks[1];
    }
...
function UpdateHeatStacks()
{
    ...
    for (i = HeatTargets.Length - 1; i >= 0; i--)
    {
        ...
        StackDuration = (HeatTargets[i].HeatStacks > class'DKUpgrade_Skill_ThermalDrill'.default.MaxStacks[0])
            ? class'DKUpgrade_Skill_ThermalDrill'.default.StackDuration[1]
            : class'DKUpgrade_Skill_ThermalDrill'.default.StackDuration[0];
