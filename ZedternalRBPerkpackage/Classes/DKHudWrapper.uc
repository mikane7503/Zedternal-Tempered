class DKHudWrapper extends WMGFxHudWrapper;
...
//     5K    -> 2.45x
//
// HudScaleMultiplier: user preference from DKConfig_HudPreferences.
...
// Scoreboard is owned by DKGFxScoreBoardWrapper subclass
...
// Dev stat overlay mode (set via DKCheatManager exec StatOverlay)
// 0 = Off, 1 = Basic (values only), 2 = Sources (per-upgrade attribution)
var byte StatOverlayMode;

// Total Stat Increase panel toggle (Phase 1 of DKStatAggregator).
// Toggled via DKPlayerController.ToggleStats() exec, or `mutate stats`.
...
// .int / .kor / etc. file under the [DKHudWrapper] section at engine
// startup. defaultproperties values below are the English fallbacks
// (used when no .int entry overrides them).
//
// To add a translation: edit ZedternalRBPerkpackage.<lang> directly
...
// Helper class DKUpgrade_Perk_Omen_Helper sends ProphecyIndex via RPC;
...
// Helper class DKUpgrade_Perk_Shapeshifter_Helper sends Buff1Index/Buff2Index;
// client looks up the localized name here so each player sees their own locale.
...
// Used by DKPlayerController.ClientShapeshifterMimicryGain / ...Transform
...
// Helper class DKUpgrade_Perk_Gambit_Helper sends GambitIndex via RPC;
// client looks up the localized name here so each player sees their own locale.
...
// Used by DKPlayerController.ClientGambit* RPCs to build localized popups.
...
function InitializeNotificationFeed()
{
    ...
    `log("DKHudWrapper: Notification Feed initialized");
}
...
function InitializeAbilityDisplay()
{
    ...
    `log("DKHudWrapper: Active Ability Display initialized (4 slots)");
}
...
function InitializeAchievementPopup()
{
    ...
    `log("DKHudWrapper: Achievement Popup initialized");
}
...
function InitializeAgonyTracking()
{
    ...
    `log("DKHudWrapper: Agony tracker initialized");
}
...
function UpdateAgonyZedTimeTracker(bool bZedTimeActive, float TotalSeconds, int RewardsEarned, int CurrentUpgradeLevel)
{
    ...
    if (CurrentUpgradeLevel < class'DKConfig_Capstone'.default.Capstone_Rank2Level)
        return;
...
{
    if (CurrentUpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level)
    {
        if (bIcicleReady)
        {
...
exec function ToggleCryophilitePanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            CryoHelper = class'DKUpgrade_Perk_Cryophilite'.static.GetHelper(OwnerPawn);
            if (CryoHelper != None)
            {
                ...
                {
                    CryoStatus = "ICICLE ARROW READY! Next shot +500% damage!";
                }
                else
                {
                ...
                class'DKMessageManager'.static.SendImportant(KFPC, "CRYOPHILITE STATUS: " $ CryoStatus);
                
                class'DKMessageManager'.static.SendMinor(KFPC, "Total Headshot Kills: " $ CryoHelper.TotalHeadshotKills);
                class'DKMessageManager'.static.SendMinor(KFPC, "Total Kills: " $ CryoHelper.TotalKills);
                class'DKMessageManager'.static.SendMinor(KFPC, "Ice Tips: Precision archery - headshots charge Icicle Arrow, all kills charge Absolute Zero");
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Cryophilite perk not active.");
            }
        }
    }
}
...
function AddNotificationMessage(string Message, Color MessageColor, byte Priority)
{
    ...
    `log("DKHudWrapper: AddNotificationMessage called - Message:" @ Message @ "Priority:" @ Priority);
    ...
    `log("DKHudWrapper: Added notification, total messages:" @ NotificationMessages.Length);
...
function ShowPerkUnlockNotification(string PerkName, Texture2D PerkIcon)
{
    ...
    `log("DKHudWrapper: Showing perk unlock popup for" @ PerkName);
}
...
function ShowAchievementUnlockNotification(string AchievementName, string Description, Texture2D AchievementIcon)
{
    ...
    `log("DKHudWrapper: Showing achievement unlock popup for" @ AchievementName);
}
...
function UpdateAbilityState(int SlotIndex, bool bIsActive, bool bOnCooldown, float RemainingTime, float MaxTime)
{
	...
	`log("DKHudWrapper (CLIENT): UpdateAbilityState slot" @ SlotIndex);
...
// Called from DKPlayerController.ToggleStats() / ClientToggleStats().
...
exec function ToggleHydraPanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            HydraHelper = class'DKUpgrade_Perk_Hydra'.static.GetHelper(OwnerPawn);
            if (HydraHelper != None)
            {
                ...
                class'DKMessageManager'.static.SendImportant(KFPC, "HYDRA STATUS: " $ HydraStatus);
                class'DKMessageManager'.static.SendMinor(KFPC, "Total Kills: " $ HydraHelper.TotalKills);
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Hydra perk not active.");
            }
        }
    }
}
...
exec function ToggleWendigoPanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            WendigoHelper = class'DKUpgrade_Perk_Wendigo'.static.GetHelper(OwnerPawn);
            if (WendigoHelper != None)
            {
                WendigoStatus = WendigoHelper.GetStalkingSummary();
                class'DKMessageManager'.static.SendImportant(KFPC, "WENDIGO STATUS: " $ WendigoStatus);
                
                if (WendigoHelper.bApexStalkerActive)
                {
                    class'DKMessageManager'.static.SendImportant(KFPC, "Apex Stalker: ACTIVE - +150% damage until hit");
                }
                else if (WendigoHelper.bFirstShotReady)
                {
                    class'DKMessageManager'.static.SendImportant(KFPC, "Perfect Ambush: READY - Next shot +200% damage");
                }
                
                class'DKMessageManager'.static.SendMinor(KFPC, "Stalking Tips: Wait 5s for damage bonus, 10s for perfect ambush, 30s no damage for Apex Stalker");
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Wendigo perk not active.");
            }
        }
    }
}
...
exec function ToggleArchangelPanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            ArchangelHelper = class'DKUpgrade_Perk_Archangel'.static.GetHelper(OwnerPawn);
            if (ArchangelHelper != None)
            {
                ArchangelStatus = ArchangelHelper.GetHealingSummary();
                class'DKMessageManager'.static.SendImportant(KFPC, "ARCHANGEL STATUS: " $ ArchangelStatus);
                
                class'DKMessageManager'.static.SendMinor(KFPC, "Total Healing Done: " $ ArchangelHelper.TotalHealingDone $ " HP");
                class'DKMessageManager'.static.SendMinor(KFPC, "Allies Healed: " $ ArchangelHelper.AlliesHealed);
                if (ArchangelHelper.MiracleRecoveries > 0)
                {
                    class'DKMessageManager'.static.SendMinor(KFPC, "Miracle Recoveries: " $ ArchangelHelper.MiracleRecoveries);
                }
                if (ArchangelHelper.bHealingAuraActive)
                {
                    class'DKMessageManager'.static.SendImportant(KFPC, "Healing Aura: ACTIVE - Allies within 6m regenerate 1 HP/sec");
                }
                class'DKMessageManager'.static.SendMinor(KFPC, "Support Tips: Heal allies for milestones, stay near team for bonuses");
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Archangel perk not active.");
            }
        }
    }
}
...
exec function ToggleMedusaPanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            MedusaHelper = class'DKUpgrade_Perk_Medusa'.static.GetHelper(OwnerPawn);
            if (MedusaHelper != None)
            {
                MedusaStatus = MedusaHelper.GetVenomStatus();
                class'DKMessageManager'.static.SendImportant(KFPC, "MEDUSA STATUS: " $ MedusaStatus);
                
                class'DKMessageManager'.static.SendMinor(KFPC, "Total Poison Damage: " $ MedusaHelper.TotalPoisonDamage);
                if (MedusaHelper.bFullGorgonAchieved)
                {
                    class'DKMessageManager'.static.SendCritical(KFPC, "Full Gorgon: ACTIVE - Permanent +20% headshot damage");
                }
                else
                {
                    class'DKMessageManager'.static.SendMinor(KFPC, "Current Bonuses: +" $ (MedusaHelper.CurrentScales * 3) $ "% damage resistance, +" $ (MedusaHelper.CurrentScales * 2) $ "% movement speed");
                }
                class'DKMessageManager'.static.SendMinor(KFPC, "Venom Tips: All damage applies poison at level 10+, kills spread poison at level 20+");
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Medusa perk not active.");
            }
        }
    }
}
...
exec function ToggleRiotPanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            RiotHelper = class'DKUpgrade_Perk_Riot'.static.GetHelper(OwnerPawn);
            if (RiotHelper != None)
            {
                ...
                if (bMaxBonus)
                {
                    ...
                    class'DKMessageManager'.static.SendCritical(KFPC, RiotStatus);
                    class'DKMessageManager'.static.SendImportant(KFPC, "Bonuses: Stumble Immunity, +30% Movement, +20% Attack Speed");
                }
                else if (NearbyCount >= 3)
                {
                    RiotStatus = NearbyCount $ " enemies nearby - Stumble Immunity active!";
                    class'DKMessageManager'.static.SendImportant(KFPC, "RIOT STATUS: " $ RiotStatus);
                }
                else if (NearbyCount > 0)
                {
                    RiotStatus = NearbyCount $ " enemies nearby - Building bonuses...";
                    class'DKMessageManager'.static.SendMinor(KFPC, "RIOT STATUS: " $ RiotStatus);
                }
                else
                {
                ...
                class'DKMessageManager'.static.SendMinor(KFPC, "Combat Tips: Get surrounded by enemies for maximum power (3+ for stumble immunity, 5+ for max bonuses)");
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Riot perk not active.");
            }
        }
    }
}
...
{
    local KFPlayerController KFPC;
    local DKUpgrade_Perk_Agony_Helper AgonyHelper;
    local KFPawn OwnerPawn;
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            AgonyHelper = class'DKUpgrade_Perk_Agony'.static.GetHelper(OwnerPawn);
            if (AgonyHelper != None)
            {
                // Show current status
                ...
                class'DKMessageManager'.static.SendImportant(KFPC, "AGONY STATUS: " $ StatusMessage);
                class'DKMessageManager'.static.SendMinor(KFPC, "Upgrade Level: " $ AgonyHelper.UpgradeLevel);
                
                if (AgonyHelper.UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level)
                {
                ...
                if (AgonyHelper.UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level)
                    class'DKMessageManager'.static.SendMinor(KFPC, "Headshot extensions enabled (30% chance)");
                if (AgonyHelper.UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level)
                    class'DKMessageManager'.static.SendMinor(KFPC, "Time Banker enabled (500 dosh per 120 seconds)");
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Agony perk not active.");
            }
        }
    }
}
...
/** Load HUD preferences from client-side INI (ZedternalRBPerkpackage_Local.ini).
 *  Called once from PostBeginPlay. Values persist across sessions. */
function LoadHudPreferences()
{
    HudScaleMultiplier = class'DKConfig_HudPreferences'.static.GetHudScaleMultiplier();
    CardStackMaxY = class'DKConfig_HudPreferences'.static.GetCardStackMaxY();
    CardStackMaxCards = class'DKConfig_HudPreferences'.static.GetCardStackMaxCards();

    // Initialize rank display preferences (sets defaults on first run)
    class'DKConfig_HudPreferences'.static.InitRankPrefs();
...
event PostRender()
{
    ...
    if (Canvas != None && EventWaveOverlayID > 0 && EventWaveAlpha > 0.0f)
    {
        ...
        class'DKEventWave'.static.DrawOverlay(Canvas, EventWaveOverlayID, EventWaveAlpha,
...
event DrawHUD()
{
    ...
    if (Canvas != None)
    {
        ...
        // Total Stat Increase panel (toggled via ToggleStats exec or `mutate stats`)
        if (bShowStatPanel && PlayerOwner != None)
        {
            Canvas.Font = class'KFGameEngine'.static.GetKFCanvasFont();
            class'DKStatAggregator'.static.DrawPanel(Canvas, KFPlayerController(PlayerOwner));
        }
        
        // Scoreboard drawing handled by DKGFxScoreBoardWrapper.PostRender()
...
function DrawRankHUDElement()
{
    ...
    if (!class'DKConfig_HudPreferences'.static.GetShowRankHUD())
        return;
    ...
    // -- show "Rookie 0" with an empty progress bar instead of hiding.
    Rank = DKPRI.PlayerRank;
    TierIdx = class'ZedternalRBPerkpackage.DKRank'.static.GetTierFromRank(Rank);
    TierColor = class'ZedternalRBPerkpackage.DKRank'.static.GetTierColor(Rank);
    TierIcon = class'ZedternalRBPerkpackage.DKRank'.static.GetTierIcon(Rank);
    ...
    // a visible label, so build it from tier 0 + literal "0" at rank 0.
    if (Rank > 0)
        RankStr = class'ZedternalRBPerkpackage.DKRank'.static.GetRankDisplayString(Rank);
    else
        RankStr = class'ZedternalRBPerkpackage.DKRank'.static.GetTierDisplayName(0) @ "0";
    ...
    Progress = class'ZedternalRBPerkpackage.DKRank'.static.GetRankProgress(StoredXP);
...
function DrawEvolutionPanel()
{
    ...
    SymbioteHelper = class'DKUpgrade_Perk_Symbiote'.static.GetHelper(OwnerPawn);
...
function ToggleScavengerAdaptationPanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            ScavengerHelper = class'DKUpgrade_Perk_Scavenger'.static.GetHelper(OwnerPawn);
            if (ScavengerHelper != None)
            {
                AdaptationSummary = ScavengerHelper.GetAdaptationSummary();
                class'DKMessageManager'.static.SendImportant(KFPC, "SCAVENGER STATUS: " $ AdaptationSummary);
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Scavenger perk not active.");
            }
        }
    }
}
...
{
    local KFPlayerController KFPC;
    local DKUpgrade_Perk_Tycoon_Helper TycoonHelper;
    local KFPawn OwnerPawn;
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            TycoonHelper = class'DKUpgrade_Perk_Tycoon'.static.GetHelper(OwnerPawn);
            if (TycoonHelper != None)
            {
                PortfolioSummary = TycoonHelper.GetPortfolioSummary();
                class'DKMessageManager'.static.SendImportant(KFPC, "TYCOON STATUS: " $ PortfolioSummary);
                
                class'DKMessageManager'.static.SendMinor(KFPC, TycoonHelper.GetDoshBreakdown());
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "Tycoon perk not active.");
            }
        }
    }
}
...
function ToggleForgeWardenMilestonePanel()
{
    ...
    if (KFPC != None && KFPC.Pawn != None)
    {
        ...
        if (OwnerPawn != None)
        {
            ForgeHelper = class'DKUpgrade_Perk_ForgeWarden'.static.GetHelper(OwnerPawn);
            if (ForgeHelper != None)
            {
                MilestoneSummary = ForgeHelper.GetMilestoneSummary();
                class'DKMessageManager'.static.SendImportant(KFPC, "FORGEWARDEN STATUS: " $ MilestoneSummary);
            }
            else
            {
                class'DKMessageManager'.static.SendMinor(KFPC, "ForgeWarden perk not active.");
            }
        }
    }
}
...
function BuildEvolutionDisplay(DKUpgrade_Perk_Symbiote_Helper SymbioteHelper, out array<string> EvolutionLines)
{
    local int i;
...
simulated function UpdateCinderTracking(int BurningCount, int FireKills, float PermanentBonus, bool bPhoenix, int UpgradeLevel, optional float PhoenixTimeRemaining = 0.0f)
{
    ...
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && bPhoenix)
    {
...
simulated function UpdateHivemindTracking(int TeammatesConnected, int SwarmProgress, int SymbioteStage, bool bSwarmActive, float SwarmTimeRemaining, int UpgradeLevel)
{
    ...
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level)
    {
        ...
        if (TeammatesConnected > 0)
        {
            ...
            if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level && SymbioteStage > 0)
            {
    ...
    // ===================================================================
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && !bSwarmActive)
    {
        if (SwarmProgress >= 20)
        {
    ...
    // ===================================================================
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && bSwarmActive && SwarmTimeRemaining > 0)
    {
...
simulated function UpdateParasiteTracking(int SiphonedCount, int HarvestProgress, bool bDoubleDrain, float DrainTimeRemaining, int UpgradeLevel, bool bPulseUsed)
{
    ...
    // ===================================================================
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && !bPulseUsed)
    {
    ...
    // ===================================================================
    if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && bDoubleDrain && DrainTimeRemaining > 0)
    {
...
/**
 * Called by DKUpgrade_Perk_Shapeshifter_Helper.ClientUpdateShapeshifterHUD
...
// Build localized Mimicry-gain popup message. Called from DKPlayerController's
...
// The Gambit helper class (DKUpgrade_Perk_Gambit_Helper) replicates
...
// the active locale's [DKHudWrapper] section.
// ===================================================================
...
// Mirrors the switch in DKUpgrade_Perk_Gambit_Helper.BuildProgressString.
...
// Called from DKPlayerController.ClientGambit* RPCs to build localized
...
exec function GambitBuffs()
{
    ...
    H = None;
    foreach KFPH.ChildActors(class'DKUpgrade_Perk_Gambit_Helper', H)
    {
        break;
    }
...
function DrawMasteryStatLines(float StartX, float StartY, float LineStep, string RollsStr)
{
    ...
    while (Len(Rest) > 0)
    {
        ...
        StatName = class'DKUpgrade_Perk_Artificer_Helper'.static.GetMasteryStatName(StatIdx);
...
simulated function DrawStatOverlay()
{
    ...
    local DKUpgrade_Perk_Shapeshifter_Helper SSHelper;
    local int SSLevel;
    ...
    SSHelper = class'DKUpgrade_Perk_Shapeshifter'.static.GetHelper(KFPH);
    if (SSHelper != None)
    {
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Carnage_PerLevel * SSLevel * 100)) $ "% all damage";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Executioner_PerLevel * SSLevel * 100)) $ "% headshot dmg";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Rampage_PerLevel * SSLevel * 100)) $ "% fire rate";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Crusher_PerLevel * SSLevel * 100)) $ "% heavy melee";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Berserker_PerLevel * SSLevel * 100)) $ "% melee speed";
        ...
        {
            BuffStatusStr = "-" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Fortress_PerLevel * SSLevel * 100)) $ "% dmg taken";
        ...
        {
            BuffStatusStr = "+" $ string(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Leech_PerLevel * SSLevel) $ " HP on kill";
        ...
        {
            BuffStatusStr = "-" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Anchor_PerLevel * SSLevel * 100)) $ "% recoil";
        ...
        {
            BuffStatusStr = "-" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Hawk_PerLevel * SSLevel * 100)) $ "% spread";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Speedloader_PerLevel * SSLevel * 100)) $ "% reload spd";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Switchblade_PerLevel * SSLevel * 100)) $ "% switch spd";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Speedfreak_PerLevel * SSLevel * 100)) $ "% move speed";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Hoarder_PerLevel * SSLevel * 100)) $ "% spare ammo";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Drumfire_PerLevel * SSLevel * 100)) $ "% mag size";
        ...
        {
            BuffStatusStr = "+" $ string(int(class'DKUpgrade_Perk_Shapeshifter'.default.Buff_Phantom_PerLevel * SSLevel * 100)) $ "% ZED fire rate";
...
simulated function string GetCleanClassName(class InClass)
{
    ...
    ClsName = Repl(ClsName, "DKUpgrade_Perk_", "");
    ClsName = Repl(ClsName, "DKUpgrade_Skill_", "");
...
static function DKHudWrapper GetReaperHUD(PlayerController PC)
{
    if (PC != None && PC.myHUD != None)
        return DKHudWrapper(PC.myHUD);
    return None;
}
...
function InitializeWatcherSystem()
{
    ...
    `log("DKHudWrapper: Watcher system initialized");
}
...
// The Omen helper class (DKUpgrade_Perk_Omen_Helper) replicates
...
// [DKHudWrapper] section. Doom values are computed as -2x of reward values
 ...
 *  Must match AdvanceCondition in DKUpgrade_Perk_Hollow_Helper. */
...
function DrawEventWaveBanner()
{
    ...
    EventName = class'DKConfig_EventWave'.static.GetEventName(EventWaveOverlayID);
    EventDesc = class'DKConfig_EventWave'.static.GetEventDescription(EventWaveOverlayID);
    EventColor = class'DKEventWave'.static.GetEventColor(EventWaveOverlayID);
    EventIcon = class'DKEventWave'.static.GetEventIcon(EventWaveOverlayID);
...
// TODO: Add heartbeat SoundCue loop via DKSoundManager registration
// ===================================================================
...
function DrawSpeedsterCard()
{
    ...
    Icon = Texture2D'ZedternalRBPerkpackage_Resources.Perks.UI_Perk_Speedfreak_Rank_0';
    if (Icon != None)
    {
        Canvas.SetDrawColor(255, 255, 255, 255);
        Canvas.SetPos(BoxX + PadX, BoxY + (BoxH - IconSz) * 0.5f);
...
function DrawPossessorCard()
{
    ...
    Icon = Texture2D'ZedternalRBPerkpackage_Resources.Perks.UI_Perk_Possessor_Rank_0';
    if (Icon != None)
    {
        Canvas.SetDrawColor(255, 255, 255, 255);
        Canvas.SetPos(BoxX + PadX, BoxY + (BoxH - IconSz) * 0.5f);
...
defaultproperties
{
    ...
    // ===================================================================
    // LOCALIZED HUD STRINGS
    // English defaults live ONLY in KFGame/Localization/INT/ZedternalRBPerkpackage.int
    // under [DKHudWrapper]. UE3 forbids `localized` vars in defaultproperties
