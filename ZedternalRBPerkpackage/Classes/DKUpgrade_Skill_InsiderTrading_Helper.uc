class DKUpgrade_Skill_InsiderTrading_Helper extends Info
    transient;
...
function PayoutWaveBonus()
{
    ...
    // Calculate bonus
    DoshPerType = class'DKUpgrade_Skill_InsiderTrading'.default.DoshPerZedType[UpgradeLevel - 1];
    ...
    if (KFPC != None)
    {
        ...
        if (KFPRI != None)
        {
            ...
            class'DKMessageManager'.static.SendCritical(KFPC, "INSIDER TRADING PAYOUT: +" $ TotalBonus $ " Dosh for " $ ZedTypesKilledThisWave.Length $ " types (" $ ZedTypesList $ ")");
        }
    }
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Skill_InsiderTrading_Helper"
}