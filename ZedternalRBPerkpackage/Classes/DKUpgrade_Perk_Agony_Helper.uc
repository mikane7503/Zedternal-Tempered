// ===================================================================
// DKUpgrade_Perk_Agony_Helper - ZED Time Tracking and Dosh Rewards
// Handles: ZED time state tracking, cumulative time counter, headshot extensions
// ===================================================================
class DKUpgrade_Perk_Agony_Helper extends Info
	transient;
...
function Tick(float DeltaTime)
{
	...
	if (UpgradeLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level && bZedTimeActive)
...
function OnZedTimeStarted()
{
	...
	// DKUpgrade_Perk_Agony.ModifySpeed never applied (or stuck after ZED time
...
function TryHeadshotExtension(KFPawn OwnerPawn)
{
	...
	if (CurrentTime - LastHeadshotExtensionTime < class'DKUpgrade_Perk_Agony'.default.HeadshotExtensionCooldown)
		return;
	
	// 30% chance to extend
	ExtensionChance = class'DKUpgrade_Perk_Agony'.default.HeadshotExtensionChance;
	RandomValue = FRand();
...
{
	local KFPlayerController KFPC;
	local DKHudWrapper HudWrapper;
	
	KFPC = KFPlayerController(Player.Controller);
	if (KFPC == None)
		return;
	
	HudWrapper = DKHudWrapper(KFPC.myHUD);
	if (HudWrapper != None)
	{
...
defaultproperties
{
	Name="Default__DKUpgrade_Perk_Agony_Helper"
}