// ===================================================================
// DKPlayerController - Handles active ability system via console commands
// UPDATED: Uses DKGFxScoreBoardWrapper as HUD class
...
// ===================================================================
class DKPlayerController extends WMPlayerController;
...
// Menu references
var DKUpgradeSelectMenu UpgradeSelectMenu;
var DKCommandWheelMovie CommandWheelMovie;
var DKDomainWheelMovie DomainWheelMovie;
var DKPossessorWheelMovie PossessorWheelMovie;
...
// bodies. See DKBulkSync.uc for architecture overview.
...
simulated event PostBeginPlay()
{
    ...
    class'DKConfig_LocalArtwork'.static.InitializeConfig();
...
reliable client function ClientInitRank()
{
    ...
    StoredXP = class'ZedternalRBPerkpackage.DKConfig_Rank'.static.GetStoredXP();
    CachedClientRank = class'ZedternalRBPerkpackage.DKRank'.static.GetRankFromXP(StoredXP);

    // Send the full encoded triplet so the server can validate independently
    ServerReportRankData(
        class'DKConfig_Rank'.default.CacheRenderFrameUID,
        class'DKConfig_Rank'.default.NetRelevancySalt,
        class'DKConfig_Rank'.default.FrameValidationHash
    );
}
...
reliable server function ServerReportRankData(int EncodedXP, int Salt, int Checksum)
{
    ...
    ExpectedChecksum = class'DKConfig_Rank'.static.GenerateChecksum(EncodedXP, Salt);
    ...
    RawXP = class'DKConfig_Rank'.static.DecodeXP(EncodedXP, Salt);
    ...
    ComputedRank = class'ZedternalRBPerkpackage.DKRank'.static.GetRankFromXP(RawXP);
...
reliable server function ServerReportRank(int RankLevel)
{
    ...
    if (!class'DKConfig_RankSettings'.static.IsRankEnabled())
    {
        DKPRI.PlayerRank = 0;
        return;
    }
    ...
    {
        SteamID = class'DKConfig_ServerRank'.static.GetSteamIDFromPRI(PlayerReplicationInfo);
        if (SteamID != "")
        {
            LocalXP = class'DKConfig_ServerRank'.static.GetPlayerXP(SteamID);
            DKPRI.PlayerRank = class'ZedternalRBPerkpackage.DKRank'.static.GetRankFromXP(LocalXP);
        }
        else
            DKPRI.PlayerRank = 0;
    ...
    DKPRI.PlayerRank = Min(RankLevel, class'ZedternalRBPerkpackage.DKRank'.const.MAX_RANK);
    EnforceRankGatedPerks(DKPRI);
}
...
function EnforceRankGatedPerks(DKPlayerReplicationInfo DKPRI)
{
    ...
    for (i = 0; i < WMGRI.PerkUpgradesList.Length; ++i)
    {
        ...
        if (class'DKConfig_PerkUnlockRules'.static.GetRankRequirement(PerkClassName) <= 0)
            continue;

        // If player does NOT meet the rank requirement, lock the perk
        if (!class'DKConfig_PerkUnlockRules'.static.MeetsRankRequirement(PerkClassName, GlobalRank, 0))
        {
            if (DKPRI.IsPerkUnlocked(i))
            {
                ...
                `log("[DK_RANKPERK] Locked rank-gated perk:" @ PerkClassName @ "for" @ DKPRI.PlayerName
                    @ "(Rank" @ GlobalRank @ "< Required" @ class'DKConfig_PerkUnlockRules'.static.GetRankRequirement(PerkClassName) $ ")");
            }
        }
...
function FlushRankXP()
{
    ...
    if (PendingRankXP <= 0 || !class'DKConfig_RankSettings'.static.IsRankEnabled())
    {
        PendingRankXP = 0;
        return;
    }
    ...
    if (class'DKConfig_RankSettings'.static.IsLocalRank())
    {
        ...
        if (SteamID != "")
        {
            NewXP = class'DKConfig_ServerRank'.static.AddPlayerXP(SteamID, PendingRankXP);
            NewRank = class'ZedternalRBPerkpackage.DKRank'.static.GetRankFromXP(NewXP);
    ...
    else
    {
        ...
        if (ValidatedMaxRank < class'ZedternalRBPerkpackage.DKRank'.const.MAX_RANK)
            ValidatedMaxRank = Min(ValidatedMaxRank + 1, class'ZedternalRBPerkpackage.DKRank'.const.MAX_RANK);
...
exec function ZURank()
{
    ...
    StoredXP = class'ZedternalRBPerkpackage.DKConfig_Rank'.static.GetStoredXP();
    MyRank = class'ZedternalRBPerkpackage.DKRank'.static.GetRankFromXP(StoredXP);

    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankHeader'));
    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankStoredXP', string(StoredXP)));
    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankRank', string(MyRank), class'ZedternalRBPerkpackage.DKRank'.static.GetTierDisplayName(MyRank)));
    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankCachedClientRank', string(CachedClientRank)));
    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankPendingXP', string(PendingRankXP)));

    DKPRI = DKPlayerReplicationInfo(PlayerReplicationInfo);
    if (DKPRI != None)
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankPRIRank', string(DKPRI.PlayerRank)));
    else
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankPRIError'));

    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZURankNextAt', string(class'ZedternalRBPerkpackage.DKRank'.static.GetCumulativeXPForRank(MyRank + 1))));
}

// Debug: Set rank XP manually with "ZUSetXP <amount>"
exec function ZUSetXP(int Amount)
{
    class'ZedternalRBPerkpackage.DKConfig_Rank'.static.SaveXP(Amount);
    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZUSetXPDone', string(Amount)));
    ClientInitRank();
}

// Toggle rank-up broadcast messages from other players
exec function DKRankMessages()
{
    local bool bCurrent;
    bCurrent = class'DKConfig_HudPreferences'.static.GetShowRankUpMessages();
    class'DKConfig_HudPreferences'.static.SetShowRankUpMessages(!bCurrent);
    if (!bCurrent)
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('RankMessagesOn'));
    else
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('RankMessagesOff'));
}

// Toggle rank HUD element visibility
exec function DKRankHUD()
{
    local bool bCurrent;
    bCurrent = class'DKConfig_HudPreferences'.static.GetShowRankHUD();
    class'DKConfig_HudPreferences'.static.SetShowRankHUD(!bCurrent);
    if (!bCurrent)
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('RankHUDOn'));
    else
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('RankHUDOff'));
}
...
exec function DKMusicVolume(optional float Volume = -1.0f)
{
    ...
    if (Volume < 0.0f)
    {
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('MusicVolumeCurrent', string(int(class'DKConfig_HudPreferences'.static.GetEventMusicVolume() * 100.0f))));
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('MusicVolumeUsage'));
        return;
    }

    class'DKConfig_HudPreferences'.static.SetEventMusicVolume(Volume);
    ClientMessage(class'DKMessageManager'.static.LocalizeMessage('MusicVolumeSet', string(int(class'DKConfig_HudPreferences'.static.GetEventMusicVolume() * 100.0f))));

    // Apply immediately to active music
    if (EventMusicComponent != None)
    {
        EventMusicComponent.VolumeMultiplier = class'DKConfig_HudPreferences'.static.GetEventMusicVolume();
    }
}
...
exec function ZUEvent(optional string EventName)
{
    if (EventName == "")
    {
        ...
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZUEventHeader'));
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZUEventSensoryList'));
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZUEventGameplayList1'));
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZUEventGameplayList2'));
        ClientMessage(class'DKMessageManager'.static.LocalizeMessage('ZUEventUsage'));
        return;
    }
...
{
    local byte EventID;
    local DKGameInfo_Endless GI;
    local DKGameInfo_Endless_AllWeapons GIAW;

    if (Caps(EventName) == "CLEAR" || Caps(EventName) == "NONE" || Caps(EventName) == "OFF")
    {
        GIAW = DKGameInfo_Endless_AllWeapons(WorldInfo.Game);
        if (GIAW != None)
            GIAW.ForcedEventWaveID = 0;
        else
        {
            GI = DKGameInfo_Endless(WorldInfo.Game);
            if (GI != None)
    ...
    ClientLocalizedChat('EventWaveSet', class'DKConfig_EventWave'.static.GetEventName(EventID));
}
...
reliable client function ClientReceiveRankXP(int XPAmount)
{
    ...
    NewTotalXP = class'ZedternalRBPerkpackage.DKConfig_Rank'.static.AddXP(XPAmount);
    NewRank = class'ZedternalRBPerkpackage.DKRank'.static.GetRankFromXP(NewTotalXP);
...
reliable server function ServerBroadcastRankUp(int NewRank)
{
    ...
    if (!class'DKConfig_RankSettings'.static.IsRankUpBroadcastEnabled())
        return;
...
reliable client function ClientReceiveRankUpMessage(name MessageKey, string PlayerName, int NewRank, bool bIsSelf)
{
    ...
    if (!bIsSelf && !class'DKConfig_HudPreferences'.static.GetShowRankUpMessages())
        return;

    // Format rank using THIS client's active locale
    RankString = class'ZedternalRBPerkpackage.DKRank'.static.GetRankDisplayString(NewRank);

    // Localize the message template, substituting locale-formatted args
    if (bIsSelf)
        Message = class'DKMessageManager'.static.LocalizeMessage(MessageKey, RankString, "");
    else
        Message = class'DKMessageManager'.static.LocalizeMessage(MessageKey, PlayerName, RankString);
    ...
    CustomHUD = class'DKHudWrapper'.static.GetReaperHUD(self);
    if (CustomHUD != None)
...
// + localized stat suffixes from DKHudWrapper).
// ===================================================================
...
reliable client function ClientShapeshifterMimicryGain(int BuffIndex, string OriginalDesc, int StackCount)
{
    ...
    HUD = DKHudWrapper(MyHud);
    if (HUD == None)
        return;

    Message = HUD.FormatShapeshifterMimicryGain(BuffIndex, OriginalDesc, StackCount);
    class'DKMessageManager'.static.SendImportant(self, Message);
}
...
// own locale via DKHudWrapper.FormatGambit* helpers.
// ===================================================================
...
reliable client function ClientGambitStart(byte Rarity, byte GambitIndex, int Target, int SecondaryParam)
{
    ...
    class'DKMessageManager'.static.SendCritical(self, Message);
}
...
function OpenUpgradeSelectMenu()
{
    ...
    // Create fresh menu
    UpgradeSelectMenu = new class'DKUpgradeSelectMenu';
    UpgradeSelectMenu.Init(LocalPlayer(Player));
    `log("[DK_ROGUELIKE_UI] Created new DKUpgradeSelectMenu");
...
reliable server function ServerSelectRoguelikeUpgrade(int OptionIndex)
{
    ...
    local DKGameInfo_Endless_AllWeapons DKGI_AW;
    ...
    // Try AllWeapons mode
    DKGI_AW = DKGameInfo_Endless_AllWeapons(WorldInfo.Game);
...
// ===================================================================
// DELUXE SKILL UPGRADE (on-demand dosh sink, DKConfig_DeluxeUpgrade)
...
function bool IsDeluxeSkillEligible(DKGameReplicationInfo DKGRI, DKPlayerReplicationInfo DKPRI, int SkillIndex)
{
    local int i, PerkIdx;
    local string PerkPath;
...
function ApplyDeluxeUpgrade(DKGameReplicationInfo DKGRI, DKPlayerReplicationInfo DKPRI, int SkillIndex)
{
...
{
    local DK_Config_Keybindings KeyConfig;
    
    // Only run on the client that owns this controller
    if (Role == ROLE_Authority)
    {
        `log("DKPlayerController: CheckAndApplyDefaultKeybindings called on SERVER - skipping");
        return;
    }
    
    if (bHasCheckedKeybindings)
    {
        `log("DKPlayerController [CLIENT]: Already checked keybindings");
        return;
    }
        
    bHasCheckedKeybindings = true;
    
    `log("DKPlayerController [CLIENT]: Starting keybinding check...");
    
    // Load the config
    KeyConfig = new class'DK_Config_Keybindings';
    
    // Initialize defaults if config is empty
    class'DK_Config_Keybindings'.static.InitializeDefaults(KeyConfig);
    
    `log("DKPlayerController [CLIENT]: Config loaded, bHasAppliedDefaults =" @ KeyConfig.bHasAppliedDefaults);
    
    // Check if we've already applied defaults for this user
    if (!KeyConfig.bHasAppliedDefaults)
    {
        `log("DKPlayerController [CLIENT]: First time - applying default keybindings...");
        ...
        `log("DKPlayerController [CLIENT]: Saved config with bHasAppliedDefaults = true");
    ...
    {
        `log("DKPlayerController [CLIENT]: Keybindings already applied previously - skipping");
    }
...
simulated function ApplyDefaultKeybindings(DK_Config_Keybindings KeyConfig)
{
    local int i;
    ...
    `log("DKPlayerController [CLIENT]: Starting to apply default keybindings...");
    ...
    for (i = 0; i < KeyConfig.DefaultKeybindings.Length; i++)
    {
        ...
        if (!ShouldOverrideKey(KeyConfig.DefaultKeybindings[i].KeyName))
        {
            ...
            `log("DKPlayerController [CLIENT]: Applied keybind -" @ KeyConfig.DefaultKeybindings[i].KeyName @ "=" @ KeyConfig.DefaultKeybindings[i].Command);
        }
        else
        {
            `log("DKPlayerController [CLIENT]: Skipped keybind -" @ KeyConfig.DefaultKeybindings[i].KeyName @ "(already bound to critical command)");
        }
    }
    
    `log("DKPlayerController [CLIENT]: Applied" @ SuccessCount @ "of" @ KeyConfig.DefaultKeybindings.Length @ "default keybindings");
}
...
reliable server function BuyPerkUpgrade(int ItemDefinition, int Cost)
{
	...
	if (Pawn == None) return;
	DKPRI = DKPlayerReplicationInfo(Pawn.PlayerReplicationInfo);
	if (DKPRI == None) return;

	CurrentLevel = DKPRI.GetPerkLevel(ItemDefinition);
	PerkCap = class'DKPlayerReplicationInfo'.const.DK_MAX_PERKS;

	// --- Perk diversity limit ---
	MaxPerks = class'DKConfig_PerkLimit'.default.Player_MaxDifferentPerks;
	if (MaxPerks > 0 && CurrentLevel == 0)
	{
		...
		// DKUI_UPGMenu.ClientPerkBuyAllowed mirrors this for the client pre-check.
		if (class'DKConfig_PerkLimit'.default.Player_ProgressivePerkUnlock)
	...
	R1Level = class'DKConfig_Capstone'.default.Capstone_Rank1Level;
	R2Level = class'DKConfig_Capstone'.default.Capstone_Rank2Level;
	MaxR1 = class'DKConfig_Capstone'.default.Capstone_MaxActiveRank1;
	MaxR2 = class'DKConfig_Capstone'.default.Capstone_MaxActiveRank2;
...
reliable server function ServerActivateHyde()
{
    ...
    H = class'DKUpgrade_Perk_JekyllHyde'.static.GetHelper(Pawn);
    if (H == None)
    {
        class'DKMessageManager'.static.SendMinor(self, "Hyde Serum: you do not have the Jekyll & Hyde perk.");
        return;
    }
...
reliable server function ServerDomainPress()
{
    ...
    H = class'DKUpgrade_Perk_Domain'.static.FindHelper(Pawn);
    if (H == None)
    {
        class'DKMessageManager'.static.SendMinor(self, "Domain: you do not have the Domain perk.");
        return;
    }
...
reliable client function ClientOpenDomainWheel(string CooldownData)
{
    ...
    DomainWheelMovie = new class'DKDomainWheelMovie';
...
reliable server function ServerPossessorPress()
{
    ...
    {
        H = class'DKUpgrade_Perk_Possessor'.static.FindHelper(PuppetSavedHuman);
    ...
    H = class'DKUpgrade_Perk_Possessor'.static.FindHelper(Pawn);
    if (H == None)
    {
        class'DKMessageManager'.static.SendMinor(self, "Possession: you do not have the Possessor perk.");
        return;
    }
    ...
    if (H.bOnCooldown)
    {
        ...
        class'DKMessageManager'.static.SendMinor(self, "Possession recharging: " $ RemainingSecs $ "s");
        return;
    }
...
reliable client function ClientOpenPossessorWheel(string SnapshotData)
{
    ...
    PossessorWheelMovie = new class'DKPossessorWheelMovie';
...
reliable server function ServerActivateAbilitySlot(int SlotIndex)
{
    ...
    local DKUpgrade_Skill_Swiftness_Helper SwiftnessHelper;
    local DKUpgrade_Skill_TacticalFocus_Helper TacticalFocusHelper;
    ...
    local DKUpgrade_Skill_Wallhack_Helper WallhackHelper;
    
    if (SlotIndex < 0 || SlotIndex >= AbilitySlots.Length)
    {
        class'DKMessageManager'.static.SendMinorLoc(self, 'AbilitySlotEmpty', string(SlotIndex + 1));
        return;
    }
    ...
    SwiftnessHelper = DKUpgrade_Skill_Swiftness_Helper(Slot.Helper);
    ...
    TacticalFocusHelper = DKUpgrade_Skill_TacticalFocus_Helper(Slot.Helper);
    ...
    ExplosiveFuryHelper = DKUpgrade_Skill_ExplosiveFury_Helper(Slot.Helper);
    ...
    FieldSurgeryHelper = DKUpgrade_Skill_FieldSurgery_Helper(Slot.Helper);
    ...
    PyromaniacRushHelper = DKUpgrade_Skill_PyromaniacRush_Helper(Slot.Helper);
    ...
    ScavengersLuckHelper = DKUpgrade_Skill_ScavengersLuck_Helper(Slot.Helper);
    ...
    TacticalShieldHelper = DKUpgrade_Skill_TacticalShield_Helper(Slot.Helper);
    ...
    BreachingChargeHelper = DKUpgrade_Skill_BreachingCharge_Helper(Slot.Helper);
    ...
    ChronoshiftHelper = DKUpgrade_Skill_Chronoshift_Helper(Slot.Helper);
    ...
    InfernoHelper = DKUpgrade_Skill_Inferno_Helper(Slot.Helper);
    ...
    BullseyeHelper = DKUpgrade_Skill_Bullseye_Helper(Slot.Helper);
    ...
    WallhackHelper = DKUpgrade_Skill_Wallhack_Helper(Slot.Helper);
    ...
    class'DKMessageManager'.static.SendMinorLoc(self, 'UnknownAbilityType', string(SlotIndex + 1));
}
...
function bool RegisterAbility(string AbilityName, Info Helper, class<Info> HelperClass, optional Texture2D AbilityIcon)
{
    ...
    {
        `log("DKPlayerController: Ability already registered in slot" @ EmptySlot);
        return true;
    }
    ...
    if (EmptySlot != INDEX_NONE)
    {
        ...
        `log("DKPlayerController: Replaced stale" @ HelperClass @ "ability in slot" @ EmptySlot);
        return true;
    }
    ...
    if (EmptySlot == INDEX_NONE)
    {
        class'DKMessageManager'.static.SendImportantLoc(self, 'AllAbilitySlotsFull');
        return false;
    }
    ...
    class'DKMessageManager'.static.SendImportantLoc(self, 'AbilityAdded', AbilityName, string(EmptySlot + 1));
    
    `log("DKPlayerController: Registered ability" @ AbilityName @ "in slot" @ EmptySlot);
    return true;
}

reliable client function ClientRegisterAbility(int SlotIndex, string AbilityName, Texture2D AbilityIcon)
{
    local DKGFxScoreBoardWrapper HUD;
    local string HUDTypeName;
    
    HUD = DKGFxScoreBoardWrapper(myHUD);
    if (HUD != None)
    {
        HUD.UpdateAbilitySlot(SlotIndex, AbilityName, true, AbilityIcon);
        `log("DKPlayerController (CLIENT): Registered ability" @ AbilityName @ "in HUD slot" @ SlotIndex);
    }
    else
    {
        if (myHUD != None)
        ...
        `log("DKPlayerController (CLIENT): ERROR - HUD is not DKGFxScoreBoardWrapper! Actual type:" @ HUDTypeName);
    }
}
...
function bool UnregisterAbility(Info Helper)
{
    ...
    `log("DKPlayerController: Unregistered ability from slot" @ SlotIndex);
    return true;
}
...
reliable client function ClientUnregisterAbility(int SlotIndex)
{
    ...
    if (HUD != None)
    {
        ...
        `log("DKPlayerController (CLIENT): Unregistered ability from HUD slot" @ SlotIndex);
    }
}
...
{
    local DKHudWrapper WendigoHUD;

    WendigoHUD = class'DKHudWrapper'.static.GetReaperHUD(self);
    if (WendigoHUD == None)
        return;
...
reliable client function ClientPlayChronoshiftSound(SoundCue ActivationSound)
{
    if (ActivationSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController [CLIENT]: Played Chronoshift activation sound");
    }
}
...
reliable client function ClientPlayAchievementSound(SoundCue AchievementSound)
{
    if (AchievementSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController [CLIENT]: Played achievement unlock sound");
    }
}
...
reliable client function ClientPlayPerkUnlockSound(SoundCue PerkUnlockSound)
{
    if (PerkUnlockSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController [CLIENT]: Played perk unlock sound");
    }
}
...
reliable client function ClientPlayInfernoSound(SoundCue InfernoSound)
{
    if (InfernoSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController (CLIENT): Played Inferno sound");
    }
}
...
reliable client function ClientPlayPhoenixProtocolSound(SoundCue PhoenixSound)
{
    if (PhoenixSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController (CLIENT): Played Phoenix Protocol sound");
    }
}
...
reliable client function ClientPlayWraithFormSound(SoundCue WraithSound)
{
    if (WraithSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController (CLIENT): Played Wraith Form sound");
    }
}
...
reliable client function ClientPlayWatcherSound(SoundCue WatcherSound)
{
    if (WatcherSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController (CLIENT): Played Watcher sound");
    }
}
...
{
    local DKHudWrapper DomainHUD;

    DomainHUD = class'DKHudWrapper'.static.GetReaperHUD(self);
    if (DomainHUD != None)
...
reliable client function ClientPlayDetonatorSound(SoundCue DetonatorSound)
{
    if (DetonatorSound != None && Pawn != None)
    {
        ...
        `log("DKPlayerController (CLIENT): Played Detonator sound");
    }
}
...
reliable server function ServerReplaceAbilitySlot(int SlotIndex)
{
    ...
    {
        class'DKMessageManager'.static.SendMinorLoc(self, 'InvalidSlot');
        return;
    }
    ...
    {
        class'DKMessageManager'.static.SendMinorLoc(self, 'SlotAlreadyEmpty', string(SlotIndex + 1));
        return;
    }
    ...
    class'DKMessageManager'.static.SendImportantLoc(self, 'SlotCleared', string(SlotIndex + 1));
}
...
exec function ListAbilities()
{
    ...
    class'DKMessageManager'.static.SendImportantLoc(self, 'ActiveAbilitiesHeader');
    
    for (i = 0; i < 4; i++)
    {
        if (i < AbilitySlots.Length && AbilitySlots[i].Helper != None)
        {
            class'DKMessageManager'.static.SendMinorLoc(self, 'SlotStatusFilled', string(i + 1), AbilitySlots[i].AbilityName);
        }
        else
        {
            class'DKMessageManager'.static.SendMinorLoc(self, 'SlotStatusEmpty', string(i + 1));
        }
    }
}
 ...
 *  Saved to ZedternalRBPerkpackage_Local.ini and persists across sessions.
 *
 *  Examples:
...
{
    local DKHudWrapper DKHUD;

    DKHUD = DKHudWrapper(myHUD);
    if (DKHUD == None)
        return;
    ...
    class'DKConfig_HudPreferences'.static.SetHudScaleMultiplier(Scale);
...
exec function DKHudInfo()
{
    ...
    Tier = class'DKHudWrapper'.static.GetResolutionTierName(DKHUD.Canvas.SizeY);
...
exec function DKHudCardLimit(float MaxY)
{
    ...
    class'DKConfig_HudPreferences'.static.SetCardStackMaxY(MaxY);
...
exec function DKHollowComplete(optional string WeaponArg)
{
    ...
    HollowHelper = class'DKUpgrade_Perk_Hollow'.static.GetHelper(Pawn);
    ...
    {
        CandidateName = class'DKHollowWeaponData'.static.GetHollowNormName(i);
...
reliable server function ServerDropTrophy(string CategoryArg)
{
    ...
    H = class'DKUpgrade_Perk_Predator'.static.GetHelper(Pawn);

    if (H == None)
    {
        ClientMessage("DropTrophy: No Predator helper (do you have the Predator perk?)");
        class'DKMessageManager'.static.SendMinorLoc(self, 'PredatorNotActive');
        return;
    }
    ...
    if (CategoryArg == "")
    {
        ...
        class'DKMessageManager'.static.SendMinorLoc(self, 'TrophiesInventory', string(H.TotalTrophies), H.GetInventoryString());
        return;
    }
    ...
    if (CategoryArg ~= "all")
    {
        ...
        class'DKMessageManager'.static.SendImportantLoc(self, 'DiscardedAllTrophies', string(Discarded));
        return;
    }
    ...
    {
        SetIdx = class'DKUpgrade_Perk_Predator_Helper'.static.SetFromName(Mid(CategoryArg, 4));
        ...
        if (H.DiscardSet(SetIdx))
            ClientMessage("Discarded set:" @ class'DKUpgrade_Perk_Predator_Helper'.static.GetSetName(SetIdx) $ ". Remaining sets:" @ H.GetCompletedSetsString());
        else
            ClientMessage(class'DKUpgrade_Perk_Predator_Helper'.static.GetSetName(SetIdx) @ "is not completed.");
        return;
    }

    // Try trophy category name lookup
    Cat = class'DKUpgrade_Perk_Predator_Helper'.static.CategoryFromName(CategoryArg);
    if (Cat == 255)
    {
        // Maybe they meant a set name without the "set" prefix
        SetIdx = class'DKUpgrade_Perk_Predator_Helper'.static.SetFromName(CategoryArg);
        if (SetIdx != INDEX_NONE)
        {
    ...
    {
        ClientMessage("Dropped" @ class'DKUpgrade_Perk_Predator_Helper'.static.GetCategoryName(Cat) @ "trophy. Remaining:" @ H.GetInventoryString());
        class'DKMessageManager'.static.SendMinorLoc(self, 'DroppedTrophy', class'DKUpgrade_Perk_Predator_Helper'.static.GetCategoryName(Cat));
    }
    else
    {
        ClientMessage("No" @ class'DKUpgrade_Perk_Predator_Helper'.static.GetCategoryName(Cat) @ "trophy to drop.");
        class'DKMessageManager'.static.SendMinorLoc(self, 'NoTrophyToDrop', class'DKUpgrade_Perk_Predator_Helper'.static.GetCategoryName(Cat));
    }
}
...
reliable server function ServerTestUnique(string ForcedID)
{
    local DKGameInfo_Endless DKGI;
    local DKGameInfo_Endless_AllWeapons DKGI_AW;
    local DKRoguelikeUpgradeManager Mgr;
    local DKPlayerReplicationInfo DKPRI;
    local int i, j, Count;
...
// Player configures slots via INI (DKConfig_CommandWheel).
// ===================================================================
...
exec function DKWheel()
{
    ...
    CommandWheelMovie = new class'DKCommandWheelMovie';
...
// Toggle the DKStatAggregator HUD overlay that aggregates passive
// bonuses from all owned perks/skills/equipment + roguelike + flags
// vanilla perk wrappers with conditional weapon-gated bonuses.
//
// Bind via:   setbind J ToggleStats
// Or use:     mutate stats   (routes through DKMutator -> ClientToggleStats)
// ===================================================================
exec function ToggleStats()
{
    ...
    DKHUD = DKHudWrapper(myHUD);
    if (DKHUD != None)
        DKHUD.ToggleStatPanel();
}

reliable client function ClientToggleStats()
{
    local DKHudWrapper DKHUD;
...
//                                       replication)
//
//  See DKBulkSync.uc for architecture overview. Struct definitions and
 ...
 *  DKGameInfo_Endless.PostLogin (server side) with a small delay so the
...
function ServerStartBulkSync(WMGameReplicationInfo InGRI, WMGameInfo_Endless InGI)
{
    ...
    SetTimer(class'DKBulkSync'.const.BULK_SEND_INTERVAL, true, 'BulkSyncSendTimer');
}
...
function ServerSendStartingWeaponEntry()
{
    ...
    ClientReceiveStartingWeaponEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.StartingWeaponsList[Idx].KFWeapon));
...
function ServerSendPerkUpgradeEntry()
{
    ...
    ClientReceivePerkUpgradeEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.PerkUpgradesList[Idx].PerkUpgrade),
        PriceVal);
...
function ServerSendSkillUpgradeEntry()
{
    ...
    ClientReceiveSkillUpgradeEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.SkillUpgradesList[Idx].SkillUpgrade),
...
function ServerSendWeaponUpgradeEntry()
{
    ...
    ClientReceiveWeaponUpgradeEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.WeaponUpgradesList[Idx].WeaponUpgrade),
...
function ServerSendEquipmentUpgradeEntry()
{
    ...
    ClientReceiveEquipmentUpgradeEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.EquipmentUpgradesList[Idx].EquipmentUpgrade),
...
function ServerSendSidearmEntry()
{
    ...
    ClientReceiveSidearmEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.SidearmsList[Idx].Sidearm),
...
function ServerSendGrenadeEntry()
{
    ...
    ClientReceiveGrenadeEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.GrenadesList[Idx].Grenade));
...
function ServerSendSpecialWaveEntry()
{
    ...
    ClientReceiveSpecialWaveEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.SpecialWavesList[Idx].SpecialWave));
...
function ServerSendZedBuffEntry()
{
    ...
    ClientReceiveZedBuffEntry(
        Idx, Total,
        class'DKBulkSync'.static.SafeClassPath(BulkSendGRI.ZedBuffsList[Idx].ZedBuff));
...
// (DKGameReplicationInfo.SlotUpgIdx_* / WeaponSlotCnt_*) and silently dropped
...
// populated + finalized by BOTH DKGameInfo_Endless and _AllWeapons before any
...
function ServerSendSlotCompositionChunk()
{
    ...
    DKGRI = DKGameReplicationInfo(BulkSendGRI);
    if (DKGRI == None)
    {
    ...
    ChunkSz  = class'DKBulkSync'.const.SLOT_CHUNK_BYTES;
    NumSlots = Min(DKGRI.ServerSlotUpgIdxRecord.Length, class'DKGameReplicationInfo'.const.DK_SLOT_PAGES_MAX);
    ...
    if (Chunk < SlotChunks)
    {
        ...
        {
            PackedHex $= class'DKBulkSync'.static.ByteToHex2(DKGRI.ServerSlotUpgIdxRecord[StartByte + j]);
        }
    }
    else
    {
        ...
        for (j = 0; j < ChunkBytes; ++j)
        {
            PackedHex $= class'DKBulkSync'.static.ByteToHex2(DKGRI.ServerWeaponSlotCntRecord[StartByte + j]);
        }
    }
...
reliable client function ClientReceiveSlotCompositionChunk(int ChunkIdx, int TotalChunks, byte Kind, int StartByte, int NumBytesInChunk, string PackedHex, int Checksum)
{
    ...
    if (DKGRI == None)
    {
        `log("[DK_BULKSYNC] ERROR: SlotComposition chunk but GRI is not DKGameReplicationInfo");
        return;
    }
    ...
    {
        hi = class'DKBulkSync'.static.HexCharToInt(Mid(PackedHex, j * 2, 1));
        lo = class'DKBulkSync'.static.HexCharToInt(Mid(PackedHex, j * 2 + 1, 1));
        b = hi * 16 + lo;
...
reliable client function ClientBulkSyncComplete()
{
    ...
    {
        `log("[DK_BULKSYNC] ERROR: Client received completion but GRI is not DKGameReplicationInfo");
    }
}
...
// DKGameReplicationInfo.IsSlotDataComplete() stays false forever,
// GenerateWeaponUpgrades() is skipped, and the trader shows no weapon
// upgrades. DKGameReplicationInfo.SyncTimer drives RequestSlotResync()
...
simulated function IngestAllowedWeaponRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestTraderWeaponDefRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestStartingWeaponRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestPerkUpgradeRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestSkillUpgradeRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestWeaponUpgradeRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestEquipmentUpgradeRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestSidearmRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestGrenadeRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
    local class<KFWeaponDefinition> Loaded;
...
simulated function IngestSpecialWaveRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
simulated function IngestZedBuffRoster(DKGameReplicationInfo DKGRI)
{
    local int i;
...
function IngestRosterIntoGRI(int RosterID)
{
    ...
    if (DKGRI == None)
    {
        `log("[DK_BULKSYNC] ERROR: IngestRosterIntoGRI(" $ RosterID $ ") -- GRI is not DKGameReplicationInfo");
        return;
    }
...
// client builder (DKGameReplicationInfo.GenerateWeaponUpgrades) is gated on
...
simulated function IngestSlotCompositionRoster(DKGameReplicationInfo DKGRI)
{
    local int i, sumCnts;
...
// DK_Config_Keybindings -> ActivateSpeedster). Mirrors the dedicated-key
...
reliable server function ServerActivateSpeedster()
{
    ...
    foreach Pawn.ChildActors(class'DKUpgrade_Perk_Speedster_Helper', H)
    {
        H.TryActivate();
        return;
    }
...
exec function PuppetScrakeEmperor() { PuppetBeginVersusInput(); ServerPuppetGrabClass(class'DKPawn_ZedScrake_Emperor_Puppet'); }
// [PUPPET] Generic Case A test hook. Console: `PuppetCustom ZedScrake_Emperor` ->
// DKPawn_ZedScrake_Emperor_Puppet; `PuppetCustom ZedCrawler_Mini`, etc. Resolves
...
exec function PuppetCustom(string N)
{
    ...
    ZedClass = class<KFPawn_Monster>(DynamicLoadObject("ZedternalRBPerkpackage.DKPawn_"$N$"_Puppet", class'Class'));
    if (ZedClass != none)
    {
...
// Bosses: Patriarch works via DKPawn_ZedPatriarch_Puppet - a de-bossed
...
exec function PuppetPatriarch()  { PuppetBeginVersusInput(); ServerPuppetGrabClass(class'DKPawn_ZedPatriarch_Puppet'); }
...
{
    local DKPawn_Doppelganger_Decoy Decoy;
    ...
    Decoy = Spawn(class'DKPawn_Doppelganger_Decoy', self, , SpawnLoc, Pawn.Rotation);
    if (Decoy == None)
    {
...
// ---- Patriarch-only extra attacks (bound J/K/P by DK_Config_Keybindings) ----
...
// DKPawn_ZedPatriarch_Puppet.CollectMortarTargets (vanilla hunts humans).
...
defaultproperties
{
    PerkList(0)=(PerkClass=class'ZedternalRBPerkpackage.DKPerk')
    CheatClass=class'ZedternalRBPerkpackage.DKCheatManager'
    ...
    Name="Default__DKPlayerController"
}
