// ===================================================================
// DKUpgrade_Skill_QuickDraw_Helper - Manages Quick Draw ability state
// ===================================================================
class DKUpgrade_Skill_QuickDraw_Helper extends Info
	transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, DKPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'DKUpgrade_Skill_QuickDraw'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'DKUpgrade_Skill_QuickDraw'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Quick Draw", self, class'DKUpgrade_Skill_QuickDraw_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Quick Draw: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Quick Draw: Already active!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'DKMessageManager'.static.SendMinor(DKPC, "Quick Draw: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "QUICK DRAW ACTIVATED! +40% fire rate for 10 seconds!");
	}
	else
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "QUICK DRAW (DELUXE) ACTIVATED! +60% fire rate for 10 seconds!");
	}
...
function Deactivate()
{
	...
	class'DKMessageManager'.static.SendMinor(DKPC, "Quick Draw ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'DKMessageManager'.static.SendImportant(DKPC, "Quick Draw ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Skill_QuickDraw_Helper"
}