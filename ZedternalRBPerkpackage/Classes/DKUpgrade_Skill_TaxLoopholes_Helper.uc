class DKUpgrade_Skill_TaxLoopholes_Helper extends Info
    transient;
...
function CheckForSpending()
{
    ...
    if (DoshDelta > 0)
    {
        ...
        SpendingThreshold = class'DKUpgrade_Skill_TaxLoopholes'.default.SpendingThreshold;
        ...
        if (NewMilestones > LastRefundMilestone)
        {
            ...
            RefundAmount = class'DKUpgrade_Skill_TaxLoopholes'.default.TaxRefundAmount[UpgradeLevel - 1];
            ...
            if (KFPC != None)
            {
                ...
                if (KFPRI != None)
                {
                    ...
                    if (RefundsToGive == 1)
                        class'DKMessageManager'.static.SendCritical(KFPC, "TAX LOOPHOLE ACTIVATED! +" $ RefundAmount $ " Dosh refund (milestone " $ NewMilestones $ ")");
                    else
                        class'DKMessageManager'.static.SendCritical(KFPC, "TAX LOOPHOLES ACTIVATED! +" $ (RefundAmount * RefundsToGive) $ " Dosh refunds (" $ RefundsToGive $ " milestones)");

                    class'DKMessageManager'.static.SendMinor(KFPC, "Total Spent: " $ TotalDoshSpent $ " | Next Refund at: " $ ((NewMilestones + 1) * SpendingThreshold));
                }
            }
...
function NotifyTycoonOfExternalDosh(KFPlayerController KFPC, int Amount)
{
    ...
    foreach KFPC.Pawn.ChildActors(class'DKUpgrade_Perk_Tycoon_Helper', TycoonHelper)
    {
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Skill_TaxLoopholes_Helper"
}
