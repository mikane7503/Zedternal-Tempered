// ===================================================================
// DKUpgrade_Skill_Inferno_Helper - Manages Inferno ability state
// Ignites all enemies in line of sight with powerful fire DoT
// ===================================================================
class DKUpgrade_Skill_Inferno_Helper extends Info
	transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, DKPlayerController InDKPC)
{
	...
	{
		`log("Inferno_Helper: Found DKMutator, requesting sound...");
	...
	if (Mutator == None)
	{
		`log("Inferno_Helper: ERROR - Could not find DKMutator!");
	}
	
	if (InTier == 1)
		AbilityIcon = class'DKUpgrade_Skill_Inferno'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'DKUpgrade_Skill_Inferno'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Inferno", self, class'DKUpgrade_Skill_Inferno_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Inferno: Cannot activate while dead!");
		return;
	}
...
function UpdateAbility()
{
	...
	if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'DKMessageManager'.static.SendImportant(DKPC, "Inferno ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Skill_Inferno_Helper"
}
