// ===================================================================
// DKUpgrade_Perk_MissingNO_Helper - State tracker for MissingNO perk
//
// Tracks:
...
//
// Sounds (registered in DKSoundManager.RegisterDefaultSounds, played via
//        DKPlayerController.ClientPlayBuffSound per SoundImplementationGuide.md):
...
// ===================================================================
class DKUpgrade_Perk_MissingNO_Helper extends Info
	transient;
...
// Number of rotating damage types in the pool (matches DKUpgrade_Perk_MissingNO.GetRotatingDT)
...
function OnWaveEnd()
{
	...
	if (UpgradeLevel >= 20)
	{
		if (FRand() < class'DKUpgrade_Perk_MissingNO'.default.WaveEndDuplicateChance)
			TryDuplicateWeapon();
	}
}
...
// registered SoundCue via DKSoundManager and routes playback through
// DKPlayerController.ClientPlayBuffSound (a reliable client RPC).
//
...
function PlayMissingNOSound(name SoundID)
{
	...
	if (Mutator == None)
	{
		`log("MissingNO Helper: Sound playback failed -" @ SoundID @ "- DKMutator not found");
		return;
	}
	...
	if (Sound == None)
	{
		`log("MissingNO Helper: Sound" @ SoundID @ "not registered in DKSoundManager");
		return;
	}
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Perk_MissingNO_Helper"
}
