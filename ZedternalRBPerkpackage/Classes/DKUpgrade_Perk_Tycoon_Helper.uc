class DKUpgrade_Perk_Tycoon_Helper extends Info
    transient;
    ...
    if (DoshChange > 0)
    {
        ...
        else
        {
            ...
            if (ThrowBuffer > 0)
            {
                ...
                {
                    DiscountRate = class'DKUpgrade_Perk_Tycoon'.default.BulkDiscount * TycoonUpgradeLevel;
...
function ApplyPortfolioMilestone()
{
    ...
    if (KFPC != None)
    {
        ...
        TycoonHUD = class'DKHudWrapper'.static.GetReaperHUD(KFPC);
        if (TycoonHUD != None)
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_Tycoon_Helper"
}
