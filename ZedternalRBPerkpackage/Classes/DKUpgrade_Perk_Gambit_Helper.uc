...
// ===================================================================
class DKUpgrade_Perk_Gambit_Helper extends Actor;
...
// Called from DKUpgrade_Perk_Gambit.WaveEnd
// ===================================================================
...
function OnWaveEnd(KFPlayerController KFPC)
{
    ...
    if (bGambitCompleted)
    {
        ...
        GiveRewards(KFPC);
        DKPlayerController(KFPC).ClientGambitAutoComplete(ActiveGambitIndex);
    ...
    else
    {
        ...
        DKPlayerController(KFPC).ClientGambitExpired(ActiveGambitIndex);
...
function RollNewGambit()
{
    ...
    for (i = 0; i < GambitPool.Length; i++)
    {
        ...
        else if (GambitPool[i].Rarity == RARITY_RARE && PerkLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level)
        {
            EligibleIndices.AddItem(i);
            EligibleWeights.AddItem(GambitPool[i].RollWeight);
        }
        else if (GambitPool[i].Rarity == RARITY_LEGENDARY && PerkLevel >= class'DKConfig_Capstone'.default.Capstone_Rank2Level)
        {
            EligibleIndices.AddItem(i);
            EligibleWeights.AddItem(GambitPool[i].RollWeight);
        }
        else if (GambitPool[i].Rarity == RARITY_MYTHIC && PerkLevel >= class'DKConfig_Capstone'.default.Capstone_Rank1Level)
        {
...
function ActivateGambit(int GambitIndex)
{
    ...
    if (Player != None)
    {
        DKPlayerController(Player.Controller).ClientGambitStart(
...
// ===================================================================
// EVENT HANDLERS -- Called from DKUpgrade_Perk_Gambit static functions
// ===================================================================
...
function OnDamageDealt(KFPawn_Monster MyKFPM, int InDamage, int HitZoneIdx, KFWeapon MyKFW)
{
    ...
    if (WildCardLvl > 0 && !bGambitCompleted)
    {
        ...
        if (FRand() < WildCardChance)
        {
            ...
            if (Player != None && Player.Controller != None)
            {
                class'DKMessageManager'.static.SendCriticalLoc(
...
function CheckGambitComplete()
{
    ...
    if (bGambitCompleted)
    {
        ...
        if (Player != None && Player.Controller != None)
        {
            DKPlayerController(Player.Controller).ClientGambitMidWaveComplete(ActiveGambitIndex);
        }
    }
}
...
function GiveRewards(KFPlayerController KFPC)
{
    ...
    if (RoyalFlushLvl > 0)
    {
        ...
        if (((TotalCompletions + 1) % RoyalFlushInterval) == 0)
        {
            ...
            class'DKMessageManager'.static.SendCriticalLoc(KFPC, 'GambitRoyalFlush');
    ...
    if (DoNLvl > 0)
    {
        ...
        if (FRand() < FlipChance)
        {
            ...
            class'DKMessageManager'.static.SendCriticalLoc(KFPC, 'GambitDoubleOrNothingWin');
        }
        else
        {
            ...
            class'DKMessageManager'.static.SendImportantLoc(KFPC, 'GambitDoubleOrNothingBust');
        }
    }
    ...
    DKPlayerController(KFPC).ClientGambitReward(
...
// ===================================================================
// BLUFF -- Called from DKUpgrade_Skill_Bluff.ModifyDamageTaken
...
// ===================================================================
// ACE UP YOUR SLEEVE -- Called from DKUpgrade_Skill_AceUpYourSleeve
...
// Client RPC -- receives display data and pushes to DKHudWrapper
...
// ===================================================================
// HUD WRAPPER INTEGRATION -- Push display data to DKHudWrapper
// ===================================================================
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_Gambit_Helper"
}
