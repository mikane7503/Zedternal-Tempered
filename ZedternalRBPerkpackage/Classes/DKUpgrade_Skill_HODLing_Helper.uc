class DKUpgrade_Skill_HODLing_Helper extends Info
    transient;
...
function PostBeginPlay()
{
    ...
    else
    {
        ...
        HODLDuration = class'DKUpgrade_Skill_HODLing'.default.HODLDuration[UpgradeLevel - 1];
...
function CheckHODLStatus()
{
    ...
    if (CurrentDosh < LastKnownDosh)
    {
        ...
        if (bCurrentlyHODLing)
        {
            ...
            if (KFPC != None)
            {
                ...
                class'DKMessageManager'.static.SendImportant(KFPC, "HODL BROKEN! Spending detected - HODL timer reset");
            }
        }
    ...
    if (bCurrentlyHODLing && bWaveInProgress)
    {
        ...
        if (ElapsedHODLTime >= HODLDuration)
        {
            ...
            InterestAmount = int(float(CurrentDosh) * class'DKUpgrade_Skill_HODLing'.default.InterestRate);
            ...
            if (InterestAmount > 0)
            {
                ...
                if (KFPC != None)
                {
                    ...
                    if (KFPRI != None)
                    {
                        ...
                        class'DKMessageManager'.static.SendCritical(KFPC, "HODL COMPLETED! +" $ InterestAmount $ " Dosh interest (50% of " $ CurrentDosh $ ")");
                    }
                }
            }
        ...
        else
        {
            ...
            if (int(RemainingTime) % 60 == 0 && int(RemainingTime) <= 300) // Show every minute in last 5 minutes
            {
                ...
                if (KFPC != None)
                {
                    ...
                    class'DKMessageManager'.static.SendMinor(KFPC, "HODL Progress: " $ int(RemainingTime) $ " seconds remaining for 50% interest");
                }
            }
        }
    }
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Skill_HODLing_Helper"
}