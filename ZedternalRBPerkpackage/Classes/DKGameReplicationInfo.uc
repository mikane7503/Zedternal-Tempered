// ===================================================================
// DKGameReplicationInfo - Extends WMGameReplicationInfo
...
//   DKPlayerReplicationInfo and DK_MAX_WEAPON_UPGRADES=16384 in the
//   GameInfo classes).
//
...
// ===================================================================
class DKGameReplicationInfo extends WMGameReplicationInfo;
...
// DKGameInfo_Endless / DKGameInfo_Endless_AllWeapons. Copied into the
...
// purchase arrays in DKPlayerReplicationInfo.
...
// MOVED to DKPlayerReplicationInfo (per-player). The unlock flags used to
...
// Replicated so DKUI_UPGMenu can pre-check purchases client-side.
...
// the server rejects via DKPlayerController.BuyPerkUpgrade.
// Pushed by DKGameInfo_Endless.PostBeginPlay from DKConfig_PerkLimit
// and DKConfig_Capstone.
// ===================================================================
...
// ===================================================================
// DELUXE SKILL UPGRADE (pushed by DKGameInfo_Endless from
// DKConfig_DeluxeUpgrade). Replicated so DKUI_UPGMenu can build and
...
// have been pushed into DKConfig_Capstone's class defaults (see Tick). Not
// replicated.
...
replication
{
    if (bNetDirty)
        ...
        // now travels over the reliable DKBulkSync stream (BR_SlotComposition) --
        ...
        // weapon-upgrade counts (host 13, friend 2). See DKPlayerController's
...
// Called by DKGameInfo_Endless / _AllWeapons from RepGameInfoLowPriority
...
// The DKUI_UPGMenu already skips entries with KFWeapon == None.
// ===================================================================
...
// The unlock flags moved to DKPlayerReplicationInfo. This GRI accessor is
...
simulated function DKPlayerReplicationInfo GetLocalDKPRI()
{
    ...
    return DKPlayerReplicationInfo(PC.PlayerReplicationInfo);
}
...
simulated function bool IsItemAllowed(STraderItem Item)
{
    ...
    // DKGameInfo_Endless desynced the weapon-upgrade lists between
    ...
    // Mirrors DKGameReplicationInfo_AllWeapons.IsItemAllowed.
...
simulated event Tick(float DeltaTime)
{
    ...
    // class'DKConfig_Capstone'.default.Capstone_RankXLevel directly. On a client
    ...
    if (!bCapstoneSyncedToConfig && WorldInfo.NetMode == NM_Client
        && class'DKConfig_Capstone'.default.Capstone_Rank1Level <= 0)
    {
        class'DKConfig_Capstone'.default.Capstone_Rank1Level = 10;
        class'DKConfig_Capstone'.default.Capstone_Rank2Level = 20;
    }

    if (!bCapstoneSyncedToConfig && CapstoneR1Level > 0 && WorldInfo.NetMode == NM_Client)
    {
        class'DKConfig_Capstone'.default.Capstone_Rank1Level = CapstoneR1Level;
        class'DKConfig_Capstone'.default.Capstone_Rank2Level = CapstoneR2Level;
...
// =====================================================================
//
//  BULK SYNC INGEST  (called from DKPlayerController after each roster
...
// ---------------------------------------------------------------------
// INGEST FUNCTIONS — MOVED TO DKPlayerController.uc
// ---------------------------------------------------------------------
// UE3 forbids cross-class struct references in function parameter lists
// (e.g. array<DKPlayerController.FBulkX>), so the ingest functions and
// their RecvBuf_X buffers must live in the same class as the FBulkX
// struct definitions: DKPlayerController. Functions are now in
// DKPlayerController.uc; each takes a DKGameReplicationInfo parameter
// and writes directly to its lists.

// ---------------------------------------------------------------------
// COMPLETION HANDSHAKE
// ---------------------------------------------------------------------
// Called from DKPlayerController.ClientBulkSyncComplete after all 11
...
defaultproperties
{
    ...
    Name="Default__DKGameReplicationInfo"
}
