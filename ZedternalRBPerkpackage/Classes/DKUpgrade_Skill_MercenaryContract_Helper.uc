class DKUpgrade_Skill_MercenaryContract_Helper extends Info
    transient;
...
function OnEnemyKilled(KFPawn_Monster KilledZed)
{
    ...
    KillsNeeded = class'DKUpgrade_Skill_MercenaryContract'.default.KillsRequired[UpgradeLevel - 1];
    
    if (ConsecutiveKills >= KillsNeeded)
    {
        // Contract completed! Give reward
        DoshReward = class'DKUpgrade_Skill_MercenaryContract'.default.DoshReward[UpgradeLevel - 1];
        ...
        if (KFPC != None)
        {
            ...
            if (KFPRI != None)
            {
                ...
                class'DKMessageManager'.static.SendCritical(KFPC, "MERCENARY CONTRACT COMPLETED! +" $ DoshReward $ " Dosh for " $ KillsNeeded $ " " $ GetZedTypeName(CurrentZedType) $ " kills!");
            }
        }
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Skill_MercenaryContract_Helper"
}