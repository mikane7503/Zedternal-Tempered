...
function OpenMenu(DKPlayerController NewOwner)
{
    DKPC = NewOwner;
    
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: OpenMenu called");
    ...
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Menu opened");
}
...
function SetUpgradeOptions(array<string> UpgradeIDs, array<string> AccumulatedDisplays)
{
    ...
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: SetUpgradeOptions with " $ UpgradeIDs.Length $ " options");
    ...
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Sent options='" $ OptionsDataStr $ "' accumulated='" $ AccumulatedDataStr $ "' to Flash");
}

function CloseMenu()
{
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: CloseMenu called");
    ...
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Menu closed");
}
...
{
    local DKPlayerController TempDKPC;
    
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Callback_UpgradeSelected SlotIndex=" $ SlotIndex);
    
    if (SlotIndex < 0 || SlotIndex >= CurrentUpgradeIDs.Length)
    {
        `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: ERROR - Invalid SlotIndex!");
        return;
    }
    ...
    if (DKPC != None)
    {
        ...
        `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Sent selection to server");
    }
}
...
{
    `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Upgrade " $ SlotIndex $ " hovered");
...
function TrySelectUpgradeBySlot(int SlotIndex)
{
    if (SlotIndex >= 0 && SlotIndex < CurrentUpgradeIDs.Length)
    {
        `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Keyboard selected slot " $ SlotIndex);
        Callback_UpgradeSelected(SlotIndex);
    }
    else
    {
        `log("[DK_ROGUELIKE_UI] DKUpgradeSelectMenu: Invalid slot " $ SlotIndex);
    }
}
