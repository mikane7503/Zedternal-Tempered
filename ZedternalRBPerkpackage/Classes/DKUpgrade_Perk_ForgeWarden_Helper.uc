class DKUpgrade_Perk_ForgeWarden_Helper extends Info
    transient;
...
{
    local DKBurningGroundPatch BurningPatch;
    local KFPlayerController KFPC;
    
    if (Owner == None || Owner.WorldInfo == None) return;
    
    // Spawn the burning ground patch
    BurningPatch = Owner.Spawn(class'DKBurningGroundPatch', Owner,, SpawnLocation);
...
reliable client function UpdateKillDisplay(byte KillCount, optional bool bMilestoneComplete = False)
{
    ...
    ForgeHUD = class'DKHudWrapper'.static.GetReaperHUD(KFPC);
    if (ForgeHUD != None)
    {
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_ForgeWarden_Helper"
}