// ===================================================================
// DKEventWaveManager — Server-side event wave logic controller
//
...
// ===================================================================
class DKEventWaveManager extends Info;
...
function StartEvent(byte EventID)
{
	...
	`log("[DK_EVENTWAVE_MGR] Started event:" @ class'DKConfig_EventWave'.static.GetEventName(EventID));
}

function EndEvent()
{
	`log("[DK_EVENTWAVE_MGR] Ending event:" @ class'DKConfig_EventWave'.static.GetEventName(ActiveEventID));
...
function StartXMen()
{
	...
	for (i = 0; i < Players.Length; i++)
	{
		...
		if (DKPlayerController(Players[i]) != None)
		{
			DKPlayerController(Players[i]).ClientReceiveXMenPower(
...
function EndXMen()
{
	...
	for (i = 0; i < XMenPlayers.Length; i++)
	{
		...
		// Clear HUD display
		if (DKPlayerController(XMenPlayers[i].PC) != None)
		{
			DKPlayerController(XMenPlayers[i].PC).ClientReceiveXMenPower("", "");
		}
	}
...
function DKGameReplicationInfo GetDKGRI()
{
	return DKGameReplicationInfo(WorldInfo.GRI);
}
...
function ClearAllClientMinigameData()
{
	...
	{
		DKPC.ClientClearEventMinigameData();
	}
}

// Plays a DKSoundManager-registered one-shot cue to every player
function PlayCueToAll(name CueID)
{
	...
	Cue = class'DKSoundManager'.static.GetSound(Mut, CueID);
	if (Cue == None)
		return;
...
function NotifyZedKilledGeneric(Controller Killer, Pawn KilledPawn)
{
	...
	switch (ActiveEventID)
	{
		case 29: // Floor Is Lava: kills scored from inside the zone pay bonus dosh
			DKGRI = GetDKGRI();
			if (DKGRI != None && KFPC != None && KFPC.Pawn != None
				&& VSize2D(KFPC.Pawn.Location - DKGRI.EventZoneLoc) <= LAVA_RADIUS
		...
		case 31: // Bounty Board: count kills of the killer's assigned category
			...
			if (GetZedCategory(KilledPawn) != BountyCat[Idx])
				break;
			++BountyHave[Idx];
			if (DKPlayerController(KFPC) != None)
				DKPlayerController(KFPC).ClientUpdateEventQuota(BountyHave[Idx]);
			if (BountyHave[Idx] >= BountyNeed[Idx])
			{
				PlayCueToPC('EventBounty_Complete', KFPC);
				class'DKMessageManager'.static.SendImportant(KFPC, "BOUNTY COMPLETE!");
...
function BroadcastMinigameMessage(string Msg)
{
	...
	foreach WorldInfo.AllControllers(class'KFPlayerController', KFPC)
	{
		class'DKMessageManager'.static.SendImportant(KFPC, Msg);
	}
}
...
function ArmBombOn(KFPlayerController NewHolder, float Fuse)
{
	...
	class'DKMessageManager'.static.SendCritical(TargetPC, "YOU HAVE THE BOMB! Touch a teammate to pass it!");
}
...
function TransferBomb(KFPlayerController NewHolder)
{
	...
	class'DKMessageManager'.static.SendMinor(TargetPC, "Bomb passed to" @ NewHolder.PlayerReplicationInfo.PlayerName $ "!");
...
function SetGRISwapSeconds(byte Seconds)
{
	...
	if (DKGRI != None)
	{
		DKGRI.EventSwapInterval = Seconds;
		DKGRI.bForceNetUpdate = True;
	}
}
...
function BuildLavaAnchors()
{
	...
	`log("[DK_EVENTWAVE_MGR] Lava anchors:" @ LavaAnchors.Length);
}
...
function LavaTelegraph()
{
	...
	NextLavaLoc = PickLavaAnchor(DKGRI.EventZoneLoc, false);
	SetGRIPhase(1);
...
function LavaTick()
{
	...
	foreach WorldInfo.AllControllers(class'KFPlayerController', KFPC)
	{
		KFPH = KFPawn_Human(KFPC.Pawn);
		if (KFPH != None && KFPH.IsAliveAndWell()
			&& VSize2D(KFPH.Location - DKGRI.EventZoneLoc) > LAVA_RADIUS)
		{
...
function StartBodyguardBond()
{
	...
	{
		DKPC = DKPlayerController(BondPCs[i]);
		if (DKPC != None && BondPCs[BondPartner[i]] != None)
		{
			DKPC.ClientSetEventPartner(BondPCs[BondPartner[i]].PlayerReplicationInfo.PlayerName);
			class'DKMessageManager'.static.SendImportant(DKPC, "BONDED with" @ BondPCs[BondPartner[i]].PlayerReplicationInfo.PlayerName $ "! Stay close!");
		}
	}
}
...
function StartBountyBoard()
{
	...
	foreach WorldInfo.AllControllers(class'KFPlayerController', KFPC)
	{
		...
		DKPC = DKPlayerController(KFPC);
		if (DKPC != None)
			DKPC.ClientSetEventQuota(GetCategoryName(Cat), Need);

		class'DKMessageManager'.static.SendImportant(KFPC, "YOUR BOUNTY:" @ Need @ GetCategoryName(Cat) $ "!");
	}
	...
	{
		DKGRI.EventDataA = 0;
		DKGRI.EventDataB = byte(BountyPCs.Length);
		DKGRI.bForceNetUpdate = True;
	}
}
...
function EndBountyBoard()
{
	...
	for (i = 0; i < BountyPCs.Length; ++i)
	{
		...
		class'DKMessageManager'.static.SendCritical(BountyPCs[i], "BOUNTY BOARD COMPLETE! +" $ BOUNTY_REWARD_DOSH @ "Dosh!");
	}
}
...
function PickGoldenZed()
{
	...
	if ((GoldenZed != None && GoldenZed.IsAliveAndWell()) || (DKGRI != None && DKGRI.EventPhase == 2))
	{
...
defaultproperties
{
	...
	Name="Default__DKEventWaveManager"
}
