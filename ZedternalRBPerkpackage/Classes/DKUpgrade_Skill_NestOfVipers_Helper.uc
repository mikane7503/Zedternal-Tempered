class DKUpgrade_Skill_NestOfVipers_Helper extends Info
    transient;
...
function ActivateStationaryCloud()
{
    ...
    if (KFPC != None)
    {
        ...
        class'DKMessageManager'.static.SendImportant(KFPC, "Nest of Vipers: Poison cloud activated!");
    }
}
...
function ActivateMovingCloud()
{
    ...
    if (KFPC != None)
    {
        ...
        class'DKMessageManager'.static.SendImportant(KFPC, "Nest of Vipers: Poison cloud follows you for " $ int(MovingCloudDuration) $ " seconds!");
    }
}
...
function DeactivateMovingCloud()
{
    ...
    if (KFPC != None)
    {
        ...
        class'DKMessageManager'.static.SendMinor(KFPC, "Nest of Vipers: Moving poison cloud dissipated");
    }
}
...
function ApplyPoisonCloudDamage()
{
    ...
    for (i = 0; i < AffectedMonsters.Length; i++)
    {
        ...
        {
            AffectedMonsters[i].TakeDamage(CloudDamage, Player.Controller, AffectedMonsters[i].Location, vect(0,0,0), class'DKDT_Medusa_Poison');
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Skill_NestOfVipers_Helper"
}