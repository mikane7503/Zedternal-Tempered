// ===================================================================
// DKMutator - Consolidated Gameplay Mutator
// Handles: Achievements, Special Wave Integration, Sound Management
// FIXED: Removed duplicate perk filtering (DKGameInfo_Endless owns it)
// FIXED: Singleplayer player registration (NotifyLogin never fires)
// ===================================================================
class DKMutator extends Mutator;
...
// Achievement System
var DKAchievementData AchievementData;
var array<DKAchievementReplicator> PlayerReplicators;
...
event PostBeginPlay()
{
	...
	// ===================================================================
	`log("===== DKMutator: Initializing Systems =====");
	class'DKSoundManager'.static.Initialize(self);
	`log("DKMutator: Sound manager initialized with" @ CustomSounds.Length @ "sounds");
	...
	`log("DKMutator: Boss Wave config initialized");
	
	// Initialize Achievement System
	AchievementData = new class'DKAchievementData';
	if (AchievementData == None)
	{
		`log("DKMutator Error: Failed to create achievement data!");
		return;
	}
	AchievementData.InitializeAchievements();
	class'DKConfig_PerkUnlockRules'.static.ApplyAchievementPerkLinks(AchievementData);
	
	`log("===== DKMutator: System Initialized =====");
	`log("DKMutator: Loaded" @ AchievementData.Achievements.Length @ "achievements");
	...
	{
		`log("DKMutator: Standalone mode detected, scheduling existing player registration");
...
function InitializeEventHooks()
{
	...
	`log("DKMutator: Event hooks initialized");
}
...
function NotifyLogin(Controller NewPlayer)
{
	...
	if (KFPC != None)
	{
		`log("DKMutator: Player connected, creating replicator and stats");
...
function RegisterExistingPlayers()
{
	...
	foreach WorldInfo.AllControllers(class'KFPlayerController', KFPC)
	{
		if (FindPlayerStatIndex(KFPC) == INDEX_NONE)
		{
			...
			InitializePlayerStats(KFPC);
			RegisteredCount++;
			`log("DKMutator: Registered existing player" @ KFPC.PlayerReplicationInfo.PlayerName @ "(singleplayer fix)");
		}
	}
	
	if (RegisteredCount > 0)
	{
		`log("DKMutator: Registered" @ RegisteredCount @ "existing player(s) via singleplayer fix");
	}
	else
	{
		// Player might not have a controller yet; retry once more
		`log("DKMutator: No unregistered players found, scheduling one more attempt");
...
function RegisterExistingPlayersRetry()
{
	...
	foreach WorldInfo.AllControllers(class'KFPlayerController', KFPC)
	{
		if (FindPlayerStatIndex(KFPC) == INDEX_NONE)
		{
			CreateReplicatorForPlayer(KFPC);
			InitializePlayerStats(KFPC);
			`log("DKMutator: Registered existing player on retry:" @ KFPC.PlayerReplicationInfo.PlayerName);
		}
	}
}
...
{
	local DKAchievementReplicator Replicator;
	
	// Don't create duplicates
	if (FindReplicatorIndex(KFPC) != INDEX_NONE)
	{
		`log("DKMutator: Replicator already exists for player, skipping");
		return;
	}
	
	Replicator = Spawn(class'DKAchievementReplicator', KFPC);
	if (Replicator != None)
	{
		Replicator.OwningPC = KFPC;
		Replicator.Mutator = self;
		PlayerReplicators.AddItem(Replicator);
		`log("DKMutator: Created achievement replicator for player");
	}
}
...
function InitializePlayerStats(KFPlayerController KFPC)
{
	...
	if (FindPlayerStatIndex(KFPC) != INDEX_NONE)
	{
		`log("DKMutator: Stats already exist for player, skipping");
		return;
	}
	...
	if (WMPRI != None)
	{
		...
		`log("DKMutator: Initialized DoshAtWaveStart for new player:" @ WMPRI.Score);
	}
	else
	{
...
// Perk filtering is handled entirely by DKGameInfo_Endless.
...
function OnTraderOpened()
{
	...
	for (i = 0; i < PlayerStats.Length; i++)
	{
		...
		if (WMPRI != None)
		{
			...
			`log("DKMutator: Player" @ i @ "dosh baseline:" @ PlayerStats[i].DoshAtWaveStart);
		}
	}
...
function OnWaveStarted()
{
	...
	for (i = 0; i < PlayerStats.Length; i++)
	{
		...
		if (!PlayerStats[i].bSpentDoshThisWave)
		{
			...
			`log("DKMutator: Player" @ i @ "completed trader without spending dosh. Streak:" @ PlayerStats[i].ConsecutiveWavesWithoutTrader);
		...
		{
			`log("DKMutator: Player" @ i @ "spent dosh. Resetting no-trader streak (was" @ PlayerStats[i].ConsecutiveWavesWithoutTrader @ ")");
...
function OnWaveEnded()
{
	...
	for (i = 0; i < PlayerStats.Length; i++)
	{
		...
		{
			`log("DKMutator: Skipping achievement checks for player" @ i @ "- wave was not active");
			continue;
		}
		...
		if (!PlayerStats[i].bTookDamageThisWave)
		{
			...
			`log("DKMutator: Player" @ i @ "completed wave without damage. Streak:" @ PlayerStats[i].ConsecutiveWavesNoDamage);
			CheckAchievement(KFPC, 'Untouchable');
		}
		else
		{
			`log("DKMutator: Player" @ i @ "took damage. Resetting no-damage streak (was" @ PlayerStats[i].ConsecutiveWavesNoDamage @ ")");
	...
	for (i = 0; i < PlayerStats.Length; i++)
	{
		...
		if (WMPRI != None)
		{
			...
			if (CurrentDosh < PlayerStats[i].DoshAtWaveStart && !PlayerStats[i].bSpentDoshThisWave)
			{
				...
				`log("DKMutator: Player" @ i @ "spent dosh (was" @ PlayerStats[i].DoshAtWaveStart @ "now" @ CurrentDosh @ ")");
			}
		}
	}
}
...
// These are still needed by DKAchievementReplicator.UnlockPerkFromAchievement()
...
function ScoreKill(Controller Killer, Controller Killed)
{
	...
	if (CurrentBossWave.bIsActive && KilledMonster.IsA(CurrentBossWave.BossClass.Name))
	{
		...
		`log("DKMutator: Boss killed (" $ CurrentBossWave.BossesKilled $ "/" $ CurrentBossWave.BossesSpawned $ ")");
	}
...
function NotifyPlayerKilled(Controller Killer, Controller Killed, Pawn KilledPawn, class<DamageType> DamageType)
{
	...
	`log("DKMutator: Player" @ StatIdx @ "died. Resetting survival streak (was" @ PlayerStats[StatIdx].ConsecutiveWavesAlive @ ")");
...
function CheckAchievement(KFPlayerController KFPC, name AchievementID)
{
	...
	local DKAchievementData.AchievementDefinition Achievement;
...
function StartBossWave(class<KFPawn_Monster> BossClass, int BossCount)
{
	...
	`log("DKMutator: Started boss wave -" @ BossClass @ "Count:" @ BossCount);
}
...
function MonitorBossWave()
{
	...
	if (CurrentBossWave.BossesKilled >= CurrentBossWave.BossesSpawned)
	{
		...
		`log("DKMutator: Boss wave completed in" @ ElapsedTime @ "seconds");
...
function CheckBossWaveAchievements(float CompletionTime)
{
	...
	foreach WorldInfo.AllControllers(class'KFPlayerController', KFPC)
	{
		...
		`log("DKMutator: Awarding BossWaveComplete achievement to player");
		
		if (CompletionTime < 180.0f)
		{
			CheckAchievement(KFPC, 'BossWaveSpeed');
			`log("DKMutator: Awarding BossWaveSpeed achievement to player (completed in" @ CompletionTime @ "seconds)");
		}
	}
}
...
{
	return class'DKSoundManager'.static.GetSound(self, SoundID);
}
...
function Mutate(string MutateString, PlayerController Sender)
{
	...
	// Toggle the stat panel HUD overlay (mutate stats)
	if (Cmd ~= "stats")
	{
		if (DKPlayerController(Sender) != None)
			DKPlayerController(Sender).ClientToggleStats();
		return;
	}
...
function HandlePresetCommand(string Arg, PlayerController Sender)
{
	...
	if (PresetIndex < 0 || PresetIndex >= class'DKPresetData'.static.GetPresetCount())
	{
	...
	PresetName = class'DKPresetData'.static.GetPresetName(PresetIndex);
	...
	class'DKPresetData'.static.ApplyPreset(PresetIndex, WorldInfo);
}
...
function ListPresets(PlayerController Sender)
{
	...
	for (i = 0; i < class'DKPresetData'.static.GetPresetCount(); ++i)
	{
		Sender.ClientMessage("  " $ i $ " -" @ class'DKPresetData'.static.GetPresetName(i));
	}
}

defaultproperties
{
	Name="Default__DKMutator"
}
