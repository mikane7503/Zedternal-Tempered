// ===================================================================
// DKUpgrade_Skill_FieldSurgery_Helper - Manages Field Surgery ability state
// ===================================================================
class DKUpgrade_Skill_FieldSurgery_Helper extends Info
	transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, DKPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'DKUpgrade_Skill_FieldSurgery'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'DKUpgrade_Skill_FieldSurgery'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Field Surgery", self, class'DKUpgrade_Skill_FieldSurgery_Helper', AbilityIcon))
	{
...
function SetTier(int InTier)
{
	...
	HealAmount = class'DKUpgrade_Skill_FieldSurgery'.default.HealAmount[Tier - 1];
...
function TryActivate()
{
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Field Surgery: Cannot activate while dead!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'DKMessageManager'.static.SendMinor(DKPC, "Field Surgery: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "FIELD SURGERY! Healed " $ HealedAmount $ " HP!");
	}
	else
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "FIELD SURGERY (DELUXE)! Healed " $ HealedAmount $ " HP!");
	}
...
function UpdateAbility()
{
	...
	if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'DKMessageManager'.static.SendImportant(DKPC, "Field Surgery ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Skill_FieldSurgery_Helper"
}