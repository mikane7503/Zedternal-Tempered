// ===================================================================
// DKGameInfo_Endless_AllWeapons - All Weapons mode variant
...
// ===================================================================
class DKGameInfo_Endless_AllWeapons extends WMGameInfo_Endless_AllWeapons;
...
// ComputeUpgradeSlotBudget (called from PickWeapons) when weapons x
// config would exceed DK_MAX_WEAPON_UPGRADES.
...
var byte ForcedEventWaveID;    // Set by exec command, consumed on next wave start
var DKEventWaveManager EventWaveManager;

// ===================================================================
// BULK SYNC SYSTEM — server-side trigger
// ===================================================================
// Mirrors DKGameInfo_Endless.PostLogin. See that class for design notes.
...
event InitGame(string Options, out string ErrorMessage)
{
    ...
    // MIRRORED in DKGameInfo_Endless / DKGameInfo_Endless_AllWeapons.
    MigrateRenamedSkillPaths();

    Super.InitGame(Options, ErrorMessage);
    
    `log("[DKGameInfo_AllWeapons] Initializing DK Zedternal Reborn Extension");
    ...
    if (PerkConfig != None)
    {
        ...
        `log("[DKGameInfo_AllWeapons] PerkFilterConfig initialized:" @ PerkConfig.UnlockRules.Length @ "unlock rules," @ PerkConfig.AchievementLockedPerks.Length @ "achievement locks");
    }
    else
    {
        `log("[DKGameInfo_AllWeapons] ERROR: Failed to create PerkFilterConfig!");
    }
}
...
// MIRRORED in DKGameInfo_Endless.
...
event PostBeginPlay()
{
    ...
    if (DKGRI != None)
    {
        ...
        // DKPlayerController.BuyPerkUpgrade). Same push as DKGameInfo_Endless.
...
// ===================================================================
// BULK SYNC TRIGGER — PostLogin override (parallel to DKGameInfo_Endless)
// ===================================================================
...
function InitializeDefaultPerkRules()
{
    ...
    `log("[DKGameInfo_AllWeapons] Perk rules: " $ PerkConfig.UnlockRules.Length $ " prerequisite rules, " $ PerkConfig.AchievementLockedPerks.Length $ " achievement locks, " $ PerkConfig.ExclusionRules.Length $ " exclusion rules");
}
...
function ApplyPerkFilterRules(WMPlayerReplicationInfo WMPRI)
{
    ...
    // Pass 1: Lock achievement-locked perks and rank-gated perks
...
{
    // Register all normal weapons first (all unlocked in AllWeapons mode)
    ...
    // and never consumes them). Precious variants are now allowed again,
    // restoring exact ZR parity. RemovePreciousWeapons() is kept below as
    // dead code for reference. Same fix as DKGameInfo_Endless.

    // Append all Reforged weapons to trader
...
{
    local int i, x, Choice, RegisteredCount, IDCount, SlotsBefore, WeaponSlotsBefore;
    ...
    DKGRI = DKGameReplicationInfo(MyKFGRI);
    WMGRI = WMGameReplicationInfo(MyKFGRI);
    if (DKGRI == None || WMGRI == None)
    {
        `log("ZR Artificer ERROR: GRI cast failed! Reforged weapons will not be registered.");
        return;
    }
    ...
    `log("ZR Artificer: ConfigData.ValidWeaponUpgrades.Length =" @ ConfigData.ValidWeaponUpgrades.Length
        @ "| WeaponUpgObjects.Length =" @ ConfigData.WeaponUpgObjects.Length
        @ "| UpgradesPerWeapon =" @ class'ZedternalReborn.Config_WeaponUpgradeOptions'.default.WeaponUpgrade_NumberUpgradePerWeapon);
    ...
    for (i = 0; i < default.ReforgedWeaponDefPaths.Length; ++i)
    {
        ...
        KFW = class<KFWeapon>(DynamicLoadObject(WD.default.WeaponClassPath, class'Class', True));
        BuyPrice = WD.default.BuyPrice;

        // 1. AllowedWeaponsList
            ...
            && WMGRI.WeaponUpgradeSlotsList.Length < DK_MAX_WEAPON_UPGRADES)
        {
            ...
            AllowedUpgrades_Idx.Length = 0;
            StaticUpgrades.Length = 0;
            StaticUpgrades_PU.Length = 0;
            StaticUpgrades_PM.Length = 0;
            StaticUpgrades_ML.Length = 0;
            StaticUpgrades_Idx.Length = 0;
            HollowUpgrades.Length = 0;
            HollowUpgrades_PU.Length = 0;
            HollowUpgrades_PM.Length = 0;
            HollowUpgrades_ML.Length = 0;
            HollowUpgrades_Idx.Length = 0;

            for (x = 0; x < Min(256, ConfigData.ValidWeaponUpgrades.Length); ++x)
            {
                if (ConfigData.WeaponUpgObjects[x].static.IsUpgradeCompatible(KFW))
                {
                    ...
                    if (class<DKWeaponUpg_HollowBase>(ConfigData.WeaponUpgObjects[x]) != None)
                    {
                        HollowUpgrades.AddItem(ConfigData.WeaponUpgObjects[x]);
                        HollowUpgrades_PU.AddItem(ConfigData.ValidWeaponUpgrades[x].PriceUnit);
                        HollowUpgrades_PM.AddItem(ConfigData.ValidWeaponUpgrades[x].PriceMultiplier);
                        HollowUpgrades_ML.AddItem(ConfigData.ValidWeaponUpgrades[x].MaxLevel);
                        HollowUpgrades_Idx.AddItem(byte(x));
                    }
                    else if (ConfigData.ValidWeaponUpgrades[x].bIsStatic)
                    {
                        StaticUpgrades.AddItem(ConfigData.WeaponUpgObjects[x]);
                        StaticUpgrades_PU.AddItem(ConfigData.ValidWeaponUpgrades[x].PriceUnit);
                        StaticUpgrades_PM.AddItem(ConfigData.ValidWeaponUpgrades[x].PriceMultiplier);
                        StaticUpgrades_ML.AddItem(ConfigData.ValidWeaponUpgrades[x].MaxLevel);
                        StaticUpgrades_Idx.AddItem(byte(x));
                    }
                    else
                    {
                        AllowedUpgrades.AddItem(ConfigData.WeaponUpgObjects[x]);
                        AllowedUpgrades_PU.AddItem(ConfigData.ValidWeaponUpgrades[x].PriceUnit);
                        AllowedUpgrades_PM.AddItem(ConfigData.ValidWeaponUpgrades[x].PriceMultiplier);
                        AllowedUpgrades_ML.AddItem(ConfigData.ValidWeaponUpgrades[x].MaxLevel);
                        AllowedUpgrades_Idx.AddItem(byte(x));
                    }
                }
            }

            // Per-weapon bound: budget-aware and byte-safe (<=255) for
            // count recording
            for (x = 0; x < GetEffectiveUpgradesPerWeapon(); ++x)
            {
            ...
            for (x = 0; x < HollowUpgrades.Length; ++x)
            {
                if (WMGRI.WeaponUpgradeSlotsList.Length >= DK_MAX_WEAPON_UPGRADES)
                {
                    `log("[DK_UPGSLOTS] WARNING: global slot cap (" $ DK_MAX_WEAPON_UPGRADES $ ") reached while adding Hollow slots at Reforged weapon" @ WMGRI.AllowedWeaponsList.Length @ "(" $ WD $ ")");
                    break;
                }

                WMGRI.AddWeaponUpgrade(KFW, HollowUpgrades[x], HollowUpgrades_PU[x], HollowUpgrades_PM[x], BuyPrice, HollowUpgrades_ML[x]);
                DKGRI.ServerSlotUpgIdxRecord.AddItem(HollowUpgrades_Idx[x]);
            }
        }
        else if (KFW == None)
        {
            `log("ZR Artificer WARNING: Failed to load weapon class:" @ WD.default.WeaponClassPath);
        }

        // DK OPTION2: record this weapon's slot count -- exactly ONE entry
        // per AllowedWeaponsList entry (this AddAllowedWeapon call),
    ...
    `log("ZR Artificer: AllowedWeaponsList.Length =" @ WMGRI.AllowedWeaponsList.Length
        @ "| SaleItems.Length =" @ TraderItems.SaleItems.Length
        @ "| KFWeaponDefPath.Length =" @ KFWeaponDefPath.Length);
    `log("ZR Artificer: WeaponUpgradeSlots:" @ SlotsBefore @ "->" @ WMGRI.WeaponUpgradeSlotsList.Length
        @ "(" $ (WMGRI.WeaponUpgradeSlotsList.Length - SlotsBefore) @ "new slots for Reforged weapons)");
...
function OpenTrader()
{
    ...
    // End event wave manager logic + clear GRI
    DKGRI = DKGameReplicationInfo(MyKFGRI);
...
function RepGameInfoHighPriority()
{
    ...
    if (DKGRI != None)
    {
        `log("DK Extended Limits (AllWeapons): AllowedWeapons=" $ WMGRI.NumberOfAllowedWeapons
...
// per-weapon count so every weapon gets an equal share instead of the
// first N weapons getting everything and the rest getting nothing.
// ===================================================================

function PickWeapons(out array<int> WeaponIndex)
{
    ComputeUpgradeSlotBudget(WeaponIndex.Length);
    super.PickWeapons(WeaponIndex);
}

function ComputeUpgradeSlotBudget(int MainWeaponCount)
{
    local int TotalWeapons, ConfigPerWeapon;

    TotalWeapons = MainWeaponCount + default.ReforgedWeaponDefPaths.Length;
    if (class'DKConfig_HollowWeapons'.static.IsEnabled())
        TotalWeapons += default.HollowWeaponDefPaths.Length;

    ConfigPerWeapon = Min(255, class'ZedternalReborn.Config_WeaponUpgradeOptions'.default.WeaponUpgrade_NumberUpgradePerWeapon);

    if (TotalWeapons > 0 && ConfigPerWeapon * TotalWeapons > DK_MAX_WEAPON_UPGRADES)
    {
        EffectiveUpgradesPerWeapon = DK_MAX_WEAPON_UPGRADES / TotalWeapons;
        `log("[DK_UPGSLOTS] Slot budget:" @ TotalWeapons @ "weapons x" @ ConfigPerWeapon @ "upgrades =" @ ConfigPerWeapon * TotalWeapons @ "wanted > cap" @ DK_MAX_WEAPON_UPGRADES @ "-- clamping to" @ EffectiveUpgradesPerWeapon @ "upgrades per weapon so every weapon gets an equal share");
    }
    else
    {
        EffectiveUpgradesPerWeapon = ConfigPerWeapon;
        `log("[DK_UPGSLOTS] Slot budget:" @ TotalWeapons @ "weapons x" @ ConfigPerWeapon @ "upgrades =" @ ConfigPerWeapon * TotalWeapons @ "slots (cap" @ DK_MAX_WEAPON_UPGRADES $ ") -- no clamp needed");
    }
}

// Returns the per-weapon upgrade slot bound. 0 = budget not computed
// yet (shouldn't happen in AllWeapons; PickWeapons sets it before any
// AddWeaponInTrader call) -- falls back to the raw config value.
// Min(255) keeps the per-weapon count byte-safe for Option-2 recording.
...
function AddWeaponInTrader(const class<KFWeaponDefinition> KFWD)
{
    ...
    // DK FIX: Precious early-return stays REMOVED (ZR parity, see
    // DKGameInfo_Endless). With Option 2 slot replication the client no
    // longer replays the RNG at all, so special-casing is unnecessary.
...
function SpawnBalanceRepHelper()
{
    ...
    if (H != None)
    {
        ...
        `log("[DKGameInfo] Balance helper spawned: 19 wrappers populated (weapon wrappers removed)");
    }
    else
    {
...
function ReduceDamage(out int Damage, Pawn Injured, Controller InstigatedBy, vector HitLocation, out vector Momentum, class<DamageType> DamageType, Actor DamageCauser, TraceHitInfo HitInfo)
{
    ...
    local int AmogusDmg;
    local DKPlayerController PuppetPC;
    local DKPlayerController PuppetHumanPC;
    ...
    if (EventWaveManager != None && EventWaveManager.ActiveEventID > 0)
    {
        // Hot Potato / Highlander damage modification
        Damage = EventWaveManager.ModifyEventDamage(Damage, Injured, InstigatedBy);

        // Amogus: if FF zeroed damage from impostor to teammate, restore half
    ...
    // PUPPET MASTER SPIKE: a player-possessed zed must never actually die.
    // Cap any lethal hit to non-lethal and flag the controller to revert to
    // the human, so a zed "death" becomes "transform ends" with no game-over.
    // *** REMOVE BEFORE SHIPPING *** (and mirror to DKGameInfo_Endless).
    PuppetPC = DKPlayerController(Injured.Controller);
    if (PuppetPC != None && PuppetPC.PuppetZed == Injured && PuppetPC.PuppetSavedHuman != None
        && Damage >= Injured.Health)
    {
        Damage = Max(Injured.Health - 1, 0);
        PuppetPC.bPuppetRevertPending = true;
    }

    // PUPPET MASTER SPIKE: a parked human body (its player is currently a zed)
    // must take NO damage - if it dies while we are away, the revert fails and
    // the round is lost. The body is controllerless while parked.
    // *** REMOVE BEFORE SHIPPING *** (and mirror to DKGameInfo_Endless).
    if (KFPawn_Human(Injured) != None && Injured.Controller == None)
    {
        foreach WorldInfo.AllControllers(class'DKPlayerController', PuppetHumanPC)
        {
            if (PuppetHumanPC.PuppetSavedHuman == Injured && PuppetHumanPC.PuppetZed != None)
            {
                Damage = 0;
                break;
            }
...
function Killed(Controller Killer, Controller KilledPlayer, Pawn KilledPawn, class<DamageType> DT)
{
    ...
    // PUPPET MASTER SPIKE: a player-driven zed just died (incl. decapitation,
    // which the ReduceDamage body-health cap cannot catch). Re-possess the
    // human BEFORE super - super calls RefreshMonsterAliveCount and the native
    // living-count recompute, and the controller must not be pawnless then or
    // the wave ends. The parked human was kept alive while we were away.
    // *** REMOVE BEFORE SHIPPING *** (and mirror to DKGameInfo_Endless).
    PuppetPC = DKPlayerController(KilledPlayer);
    if (PuppetPC != None && PuppetPC.PuppetZed == KilledPawn && PuppetPC.PuppetSavedHuman != None)
    {
        ForceLivingPlayerCount(1);
        PuppetPC.ServerPuppetDrop();
    }
...
// zeds. *** REMOVE/REVISIT BEFORE SHIPPING *** (mirrored in DKGameInfo_Endless).
...
function WaveEnded(EWaveEndCondition WinCondition)
{
    ...
    // *** REMOVE/REVISIT BEFORE SHIPPING *** (mirrored in DKGameInfo_Endless).
    foreach WorldInfo.AllControllers(class'DKPlayerController', DKPC)
    {
...
defaultproperties
{
    ...
    // Uses AllWeapons GRI that unlocks all weapons
    GameReplicationInfoClass=class'ZedternalRBPerkpackage.DKGameReplicationInfo_AllWeapons'
    ...
    Name="Default__DKGameInfo_Endless_AllWeapons"
}
