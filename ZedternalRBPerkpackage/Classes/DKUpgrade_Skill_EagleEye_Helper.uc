// ===================================================================
// DKUpgrade_Skill_EagleEye_Helper - Manages Eagle Eye ability state
// (renamed from DKUpgrade_Skill_DeadEye_Helper)
// ===================================================================
class DKUpgrade_Skill_EagleEye_Helper extends Info
	transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, DKPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'DKUpgrade_Skill_EagleEye'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'DKUpgrade_Skill_EagleEye'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Eagle Eye", self, class'DKUpgrade_Skill_EagleEye_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Eagle Eye: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Eagle Eye: Already active!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'DKMessageManager'.static.SendMinor(DKPC, "Eagle Eye: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "EAGLE EYE ACTIVATED! +25% headshot damage for 10 seconds!");
	}
	else
	{
		class'DKMessageManager'.static.SendImportant(DKPC, "EAGLE EYE (DELUXE) ACTIVATED! +40% headshot damage for 10 seconds!");
	}
...
function Deactivate()
{
	...
	class'DKMessageManager'.static.SendMinor(DKPC, "Eagle Eye ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'DKMessageManager'.static.SendImportant(DKPC, "Eagle Eye ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Skill_EagleEye_Helper"
}
