// ===================================================================
// DKUpgrade_Skill_Chronoshift_Helper - Manages Chronoshift ability state
// Triggers ZED time on demand with active ability system
// FIXED: Routes sound through DKPlayerController for client replication
// ===================================================================
class DKUpgrade_Skill_Chronoshift_Helper extends Info
	transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, DKPlayerController InDKPC)
{
	...
	// Load custom activation sound via DKMutator
	foreach OwnerPawn.WorldInfo.AllActors(class'DKMutator', Mutator)
	{
		`log("Chronoshift_Helper: Found DKMutator, requesting sound...");
	...
	if (Mutator == None)
	{
		`log("Chronoshift_Helper: ERROR - Could not find DKMutator!");
	}
	
	if (InTier == 1)
		AbilityIcon = class'DKUpgrade_Skill_Chronoshift'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'DKUpgrade_Skill_Chronoshift'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Chronoshift", self, class'DKUpgrade_Skill_Chronoshift_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Chronoshift: Cannot activate while dead!");
		return;
	}
	...
	{
		class'DKMessageManager'.static.SendMinor(DKPC, "Chronoshift: ZED time already active!");
		return;
	}
...
function Activate()
{
	...
	else
	{
		...
		class'DKMessageManager'.static.SendMinor(DKPC, "Chronoshift: Activation failed!");
	}
...
function UpdateAbility()
{
	...
	if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'DKMessageManager'.static.SendImportant(DKPC, "Chronoshift ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__DKUpgrade_Skill_Chronoshift_Helper"
}