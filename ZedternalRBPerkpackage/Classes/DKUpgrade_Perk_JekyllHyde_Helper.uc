// ===================================================================
// DKUpgrade_Perk_JekyllHyde_Helper - per-player Hyde state machine
//
...
// ===================================================================
class DKUpgrade_Perk_JekyllHyde_Helper extends Info;
...
// HUD card states (mirrored in DKHudWrapper.DrawHydeDisplay)
...
simulated function PostBeginPlay()
{
    ...
    if (Role == ROLE_Authority)
    {
        ...
        Charges = 0;
        MaxCharges = class'DKUpgrade_Perk_JekyllHyde'.default.HydeChargesBase;
        bNeedsInitialCharge = True;
    }
}

function DKPlayerController GetPC()
{
...
{
    if (L >= class'DKConfig_Capstone'.static.GetRank2Level())
        return class'DKUpgrade_Perk_JekyllHyde'.default.HydeChargesCapstone2;
    return class'DKUpgrade_Perk_JekyllHyde'.default.HydeChargesBase;
}
...
function float DurationForLevel(int L)
{
    ...
    D = class'DKUpgrade_Perk_JekyllHyde'.default.HydeDuration;
    if (L >= class'DKConfig_Capstone'.static.GetRank1Level())
        D *= class'DKUpgrade_Perk_JekyllHyde'.default.HydeDurationCapstoneMult;
    return D;
}
...
function TryActivate()
{
    ...
    {
        class'DKMessageManager'.static.SendMinor(GetPC(), "Hyde Serum: cannot transform while down.");
        return;
    }

    if (bHyde)
    {
        class'DKMessageManager'.static.SendMinor(GetPC(), "Hyde Serum: already transformed.");
        return;
    }

    if (Charges <= 0)
    {
        class'DKMessageManager'.static.SendMinor(GetPC(), "Hyde Serum: no doses left this wave.");
        return;
    }
...
function Activate()
{
    ...
    UseScale = class'DKUpgrade_Perk_JekyllHyde'.default.HydeBodyScale;
    if (TitanicScaleOverride > 0.0f)
        UseScale = TitanicScaleOverride;

    // Grow-in transform (mesh-only scale, anchored at feet -> no sinking).
    OwnerPawn.BodyScaleChangePerSecond = class'DKUpgrade_Perk_JekyllHyde'.default.HydeScaleRate;
    OwnerPawn.IntendedBodyScale = UseScale;

    PlayHydeSound('Hyde_Activate');
    class'DKMessageManager'.static.SendImportant(GetPC(), "MR. HYDE UNLEASHED!");
...
function EndHyde()
{
    ...
    class'DKMessageManager'.static.SendMinor(GetPC(), "Reverting to Dr. Jekyll...");
    PushHUD(HYDE_HUD_READY);
}
...
function MeleeShockwave(KFPawn_Monster HitZed, int DamageAmount, class<KFDamageType> DT, KFPlayerController Instig)
{
    ...
    Radius = class'DKUpgrade_Perk_JekyllHyde'.default.HydeAoERadius
        * (1.0f + SeismicRadiusBonus + TitanicRadiusBonus);
    MinFrac = class'DKUpgrade_Perk_JekyllHyde'.default.HydeAoEMinDamageFrac;
...
function PlayHydeSound(name SoundID)
{
    ...
    Mut = class'DKSoundManager'.static.GetMutator(OwnerPawn.WorldInfo);
    if (Mut == None)
        return;

    Cue = class'DKSoundManager'.static.GetSound(Mut, SoundID);
    if (Cue != None)
...
// animates the meter smoothly in DKHudWrapper.DrawHydeDisplay.
// ===================================================================
...
defaultproperties
{
    ...
    Name="Default__DKUpgrade_Perk_JekyllHyde_Helper"
}
