// ===================================================================
// ZTUpgrade_Perk_Possessor_Helper - per-player Possession state machine
//
...
// dedicated O key (ZTPlayerController.PossessorPress).
//
// Flow: PossessorPress -> ServerPossessorPress -> (ready) ClientOpenPossessorWheel
// with BuildWheelSnapshot(). The wheel (ZTPossessorWheelMovie, reusing the
...
// ZTHudWrapper.DrawPossessorCard; the possession bar + seconds tick in
// real time client-side off EndTime.
// ===================================================================
class ZTUpgrade_Perk_Possessor_Helper extends Info transient;
...
function float GetCooldown()
{
    ...
    CD = class'ZTUpgrade_Perk_Possessor'.default.Cooldown;
    CD *= 1.0f - class'ZTUpgrade_Perk_Possessor'.default.CooldownReductionPerRank * float(PerkLevel);
...
function float GetDuration()
{
    ...
    Dur = class'ZTUpgrade_Perk_Possessor'.default.PossessDuration;
    if (PerkLevel >= 10)
        Dur += class'ZTUpgrade_Perk_Possessor'.default.DurationBonusCapstone10;
...
function class<KFPawn_Monster> GetFormClass(int FormIndex)
{
    switch (FormIndex)
    {
        ...
        case 9: return class'ZTPawn_ZedPatriarch_Puppet';
    }
...
function int GetPuppetHealth(int FormIndex, class<KFPawn_Monster> ZedClass)
{
    ...
    Mult = class'ZTUpgrade_Perk_Possessor'.default.PuppetHealthMultBase
         + class'ZTUpgrade_Perk_Possessor'.default.PuppetHealthPerRank * float(PerkLevel);

    if (FormIndex >= 0 && FormIndex <= 9 && FormUnlockLevel[FormIndex] >= 2)
        Mult *= class'ZTUpgrade_Perk_Possessor'.default.DeluxeFormHealthMult;
...
// Read in ZTGameInfo_Endless[_AllWeapons].ReduceDamage and applied to every
// hit the possessed zed lands. Always >= 1.0 (never weakens a form).
function float GetPuppetDamageScalar(int FormIndex)
{
    local float Mult;

    Mult = class'ZTUpgrade_Perk_Possessor'.default.PuppetDamageMultBase
         + class'ZTUpgrade_Perk_Possessor'.default.PuppetDamagePerRank * float(PerkLevel);

    if (FormIndex >= 0 && FormIndex <= 9 && FormUnlockLevel[FormIndex] >= 2)
        Mult *= class'ZTUpgrade_Perk_Possessor'.default.DeluxeFormDamageMult;
...
function float GetFormDamageTierMult(int FormIndex)
{
    switch (FormIndex)
    {
        case 7:   // Scrake
        case 8:   // Fleshpound
            return class'ZTUpgrade_Perk_Possessor'.default.FormDamageMult_Large;
        case 9:   // Patriarch
            return class'ZTUpgrade_Perk_Possessor'.default.FormDamageMult_Boss;
        case 3:   // Bloat
        case 5:   // Siren
        case 6:   // Husk
            return class'ZTUpgrade_Perk_Possessor'.default.FormDamageMult_Medium;
        default:  // Clot/Slasher, Crawler, Stalker, Gorefast
            return class'ZTUpgrade_Perk_Possessor'.default.FormDamageMult_Trash;
    }
}
...
// Fired by the wheel click (ZTPlayerController.ServerFirePossessorForm).
function FirePossess(int FormIndex)
{
    ...
    {
        class'ZTMessageManager'.static.SendMinor(DKPC, "Possession: that form is not unlocked.");
        return;
    }
...
// Called by ZTPlayerController.ServerPuppetDrop on EVERY revert path
...
function NotifyPuppetKill()
{
    ...
    Bonus = class'ZTUpgrade_Perk_Possessor'.default.KillDurationBonusCapstone20;
...
function UpdateAbility()
{
    ...
    if (bOnCooldown)
    {
        ...
        if (Elapsed >= GetCooldown())
        {
            bOnCooldown = false;
            PushHUD(HUD_READY);
            class'ZTMessageManager'.static.SendImportant(DKPC, "Possession ready!");
        }
    }
}
...
// and animates the bar + seconds in ZTHudWrapper.DrawPossessorCard.
// ===================================================================
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Perk_Possessor_Helper"
}
