class ZTUpgrade_Skill_AssetLiquidation_Helper extends Info transient;
...
function PayLiquidationBonus(KFWeapon SoldWeapon)
{
    ...
    DamagePercent = class'ZTUpgrade_Skill_AssetLiquidation'.default.DamagePercentage[UpgradeLevel - 1];
    ...
    if (BonusDosh >= 1)
    {
        ...
        if (KFPC != None)
        {
            ...
            if (KFPRI != None)
            {
                ...
                class'ZTMessageManager'.static.SendImportant(KFPC, "ASSET LIQUIDATION: +" $ BonusDosh $ " Dosh from " $ DamageDealt $ " damage with " $ GetWeaponName(SoldWeapon.Class));
            }
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_AssetLiquidation_Helper"
}