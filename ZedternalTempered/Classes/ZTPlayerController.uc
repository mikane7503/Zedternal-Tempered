...
simulated function ApplyMissingKeybindings(ZT_Config_Keybindings KeyConfig)
{
    ...
    for (i = 0; i < KeyConfig.DefaultKeybindings.Length; i++)
    {
        ...
        if (BindIdx == INDEX_NONE || PlayerInput.Bindings[BindIdx].Command == "")
        {
            ...
            `log("ZTPlayerController [CLIENT]: Filled missing keybind -" @ KeyName @ "=" @ WantedCmd);
            continue;
        }
        ...
        if (IsJunkCommand(CurCmd))
        {
            ...
            `log("ZTPlayerController [CLIENT]: Overwrote junk keybind -" @ KeyName @ "=" @ WantedCmd);
        }
            ...
            && (PlayerInput.Bindings[BindIdx].Control || PlayerInput.Bindings[BindIdx].Shift || PlayerInput.Bindings[BindIdx].Alt))
        {
            ...
            SanitizeBinding(BindIdx, WantedCmd);
            `log("ZTPlayerController [CLIENT]: Repaired modifier-gated keybind -" @ KeyName @ "=" @ WantedCmd);
        }
    }
}
...
reliable server function ServerWishCycle()
{
    ...
    H = class'ZTUpgrade_Perk_Wishmaster'.static.FindHelper(Pawn);
    if (H != None)
        H.CycleSelection();
}
...
reliable server function ServerActivateGoalkeeper()
{
    ...
    H = class'ZTUpgrade_Perk_Goalkeeper'.static.FindHelper(Pawn);
    if (H == None)
    {
        class'ZTMessageManager'.static.SendMinor(self, "Goalkeeper: you do not have the Goalkeeper perk.");
        return;
    }
...
reliable server function ServerActivateFastball()
{
    ...
    H = class'ZTUpgrade_Perk_Fastball'.static.FindHelper(Pawn);
    if (H == None)
    {
        class'ZTMessageManager'.static.SendMinor(self, "Fastball: you do not have the Fastball perk.");
        return;
    }
...
reliable server function ServerActivateAbilitySlot(int SlotIndex)
{
    ...
    local ZTUpgrade_Skill_FastHands_Helper QuickDrawHelper;
    ...
    local ZTUpgrade_Skill_EagleEye_Helper DeadEyeHelper;
    ...
    QuickDrawHelper = ZTUpgrade_Skill_FastHands_Helper(Slot.Helper);
    ...
    DeadEyeHelper = ZTUpgrade_Skill_EagleEye_Helper(Slot.Helper);
