class ZTUpgrade_Skill_FrostbiteArrows_Helper extends Info transient;
...
function float GetDamageBonusForTarget(KFPawn_Monster Target)
{
    ...
    for (i = 0; i < FrostbiteStacks.Length; i++)
    {
        if (FrostbiteStacks[i].Target == Target)
        {
            return float(FrostbiteStacks[i].StackCount) * class'ZTUpgrade_Skill_FrostbiteArrows'.default.DamagePerStack[UpgradeLevel - 1];
        }
    }
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_FrostbiteArrows_Helper"
}