// ===================================================================
// ZTUpgrade_Perk_Hivemind_Helper - Team Buff Tracking
// Handles: Nearby teammate count, kill tracking, Swarm Collective
// ===================================================================
class ZTUpgrade_Perk_Hivemind_Helper extends Info transient;
...
function Initialize(KFPawn_Human NewPlayer, int NewUpgradeLevel)
{
	...
	{
		`log("Hivemind Helper: Found ZTMutator, requesting sounds...");
...
function ScanNearbyTeammates()
{
	...
	foreach Player.WorldInfo.AllPawns(class'KFPawn_Human', Teammate)
	{
		if (Teammate != Player && Teammate.IsAliveAndWell())
		{
			...
			if (Distance <= class'ZTUpgrade_Perk_Hivemind'.default.NeuralNetworkRadius)
			{
				Count++;
			}
		}
...
function OnKill(KFPawn_Monster KilledMonster)
{
	...
	if (UpgradeLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level && SwarmKillProgress >= class'ZTUpgrade_Perk_Hivemind'.default.SwarmCollectiveKillsRequired)
	{
	...
	`log("Hivemind: Kill tracked -" @ SwarmKillProgress @ "/" @ class'ZTUpgrade_Perk_Hivemind'.default.SwarmCollectiveKillsRequired);
}
...
function TriggerSwarmCollective()
{
	...
	SwarmCollectiveEndTime = Player.WorldInfo.TimeSeconds + class'ZTUpgrade_Perk_Hivemind'.default.SwarmCollectiveDuration;
...
function UpdateHUD()
{
	...
	SymbioteHelper = class'ZTUpgrade_Perk_Symbiote'.static.GetHelper(Player);
...
{
	local ZTPlayerController DKPC;
	local ZTHudWrapper HivemindHUD;
	...
	{
		if (DebugUpdateCount % 50 == 1)
			`log("Hivemind Helper CLIENT: âœ— GetALocalPlayerController returned None or not ZTPlayerController!");
		return;
	}
	
	if (DebugUpdateCount % 50 == 1)
		`log("Hivemind Helper CLIENT: âœ“ Got ZTPlayerController:" @ DKPC);
	
	// Get HUD using GetReaperHUD static function
	HivemindHUD = class'ZTHudWrapper'.static.GetReaperHUD(DKPC);
	if (HivemindHUD == None)
	{
		if (DebugUpdateCount % 50 == 1)
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Hivemind_Helper"
}