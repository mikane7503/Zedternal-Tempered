...
class ZTMessageReplicator extends ReplicationInfo;
...
reliable client function ShowPerkUnlockPopup(string PerkName, Texture2D PerkIcon)
{
    ...
    if (OwningPC != None)
    {
        ...
        if (CustomHUD != None)
        {
            ...
            `log("ZTMessageReplicator (CLIENT): Showed perk unlock popup for" @ PerkName);
        }
        else
        {
            `log("ZTMessageReplicator (CLIENT): ERROR - Could not get HUD for unlock popup");
        }
    }
}
...
reliable client function ShowPerkUnlockPopupByIndex(string PerkName, int PerkIndex)
{
    ...
    {
        CustomHUD.ShowPerkUnlockNotification(PerkName, PerkIcon);
        `log("ZTMessageReplicator (CLIENT): Showed perk unlock popup for" @ PerkName @ "using index" @ PerkIndex);
    }
    else
    {
        `log("ZTMessageReplicator (CLIENT): ERROR - Could not get HUD for unlock popup");
...
static function ZTMessageReplicator GetReplicatorForPlayer(KFPlayerController KFPC)
{
    ...
    foreach KFPC.DynamicActors(class'ZTMessageReplicator', Replicator)
    {
    ...
    Replicator = KFPC.Spawn(class'ZTMessageReplicator', KFPC);
    if (Replicator != None)
    {
...
defaultproperties
{
    ...
    Name="Default__ZTMessageReplicator"
}
