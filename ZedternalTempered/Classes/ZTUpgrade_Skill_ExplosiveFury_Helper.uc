// ===================================================================
// ZTUpgrade_Skill_ExplosiveFury_Helper - Manages Explosive Fury ability state
// ===================================================================
class ZTUpgrade_Skill_ExplosiveFury_Helper extends Info transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, ZTPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'ZTUpgrade_Skill_ExplosiveFury'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'ZTUpgrade_Skill_ExplosiveFury'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Explosive Fury", self, class'ZTUpgrade_Skill_ExplosiveFury_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Explosive Fury: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Explosive Fury: Already active!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'ZTMessageManager'.static.SendMinor(DKPC, "Explosive Fury: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'ZTMessageManager'.static.SendImportant(DKPC, "EXPLOSIVE FURY ACTIVATED! +30% explosive damage for 10 seconds!");
	}
	else
	{
		class'ZTMessageManager'.static.SendImportant(DKPC, "EXPLOSIVE FURY (DELUXE) ACTIVATED! +50% explosive damage for 10 seconds!");
	}
...
function Deactivate()
{
	...
	class'ZTMessageManager'.static.SendMinor(DKPC, "Explosive Fury ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'ZTMessageManager'.static.SendImportant(DKPC, "Explosive Fury ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Skill_ExplosiveFury_Helper"
}