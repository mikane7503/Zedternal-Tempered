// ===================================================================
// ZTUI_UPGMenu - DK fork of ZedternalReborn's WMUI_UPGMenu
// ===================================================================
...
//
// Server side of the Deluxe upgrade lives in ZTPlayerController
// (ServerBuyDeluxeUpgrade / ServerBuyDeluxeUpgradeRandom) and the gating
// settings replicate via ZTGameReplicationInfo (bDeluxeUpgradeEnabled,
// DeluxeMinPerkLevel, DeluxeUpgradeCost, bDeluxeTargetedSelection).
// ===================================================================
class ZTUI_UPGMenu extends WMUI_UPGMenu;
...
// Return the short class name used by Tempered's unlock-family table.
function string GetTemperedPerkClassName(class<WMUpgrade_Perk> PerkClass)
{
	local string FullClassName;
	local int SeparatorPosition;

	FullClassName = PathName(PerkClass);
	SeparatorPosition = InStr(FullClassName, ".");
	if (SeparatorPosition != INDEX_NONE)
		return Mid(FullClassName, SeparatorPosition + 1);

	return FullClassName;
}

// Tempered has no Ascension advancement base class, so preserve Ascension's
// family ordering with the equivalent Tempered specialization families.
function string GetTemperedBasePerkName(string PerkClassName)
{
	if (PerkClassName ~= "ZTUpgrade_Perk_Bulwark" || PerkClassName ~= "ZTUpgrade_Perk_Riot"
		|| PerkClassName ~= "ZTUpgrade_Perk_Voodoo" || PerkClassName ~= "ZTUpgrade_Perk_Diablo"
		|| PerkClassName ~= "ZTUpgrade_Perk_Haunted")
		return "ZUTUpgrade_Perk_Base_Berserker";
	if (PerkClassName ~= "ZTUpgrade_Perk_SpecialAgent" || PerkClassName ~= "ZTUpgrade_Perk_Warlord"
		|| PerkClassName ~= "ZTUpgrade_Perk_Metronome" || PerkClassName ~= "ZTUpgrade_Perk_Agony")
		return "ZUTUpgrade_Perk_Base_Commando";
	if (PerkClassName ~= "ZTUpgrade_Perk_Maniac" || PerkClassName ~= "ZTUpgrade_Perk_ForgeWarden"
		|| PerkClassName ~= "ZTUpgrade_Perk_Hollow" || PerkClassName ~= "ZTUpgrade_Perk_Detonator")
		return "ZUTUpgrade_Perk_Base_Demolitionist";
	if (PerkClassName ~= "ZTUpgrade_Perk_Archangel" || PerkClassName ~= "ZTUpgrade_Perk_Parasite"
		|| PerkClassName ~= "ZTUpgrade_Perk_Hivemind" || PerkClassName ~= "ZTUpgrade_Perk_Symbiote")
		return "ZUTUpgrade_Perk_Base_FieldMedic";
	if (PerkClassName ~= "ZTUpgrade_Perk_Scavenger" || PerkClassName ~= "ZTUpgrade_Perk_Taskmaster"
		|| PerkClassName ~= "ZTUpgrade_Perk_Frost" || PerkClassName ~= "ZTUpgrade_Perk_Omen")
		return "ZUTUpgrade_Perk_Base_SWAT";
	if (PerkClassName ~= "ZTUpgrade_Perk_Headhunter" || PerkClassName ~= "ZTUpgrade_Perk_TimeTraveler"
		|| PerkClassName ~= "ZTUpgrade_Perk_Gambit" || PerkClassName ~= "ZTUpgrade_Perk_Reaper")
		return "ZUTUpgrade_Perk_Base_Gunslinger";
	if (PerkClassName ~= "ZTUpgrade_Perk_Daredevil" || PerkClassName ~= "ZTUpgrade_Perk_Wendigo"
		|| PerkClassName ~= "ZTUpgrade_Perk_Cryophilite" || PerkClassName ~= "ZTUpgrade_Perk_Predator")
		return "ZUTUpgrade_Perk_Base_Sharpshooter";
	if (PerkClassName ~= "ZTUpgrade_Perk_Tycoon" || PerkClassName ~= "ZTUpgrade_Perk_Artificer"
		|| PerkClassName ~= "ZTUpgrade_Perk_Hydra")
		return "ZUTUpgrade_Perk_Base_Support";
	if (PerkClassName ~= "ZTUpgrade_Perk_Pyrokinetic" || PerkClassName ~= "ZTUpgrade_Perk_Cinder")
		return "ZUTUpgrade_Perk_Base_Firebug";
	if (PerkClassName ~= "ZTUpgrade_Perk_Venomancer" || PerkClassName ~= "ZTUpgrade_Perk_Medusa"
		|| PerkClassName ~= "ZTUpgrade_Perk_Gambler" || PerkClassName ~= "ZTUpgrade_Perk_Shapeshifter")
		return "ZUTUpgrade_Perk_Base_Survivalist";

	return "";
}

// Ascension display rule adapted to Tempered:
// purchased base perks first (level descending), immediately followed by
// their level-5/10/15/20 specialization family in registry order.
function BuildTemperedPerkDisplayOrder(ZTPlayerReplicationInfo DKPRI, out array<int> DisplayOrder)
{
	local array<int> BasePerks;
	local array<bool> Added;
	local int i, j, SwapIndex, BaseIndex;
	local string BaseClassName, PerkClassName;

	DisplayOrder.Length = 0;
	Added.Length = WMGRI.PerkUpgradesList.Length;

	for (i = 0; i < WMGRI.PerkUpgradesList.Length; ++i)
	{
		PerkClassName = GetTemperedPerkClassName(WMGRI.PerkUpgradesList[i].PerkUpgrade);
		if (Left(PerkClassName, 21) ~= "ZUTUpgrade_Perk_Base_")
			BasePerks.AddItem(i);
	}

	for (i = 0; i < BasePerks.Length; ++i)
	{
		for (j = i + 1; j < BasePerks.Length; ++j)
		{
			if (DKPRI.GetPerkLevel(BasePerks[j]) > DKPRI.GetPerkLevel(BasePerks[i]))
			{
				SwapIndex = BasePerks[i];
				BasePerks[i] = BasePerks[j];
				BasePerks[j] = SwapIndex;
			}
		}
	}
	...
	for (i = 0; i < BasePerks.Length; ++i)
	{
		BaseIndex = BasePerks[i];
		BaseClassName = GetTemperedPerkClassName(WMGRI.PerkUpgradesList[BaseIndex].PerkUpgrade);
		DisplayOrder.AddItem(BaseIndex);
		Added[BaseIndex] = True;

		for (j = 0; j < WMGRI.PerkUpgradesList.Length; ++j)
		{
			if (!Added[j]
				&& GetTemperedBasePerkName(GetTemperedPerkClassName(WMGRI.PerkUpgradesList[j].PerkUpgrade)) ~= BaseClassName)
			{
				DisplayOrder.AddItem(j);
				Added[j] = True;
			}
		}
	}
...
function BuildPerkUpgradeList(out GFxObject ItemArray)
{
	...
	// Original Zedternal registry order. Ascension family/level sorting removed.
	for (i = 0; i < WMGRI.PerkUpgradesList.Length; ++i)
	{
...
//
// Eligibility mirrors the server (ZTPlayerController.IsDeluxeSkillEligible):
...
function bool PerkHasEligibleDeluxeSkill(ZTGameReplicationInfo DKGRI, int PerkIdx)
{
	local int i;
...
function BuildWeaponUpgradeList(out GFxObject ItemArray)
{
	...
	for (i = 0; i < WMGRI.WeaponUpgradeSlotsList.Length; ++i)
	{
		if (IsWeaponInInventory(WMGRI.WeaponUpgradeSlotsList[i].KFWeapon))
		{
			...
			if (ClassIsChildOf(WMGRI.WeaponUpgradeSlotsList[i].WeaponUpgrade, class'ZTWeaponUpg_HollowBase')
				&& !PlayerOwnsRequiredHollowSkill(class<ZTWeaponUpg_HollowBase>(WMGRI.WeaponUpgradeSlotsList[i].WeaponUpgrade)))
				continue;
...
// ===================================================================
function bool PlayerOwnsRequiredHollowSkill(class<ZTWeaponUpg_HollowBase> HollowUpg)
{
	local int s;
