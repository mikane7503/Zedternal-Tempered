...
// ===================================================================
class ZTUpgrade_Perk_Shapeshifter_Helper extends Info;
...
function SetPerkLevel(int NewLevel)
{
    ...
    if (NewLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level && !bMimicryActive)
    {
        ActivateMimicry();
    }
...
function ActivateMimicry()
{
    ...
    if (KFPC != None)
    {
        class'ZTMessageManager'.static.SendGameMessage(
            KFPC,
...
function AddMimicryBuff()
{
    ...
    if (NewBuff >= 0 && NewBuff < TOTAL_BUFFS)
    {
        ...
        if (KFPC != None)
        {
            ZTPlayerController(KFPC).ClientShapeshifterMimicryGain(
                NewBuff,
...
// ===================================================================
// HUD COMMUNICATION
// Uses the standalone ShapeshifterDisplay system on ZTHudWrapper
...
// Now calls UpdateShapeshifterDisplay() on ZTHudWrapper (standalone system)
    ...
    int BuffMask)
{
    ...
    local ZTHudWrapper ShapeshifterHUD;

    LocalKFPC = KFPlayerController(GetALocalPlayerController());
    if (LocalKFPC == None) return;

    ShapeshifterHUD = class'ZTHudWrapper'.static.GetReaperHUD(LocalKFPC);
...
function SendBuffMessage(int Buff1, int Buff2)
{
    ...
    ZTPlayerController(KFPC).ClientShapeshifterTransform(
        Buff1,
...
// Routes through ZTPlayerController client RPC for proper audio playback
function PlayBuffSound(int BuffIndex)
{
    ...
    Sound = class'ZTSoundManager'.static.GetSound(Mutator, name("Shapeshifter_Buff_" $ BuffNames[BuffIndex]));
    if (Sound != None)
    {
...
defaultproperties
{
    ...
    // Sound registration: see ZTSoundManager.RegisterDefaultSounds()

    Name="Default__ZTUpgrade_Perk_Shapeshifter_Helper"
}
