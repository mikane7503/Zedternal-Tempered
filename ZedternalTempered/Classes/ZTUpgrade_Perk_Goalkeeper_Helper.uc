// ===================================================================
// ZTUpgrade_Perk_Goalkeeper_Helper - Catch window, charge, throwback
//
// Owned by the perk holder's pawn. Server-side logic + client HUD RPC:
//   - TryActivate() from ZTPlayerController.ServerActivateGoalkeeper:
...
//   - HUD: card-stack card via ClientGoalkeeperHUD -> ZTHudWrapper
...
// ===================================================================
class ZTUpgrade_Perk_Goalkeeper_Helper extends Actor;
...
function TryActivate()
{
	...
	if (bWindowOpen)
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Goalkeeper: catch window already open!");
		return;
	}

	if (bOnCooldown)
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Goalkeeper: recovering from the last miss.");
		return;
	}
...
function OpenCatchWindow()
{
	...
	SetTimer(class'ZTUpgrade_Perk_Goalkeeper'.default.CatchWindowDuration, False, NameOf(CloseCatchWindow));
	PushHUD(GK_HUD_WINDOW, class'ZTUpgrade_Perk_Goalkeeper'.default.CatchWindowDuration);
	class'ZTMessageManager'.static.SendMinor(GetPC(), "Goalkeeper: CATCH!");
}
...
function CloseCatchWindow()
{
	...
	bOnCooldown = True;
	SetTimer(class'ZTUpgrade_Perk_Goalkeeper'.default.CatchCooldown * SkillCooldownMult, False, NameOf(EndCooldown));
	PushHUD(GK_HUD_COOLDOWN, class'ZTUpgrade_Perk_Goalkeeper'.default.CatchCooldown * SkillCooldownMult);
	class'ZTMessageManager'.static.SendMinor(GetPC(), "Goalkeeper: nothing caught.");
}
...
function EndCooldown()
{
	bOnCooldown = False;
	PushHUD(GK_HUD_READY, 0.0f);
	class'ZTMessageManager'.static.SendMinor(GetPC(), "Goalkeeper: ready.");
}
...
function ScanForProjectiles()
{
	...
	foreach DynamicActors(class'KFGame.KFProjectile', Proj)
	{
		...
		if (!bScanLogged)
		{
			...
			`log("[ZT_GOALKEEPER] Candidate:" @ string(Proj.Class.Name)
				@ "Instigator:" @ InstName
		...
		if (Dist > class'ZTUpgrade_Perk_Goalkeeper'.default.CatchRange * SkillRangeMult)
			continue;

		// Front cone check (skip for point-blank - anything that close counts)
		if (Dist > 100.0f
			&& FaceDot < class'ZTUpgrade_Perk_Goalkeeper'.default.CatchConeDot * SkillConeMult)
			continue;
...
function bool IsCatchableClass(string ProjClassName)
{
	...
	for (i = 0; i < class'ZTUpgrade_Perk_Goalkeeper'.default.CatchableClassKeywords.Length; ++i)
	{
		if (InStr(Caps(ProjClassName), Caps(class'ZTUpgrade_Perk_Goalkeeper'.default.CatchableClassKeywords[i])) != INDEX_NONE)
			return True;
	}
...
function CatchProjectile(KFProjectile Proj, float Dist)
{
	...
	bHoldingCharge = True;
	bPerfectCharge = (Dist <= class'ZTUpgrade_Perk_Goalkeeper'.default.PerfectCatchRange * SkillPerfectMult);

	`log("[ZT_GOALKEEPER] CAUGHT" @ string(Proj.Class.Name) @ "at dist" @ string(Dist) @ "perfect:" @ string(bPerfectCharge));

	// Level 10: armor on catch
	if (PerkLevel >= 10 && OwnerPawn != None)
	{
		OwnerPawn.Armor = Min(OwnerPawn.MaxArmor,
			OwnerPawn.Armor + class'ZTUpgrade_Perk_Goalkeeper'.default.ArmorPerCatch);
	}
	...
	if (bPerfectCharge)
		class'ZTMessageManager'.static.SendCritical(GetPC(), "PERFECT CATCH! Press again to return it!");
	else
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Goalkeeper: caught! Press again to return it.");
}
...
function ThrowCharge()
{
	...
	local ZTProj_Goalkeeper_Return Ret;
	...
	if (bPerfectCharge && PerkLevel >= 20)
		Count = Max(1, class'ZTUpgrade_Perk_Goalkeeper'.default.PerfectBarrageCount);

	SpreadStep = 0.0f;
	SpreadStart = 0.0f;
	if (Count > 1)
	{
		SpreadStep = class'ZTUpgrade_Perk_Goalkeeper'.default.PerfectBarrageSpreadDeg * 182.044f; // deg -> unreal rot units
	...
	for (i = 0; i < Count; ++i)
	{
		...
		Ret = Spawn(class'ZTProj_Goalkeeper_Return', OwnerPawn,, StartLoc, SpreadRot);
		if (Ret != None)
		{
	...
	PushHUD(GK_HUD_READY, 0.0f);
	class'ZTMessageManager'.static.SendMinor(KFPC, "Goalkeeper: RETURNED!");
}
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Goalkeeper_Helper"
}
