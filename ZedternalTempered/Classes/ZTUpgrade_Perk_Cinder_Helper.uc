// ===================================================================
// ZTUpgrade_Perk_Cinder_Helper - Burning Enemy Tracking
// Handles: Burning enemy count, fire kill tracking, Phoenix Protocol
// ===================================================================
class ZTUpgrade_Perk_Cinder_Helper extends Info transient;
...
function Initialize(KFPawn_Human NewPlayer, int NewUpgradeLevel)
{
	...
	// Load Phoenix Protocol sound via ZTMutator
	foreach Player.WorldInfo.AllActors(class'ZTMutator', Mutator)
	{
		`log("Cinder Helper: Found ZTMutator, requesting Phoenix sound...");
		...
		if (Monster.IsAliveAndWell())
		{
			...
			for (i = 0; i < Monster.DamageOverTimeArray.Length; i++)
			{
				if (Monster.DamageOverTimeArray[i].DoT_Type == DOT_Fire)
				{
					...
					if (Count >= class'ZTUpgrade_Perk_Cinder'.default.MaxBurningEnemyBonus)
					{
						Count = class'ZTUpgrade_Perk_Cinder'.default.MaxBurningEnemyBonus;
...
function OnFireKill()
{
	...
	{
		PermanentFireDamageBonus = float(CurrentMilestone) * class'ZTUpgrade_Perk_Cinder'.default.PermanentBonusPerKills;
...
function TriggerPhoenixProtocol()
{
	...
	PhoenixProtocolEndTime = Player.WorldInfo.TimeSeconds + class'ZTUpgrade_Perk_Cinder'.default.PhoenixDuration;
	...
	{
		if (DKPC == None)
			`log("Cinder: ✗ Cannot play sound - Not a ZTPlayerController!");
		if (PhoenixSound == None)
			`log("Cinder: ✗ Cannot play sound - PhoenixSound is None!");
	}
	
	PlayerLoc = Player.Location;
	RadiusSq = class'ZTUpgrade_Perk_Cinder'.default.PhoenixRadius * class'ZTUpgrade_Perk_Cinder'.default.PhoenixRadius;
...
reliable client function ClientUpdateHUD(int BurningCount, int FireKills, float PermanentBonus, bool bPhoenix, float PhoenixTimeRemaining)
{
	...
	{
		if (DebugUpdateCount % 50 == 1)
			`log("Cinder Helper CLIENT: ✗ GetALocalPlayerController returned None or not ZTPlayerController!");
		return;
	}
	
	if (DebugUpdateCount % 50 == 1)
		`log("Cinder Helper CLIENT: ✓ Got ZTPlayerController:" @ DKPC);
	
	// Get HUD using GetReaperHUD static function
	CinderHUD = class'ZTHudWrapper'.static.GetReaperHUD(DKPC);
	if (CinderHUD == None)
	{
		if (DebugUpdateCount % 50 == 1)
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Cinder_Helper"
}
