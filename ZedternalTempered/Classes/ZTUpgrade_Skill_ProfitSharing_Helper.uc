class ZTUpgrade_Skill_ProfitSharing_Helper extends Info transient;
...
function OnEnemyKilled()
{
    ...
    if (KillCount >= KillsRequired)
    {
        ...
        DoshToGive = class'ZTUpgrade_Skill_ProfitSharing'.default.DoshPerTeammate[UpgradeLevel - 1];
            ...
            foreach Player.WorldInfo.AllControllers(class'KFPlayerController', TeammatePCs)
            {
                if (TeammatePCs != KFPC) // Don't give to self
                {
                    ...
                    if (TeammatePRI != None)
                    {
                        ...
                        class'ZTMessageManager'.static.SendImportant(TeammatePCs, "PROFIT SHARING: +" $ DoshToGive $ " Dosh from " $ KFPC.PlayerReplicationInfo.PlayerName);
                    }
                }
            }
            
            // Critical - profit sharer activating major team benefit
            class'ZTMessageManager'.static.SendCritical(KFPC, "PROFIT SHARING ACTIVATED: Gave " $ DoshToGive $ " Dosh to all teammates!");
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_ProfitSharing_Helper"
}