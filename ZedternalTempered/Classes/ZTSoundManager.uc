// ===================================================================
// ZTSoundManager - Native Audio System (NO WWISE REQUIRED!)
...
// ===================================================================
class ZTSoundManager extends Object abstract;
...
static function Initialize(ZTMutator Mutator)
{
    `log("===== ZTSoundManager: Initializing Native Audio System =====");
    `log("ZTSoundManager: Using UE3 SoundCue system (NO WWISE REQUIRED!)");
    
    RegisterDefaultSounds(Mutator);
    LoadCustomSounds(Mutator);
    
    `log("ZTSoundManager: Initialization complete -" @ Mutator.CustomSounds.Length @ "sounds registered");
}
...
static function RegisterDefaultSounds(ZTMutator Mutator)
{
    ...
    `log("ZTSoundManager: Registered" @ Mutator.CustomSounds.Length @ "default sounds");
}

static function RegisterSound(ZTMutator Mutator, name SoundID, string CustomPath)
{
    local ZTMutator.CustomSoundEntry NewEntry;
...
static function LoadCustomSounds(ZTMutator Mutator)
{
    local int i;
    local SoundCue LoadedCustomSound;
    
    `log("ZTSoundManager: Attempting to load custom sounds...");
    ...
    for (i = 0; i < Mutator.CustomSounds.Length; i++)
    {
        ...
        if (LoadedCustomSound != None)
        {
            ...
            `log("ZTSoundManager: Loaded custom sound:" @ Mutator.CustomSounds[i].SoundID);
        }
        else
        {
            Mutator.CustomSounds[i].bCustomSoundLoaded = false;
            Mutator.CustomSounds[i].LoadedSound = None;
            `log("ZTSoundManager: Custom sound not found for" @ Mutator.CustomSounds[i].SoundID);
        }
    }
    
    `log("ZTSoundManager: Custom sound loading complete");
}
...
static function PlaySound(string SoundID, Pawn TargetPawn)
{
    ...
    if (TargetPawn == None)
    {
        `log("ZTSoundManager::PlaySound - No target pawn!");
        return;
    }
    ...
    if (Mutator == None)
    {
        `log("ZTSoundManager::PlaySound - Could not find ZTMutator!");
        return;
    }
    ...
    if (Sound == None)
    {
        `log("ZTSoundManager::PlaySound - Sound not found:" @ SoundID);
        return;
    }
...
static function PlaySoundByName(name SoundID, Pawn TargetPawn)
{
    ...
    if (TargetPawn == None)
    {
        `log("ZTSoundManager::PlaySoundByName - No target pawn!");
        return;
    }
    ...
    if (Mutator == None)
    {
        `log("ZTSoundManager::PlaySoundByName - Could not find ZTMutator!");
        return;
    }
    ...
    if (Sound == None)
    {
        `log("ZTSoundManager::PlaySoundByName - Sound not found:" @ SoundID);
        return;
    }
...
/**
 * GetMutator - Helper to find the ZTMutator in the world
 */
static function ZTMutator GetMutator(WorldInfo WI)
{
    local Mutator M;
...
static function SoundCue GetSound(ZTMutator Mutator, name SoundID)
{
    local int i;
    ...
    `log("ZTSoundManager: WARNING - Unregistered sound ID:" @ SoundID);
    return None;
}

static function bool IsCustomSoundLoaded(ZTMutator Mutator, name SoundID)
{
    local int i;
...
static function string GetSoundInfo(ZTMutator Mutator, name SoundID)
{
    local int i;
    local string Info;
...
static function LogAllSounds(ZTMutator Mutator)
{
    local int i;
    
    `log("===== ZTSoundManager: Registered Sounds (Native SoundCue) =====");
...
static function RegisterSoundAtRuntime(ZTMutator Mutator, name SoundID, string CustomPath)
{
    local int i;
    local SoundCue LoadedCustomSound;
    local ZTMutator.CustomSoundEntry NewEntry;
    ...
    for (i = 0; i < Mutator.CustomSounds.Length; i++)
    {
        if (Mutator.CustomSounds[i].SoundID == SoundID)
        {
            `log("ZTSoundManager: Sound already registered:" @ SoundID);
            return;
        }
    }
    ...
    if (LoadedCustomSound != None)
    {
        ...
        `log("ZTSoundManager: Registered custom sound at runtime:" @ SoundID);
    }
    else
    {
        NewEntry.bCustomSoundLoaded = false;
        NewEntry.LoadedSound = None;
        `log("ZTSoundManager: Registered runtime sound" @ SoundID @ "(not loaded)");
    }
