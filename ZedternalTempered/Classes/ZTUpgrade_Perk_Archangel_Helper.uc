class ZTUpgrade_Perk_Archangel_Helper extends Info transient;
...
function SetPerkLevel(int NewLevel)
{
    ...
    bHealingAuraActive = (CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank1Level);
}
...
{
    local KFPlayerController KFPC;
    local ZTHudWrapper ArchangelHUD;
    ...
    if (TargetPawn != None)
    {
        ...
        if (KFPC != None)
        {
            ...
            ArchangelHUD = class'ZTHudWrapper'.static.GetReaperHUD(KFPC);
...
function ApplyHealingAura()
{
    ...
    HealingAmount = class'ZTUpgrade_Perk_Archangel'.default.Level10AuraHealing;
...
function UpdateNearbyAllies()
{
    ...
    foreach OwnerPawn.WorldInfo.DynamicActors(class'KFPawn_Human', AllyPawn)
    {
        if (AllyPawn != OwnerPawn && AllyPawn.Health > 0)
        {
            ...
            if (Distance <= class'ZTUpgrade_Perk_Archangel'.default.AllyProximityRange)
            {
...
{
    bHealingAuraActive = (UpgradeLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank1Level);
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Perk_Archangel_Helper"
}
