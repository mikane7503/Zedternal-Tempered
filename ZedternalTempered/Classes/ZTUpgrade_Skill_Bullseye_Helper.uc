// ===================================================================
// ZTUpgrade_Skill_Bullseye_Helper - Manages Bullseye ability state
// Provides temporary recoil and spread reduction
// ===================================================================
class ZTUpgrade_Skill_Bullseye_Helper extends Info transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, ZTPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'ZTUpgrade_Skill_Bullseye'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'ZTUpgrade_Skill_Bullseye'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Bullseye", self, class'ZTUpgrade_Skill_Bullseye_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Bullseye: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Bullseye: Already active!");
		return;
	}
...
function Deactivate()
{
	...
	class'ZTMessageManager'.static.SendMinor(DKPC, "Bullseye effect ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'ZTMessageManager'.static.SendImportant(DKPC, "Bullseye ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Skill_Bullseye_Helper"
}
