// ===================================================================
// ZTUpgrade_Perk_Fastball_Helper - Target pick, launch, landing watch
//
// Owned by the LAUNCHER's pawn. Server-side logic + client HUD RPC:
//   - TryLaunch() from ZTPlayerController.ServerActivateFastball:
//     picks the teammate in the launcher's aim cone within range,
//     enforces consent (mutual facing by default), applies velocity,
//     spawns a ZTFastball_PayloadMarker on the payload and starts a
//     landing poll.
...
//     ZTHudWrapper (states 0 ready / 2 cooldown).
// Landing detection is poll-based on purpose: no ZTPawn_Human edits,
...
// ===================================================================
class ZTUpgrade_Perk_Fastball_Helper extends Actor;
...
var float ReliefEndTime;        // read by ZTUpgrade_Skill_ReliefPitcher conditional hooks

// Active flight tracking
var KFPawn_Human PayloadPawn;
var ZTFastball_PayloadMarker PayloadMarker;
...
function TryLaunch()
{
	...
	if (bOnCooldown)
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Fastball: winding up, not ready yet.");
		return;
	}

	if (bWatchingFlight)
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Fastball: your last throw is still airborne!");
		return;
	}
	...
	if (Target == None)
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Fastball: no willing teammate in reach (they must be close and looking at you).");
		return;
	}

	AimDir = vector(OwnerPawn.GetViewRotation());

	Force = class'ZTUpgrade_Perk_Fastball'.default.LaunchForce;
	if (PerkLevel >= 20)
		Force *= class'ZTUpgrade_Perk_Fastball'.default.LaunchForceMultL20;
	Force *= SkillForceMult;

	LaunchVel = AimDir * Force;
	LaunchVel.Z += class'ZTUpgrade_Perk_Fastball'.default.LaunchZBoost;

	// Payload marker: fall damage shield (zeroed in both ZTGameInfos'
	// ReduceDamage) + flight bookkeeping
	PayloadMarker = Target.Spawn(class'ZTFastball_PayloadMarker', Target);
	...
	bOnCooldown = True;
	SetTimer(class'ZTUpgrade_Perk_Fastball'.default.CooldownSeconds * SkillCooldownMult, False, NameOf(EndCooldown));
	PushHUD(FB_HUD_COOLDOWN, class'ZTUpgrade_Perk_Fastball'.default.CooldownSeconds * SkillCooldownMult);

	class'ZTMessageManager'.static.SendMinor(GetPC(), "FASTBALL:" @ Target.PlayerReplicationInfo.PlayerName @ "is airborne!");
	class'ZTMessageManager'.static.SendCritical(KFPlayerController(Target.Controller), "FASTBALL! You are the projectile now!");
}
...
function KFPawn_Human PickTarget()
{
	...
	BestDot = class'ZTUpgrade_Perk_Fastball'.default.TargetConeDot;
	...
	foreach WorldInfo.AllPawns(class'KFGame.KFPawn_Human', Candidate)
	{
		...
		Dist = VSize(ToTarget);
		if (Dist > class'ZTUpgrade_Perk_Fastball'.default.TargetRange)
			continue;
		...
		if (class'ZTUpgrade_Perk_Fastball'.default.ConsentMode == 1)
		{
			TargetAim = vector(Candidate.GetViewRotation());
			if ((Normal(OwnerPawn.Location - Candidate.Location) dot TargetAim)
				< class'ZTUpgrade_Perk_Fastball'.default.ConsentFacingDot)
				continue;
		}
...
function EndCooldown()
{
	bOnCooldown = False;
	PushHUD(FB_HUD_READY, 0.0f);
	class'ZTMessageManager'.static.SendMinor(GetPC(), "Fastball: ready.");
}
...
function DetonateLanding()
{
	...
	// ZTUpgrade_Perk_Fastball.ModifyDamageGiven via ZTDT_Fastball_Impact.
	Damage = class'ZTUpgrade_Perk_Fastball'.default.ImpactBaseDamage;
	Damage += Damage * class'ZTUpgrade_Perk_Fastball'.default.ImpactSpeedDamageScale * (PeakSpeed / 1000.0f);

	Radius = class'ZTUpgrade_Perk_Fastball'.default.ImpactRadius;
	if (PerkLevel >= 10)
		Radius *= class'ZTUpgrade_Perk_Fastball'.default.ImpactRadiusMultL10;
	Radius *= SkillRadiusMult;

	PayloadPawn.HurtRadius(Damage, Radius, class'ZTDT_Fastball_Impact', 2000.0f,
	...
	HealAmount = (PerkLevel >= 20)
		? class'ZTUpgrade_Perk_Fastball'.default.PayloadHealOnLandingL20
		: class'ZTUpgrade_Perk_Fastball'.default.PayloadHealOnLanding;
	...
	if (class'ZTUpgrade_Perk_Fastball'.default.PayloadArmorOnLanding > 0)
	{
		PayloadPawn.Armor = Min(PayloadPawn.MaxArmor,
			PayloadPawn.Armor + class'ZTUpgrade_Perk_Fastball'.default.PayloadArmorOnLanding);
	}
	...
	class'ZTMessageManager'.static.SendMinor(KFPlayerController(PayloadPawn.Controller), "Fastball: STUCK THE LANDING!");
}
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Fastball_Helper"
}
