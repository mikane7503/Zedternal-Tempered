class ZTUpgrade_Perk_Hydra_Helper extends Info transient;
...
var float FuryModeTimer;               // Remaining fury mode time (duration comes from ZTUpgrade_Perk_Hydra config)
        ...
        if (!bInFuryMode)
        {
            ...
            if (OwnerPawn != None)
            {
                ...
                if (KFPC != None)
                {
                    ...
                    KFPC.ClientMessage("HYDRA BARRAGE ACTIVATED! " $ int(class'ZTUpgrade_Perk_Hydra'.default.FuryModeDuration) $ " seconds of +100% fire rate + triple damage! (" $ TotalKills $ " total kills)");
                }
            }
        }
    }
...
function StartFuryMode()
{
    ...
    bInFuryMode = true;
    FuryModeTimer = class'ZTUpgrade_Perk_Hydra'.default.FuryModeDuration;
    
    // Start the fury mode timer
    SetTimer(class'ZTUpgrade_Perk_Hydra'.default.FuryModeDuration, False, FuryModeTimerName);
...
{
    local KFPlayerController KFPC;
    local ZTHudWrapper HydraHUD;
    
    KFPC = KFPlayerController(GetALocalPlayerController());
    if (KFPC != None)
    {
        // Trigger the HUD chain notification for fury mode
        HydraHUD = class'ZTHudWrapper'.static.GetReaperHUD(KFPC);
        if (HydraHUD != None)
        {
            HydraHUD.TriggerChainNotification("HYDRA BARRAGE!", int(class'ZTUpgrade_Perk_Hydra'.default.FuryModeDuration) $ " seconds of double fire rate + triple damage!", 4.0f);
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Perk_Hydra_Helper"
}