class ZTUpgrade_Skill_CurseOfStone_Helper extends Info transient;
...
function CurseMonster(KFPawn_Monster Monster, int SkillLevel)
{
    ...
    if (KFPC != None)
    {
        ...
        class'ZTMessageManager'.static.SendMinor(KFPC, "Curse of Stone: " $ Monster.GetLocalizedName() $ " cursed for " $ int(CurseDuration) $ " seconds!");
    }
}
...
function SpreadCurseOnDeath(KFPawn_Monster DyingMonster)
{
    ...
    if (CursesSpread > 0)
    {
        ...
        if (KFPC != None)
        {
            ...
            class'ZTMessageManager'.static.SendImportant(KFPC, "Curse of Stone: Curse spread to " $ CursesSpread $ " nearby enemies!");
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_CurseOfStone_Helper"
}