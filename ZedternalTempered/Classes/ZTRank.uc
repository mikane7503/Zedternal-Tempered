// ===================================================================
// ZTRank - Rank system data and calculations
...
// non-INT client these auto-load from [ZTRank] in the active locale
...
// Tempered adopts Ascension's half-length curve so title progression remains
// meaningful without requiring an excessive lifetime XP total.
static function int GetCumulativeXPForRank(int Rank)
{
    if (Rank <= 0) return 0;
    if (Rank > MAX_RANK) Rank = MAX_RANK;
    return (Rank * (125 + 75 * Rank + (Rank * Rank) / 10)) / 2;
}
...
//                        which UE3 auto-loads from [ZTRank] in the active locale.
// ===================================================================
...
{
    if (Rank <= 0) return "";
    return "[" $ GetTierDisplayName(Rank) $ "]";
}

// e.g. "Veteran" (for scoreboard/HUD/rank-up notifications)
