// ===================================================================
// ZTUpgrade_Skill_Swiftness_Helper - Manages Swiftness ability state
// Provides temporary speed boost with active ability system
// ===================================================================
class ZTUpgrade_Skill_Swiftness_Helper extends Info transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, ZTPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'ZTUpgrade_Skill_Swiftness'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'ZTUpgrade_Skill_Swiftness'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Swiftness", self, class'ZTUpgrade_Skill_Swiftness_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Swiftness: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Swiftness: Already active!");
		return;
	}
...
function Deactivate()
{
	...
	class'ZTMessageManager'.static.SendMinor(DKPC, "Swiftness effect ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'ZTMessageManager'.static.SendImportant(DKPC, "Swiftness ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Skill_Swiftness_Helper"
}