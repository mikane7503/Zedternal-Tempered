// Helper for Miasma.
// Periodically applies a custom STACKING toxic DoT (ZTDT_Miasma) to all nearby ZEDs.
// Pattern based on HotPepper_Helper.
class ZTUpgrade_Skill_Miasma_Helper extends Info transient;
...
// stacking ZTDT_Miasma DoT. With that DoT's 3s duration vs this 1s re-apply, stacks
...
function Timer()
{
	...
	foreach DynamicActors(class'KFPawn_Monster', KFM)
	{
		if (KFM.IsAliveAndWell() && VSizeSQ(Player.Location - KFM.Location) <= Radius)
		{
			...
			if (Dmg > 0)
				KFM.ApplyDamageOverTime(Dmg, KFPC, class'ZTDT_Miasma');
		}
	}
}
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Skill_Miasma_Helper"
}
