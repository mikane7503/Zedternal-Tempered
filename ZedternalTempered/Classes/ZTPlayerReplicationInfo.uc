// ===================================================================
// ZTPlayerReplicationInfo - Extends WMPlayerReplicationInfo
...
// ===================================================================
class ZTPlayerReplicationInfo extends WMPlayerReplicationInfo;
...
// ===================================================================
var ZTUI_Menu UPGMenuManagerDK;
...
// ZTPlayerController and ZTPerk use the helpers for indices >= 256.
// ===================================================================
...
// ZedternalReborn defines this as a package-global preprocessor macro.
// Compiling Tempered against the released dependency .u does not import that
// macro, so keep the inherited storage boundary explicit in this package.
const DK_PARENT_MAX_WEAPON_UPGRADES = 4096;
...
// Scoreboard-only totals. Keep PlayerLevel's legacy gameplay meaning intact.
var repnotify int ScoreboardPerkLevel;
var repnotify int ScoreboardUpgradeCount;

// ===================================================================
// REFORGED WEAPON UNLOCK BITMASK (per-player)
// Moved off ZTGameReplicationInfo, where it was a single game-wide mask
...
// Layout matches ZTGameReplicationInfo.ReforgedStartIndex bit ordering.
// ===================================================================
...
replication
{
    if (bNetDirty)
        ...
        PlayerRank, ScoreboardPerkLevel, ScoreboardUpgradeCount,
...
function AccumulateUpgradeStats(string UpgradeID, int Stacks)
{
    ...
    // ========== DAMAGE RESIST ==========
    if (UpgradeID == "UNIV_R_RESIST") { CachedRoguelikeDamageResist += 0.03 * float(Stacks); return; }
    if (UpgradeID == "UNIV_E_RESIST") { CachedRoguelikeDamageResist += 0.04 * float(Stacks); return; }
    if (UpgradeID == "UNIV_L_RESIST") { CachedRoguelikeDamageResist += 0.06 * float(Stacks); return; }
    ...
    if (UpgradeID == "UNIV_C_SUMO") { CachedRoguelikeDamageResist += 0.04 * float(Stacks); CachedRoguelikeSpeedPenaltyPct += 0.05 * float(Stacks); return; }
    if (UpgradeID == "UNIV_U_SUMO") { CachedRoguelikeDamageResist += 0.06 * float(Stacks); CachedRoguelikeSpeedPenaltyPct += 0.08 * float(Stacks); return; }
    if (UpgradeID == "UNIV_R_SUMO") { CachedRoguelikeDamageResist += 0.09 * float(Stacks); CachedRoguelikeSpeedPenaltyPct += 0.10 * float(Stacks); return; }
    if (UpgradeID == "UNIV_E_SUMO") { CachedRoguelikeDamageResist += 0.13 * float(Stacks); CachedRoguelikeSpeedPenaltyPct += 0.12 * float(Stacks); return; }
    if (UpgradeID == "UNIV_L_SUMO") { CachedRoguelikeDamageResist += 0.18 * float(Stacks); CachedRoguelikeSpeedPenaltyPct += 0.15 * float(Stacks); return; }
...
function SpawnRoguelikeHelper(string UpgradeID)
{
    ...
    local class<ZTRoguelikeHelper> HelperClass;
    local ZTRoguelikeHelper Helper;
    ...
    // Build class path: ZTRoguelikeHelper_BERSERKER
    HelperPath = "ZedternalTempered.ZTRoguelikeHelper_" $ PerkSuffix;
    HelperClass = class<ZTRoguelikeHelper>(DynamicLoadObject(HelperPath, class'Class', true));
...
simulated function UpdatePurchase()
{
    ...
    for (i = DK_PARENT_MAX_WEAPON_UPGRADES; i < DK_MAX_WEAPON_UPGRADES; ++i)
    {
...
function RefreshScoreboardUpgradeTotals()
{
    local int i;
    ...
    ScoreboardPerkLevel = 0;
    ScoreboardUpgradeCount = 0;

    for (i = 0; i < WMGRI.PerkUpgradesList.Length; ++i)
    {
        ScoreboardPerkLevel += GetPerkLevel(i);
        ScoreboardUpgradeCount += GetPerkLevel(i);
    }

    for (i = 0; i < WMGRI.SkillUpgradesList.Length; ++i)
        ScoreboardUpgradeCount += GetSkillUpgrade(i);

    for (i = 0; i < WMGRI.EquipmentUpgradesList.Length; ++i)
        ScoreboardUpgradeCount += bEquipmentUpgrade[i];

    for (i = 0; i < WMGRI.WeaponUpgradeSlotsList.Length; ++i)
        ScoreboardUpgradeCount += GetWeaponUpgrade(i);
...
function RecalculatePlayerLevel()
{
    ...
    if (WMGRI != None)
    {
        ...
        RefreshScoreboardUpgradeTotals();
    }
}
...
function CopyProperties(PlayerReplicationInfo PRI)
{
    ...
    DKPRI = ZTPlayerReplicationInfo(PRI);
    if (DKPRI != None)
    {
        for (i = 0; i < 256; ++i)
        {
        ...
        DKPRI.ScoreboardPerkLevel = ScoreboardPerkLevel;
        DKPRI.ScoreboardUpgradeCount = ScoreboardUpgradeCount;
    }
}
...
//
// We swap the spawn site to `class'ZedternalTempered.ZTUI_Menu'`,
// which inherits everything from WMUI_Menu but rebinds InventoryMenu
// to ZTUI_UPGMenu (a fork that uses GetPerkLevel/IsPerkUnlocked/
...
//  consolidated there onto the single ZTUI_Menu-typed UPGMenuManagerDK.)
...
// Redirect ZR's menu creation to the DK fork (ZTUI_Menu -> ZTUI_UPGMenu)
...
// instantiates ZTUI_Menu and tracks it in our own UPGMenuManagerDK (the
...
simulated function CreateUPGMenu()
{
    ...
    UPGMenuManagerDK = new class'ZedternalTempered.ZTUI_Menu';
...
defaultproperties
{
    ...
    Name="Default__ZTPlayerReplicationInfo"
}
