class ZTUpgrade_Perk_Omen extends ZTUpgrade_Perk config(ZedternalUnlimited);
...
static function UpdateConfig()
{
	if (default.MODEVERSION < 1)
	{
		...
		default.DamageResist = 0.004000f;
		default.HeadshotDamage = 0.010000f;
...
{
    local ZTUpgrade_Perk_Omen_Helper H;
    ...
    optional Controller InstigatedBy, optional KFWeapon MyKFW)
{
    ...
    {
        AccMult = 1.0f;
        if (H.CurrentUpgradeLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level)
            AccMult = 1.5f;
...
{
    local ZTUpgrade_Perk_Omen_Helper OmenHelper;
    local bool bFound;

    if (KFPawn_Human(OwnerPawn) != None && OwnerPawn.Role == ROLE_Authority)
    {
        bFound = False;
        foreach OwnerPawn.ChildActors(class'ZTUpgrade_Perk_Omen_Helper', OmenHelper)
        {
            bFound = True;
        ...
        if (!bFound)
        {
            OmenHelper = OwnerPawn.Spawn(class'ZTUpgrade_Perk_Omen_Helper', OwnerPawn);
            if (OmenHelper != None)
...
static function ZTUpgrade_Perk_Omen_Helper GetHelper(Pawn OwnerPawn)
{
    local ZTUpgrade_Perk_Omen_Helper OmenHelper;
...
defaultproperties
{
	...
	UpgradeIcon(0)=Texture2D'ZedternalRBPerkpackage_Resources.Perks.UI_Perk_Omen_Rank_0'
    // --- Localization (auto-generated by mass_localize_dk_upgrades.py) ---
    // Strings live in KFGame/Localization/<lang>/ZedternalTempered.<langext>
    // Section: [ZTUpgrade_Perk_Omen]  -- see DKUpgrade_* base class for details.
    bShouldLocalize=True
    LocPackage="ZedternalTempered"
    LocSection="ZTUpgrade_Perk_Omen"
    ...
    Name="Default__ZTUpgrade_Perk_Omen"
}
