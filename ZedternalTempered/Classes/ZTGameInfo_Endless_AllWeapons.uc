// ===================================================================
// ZTGameInfo_Endless_AllWeapons - All Weapons mode variant
...
// ===================================================================
class ZTGameInfo_Endless_AllWeapons extends WMGameInfo_Endless_AllWeapons;
...
event PostBeginPlay()
{
    ...
    if (DKGRI != None)
    {
        ...
        // ZTPlayerController.BuyPerkUpgrade). Same push as ZTGameInfo_Endless.
...
function int FindPerkIndex(WMGameReplicationInfo WMGRI, string ClassName)
{
    ...
    // Backward-compatible alias for server INIs written before Tempered's
    // independent ZUTUpgrade_Perk_Base_* migration.
...
function BuildWeaponList()
{
    ...
    // dead code for reference. Same fix as ZTGameInfo_Endless.
            ...
            && WMGRI.WeaponUpgradeSlotsList.Length < DK_MAX_WEAPON_UPGRADES)
        {
            ...
            for (x = 0; x < Min(256, ConfigData.ValidWeaponUpgrades.Length); ++x)
            {
                if (ConfigData.WeaponUpgObjects[x].static.IsUpgradeCompatible(KFW))
                {
                    ...
                    if (class<ZTWeaponUpg_HollowBase>(ConfigData.WeaponUpgObjects[x]) != None)
                    {
...
function ComputeUpgradeSlotBudget(int MainWeaponCount)
{
    ...
    if (class'ZTConfig_HollowWeapons'.static.IsEnabled())
        TotalWeapons += class'ZTHollowWeaponData'.static.GetHollowWeaponCount();
...
function AddWeaponInTrader(const class<KFWeaponDefinition> KFWD)
{
    ...
    // ZTGameInfo_Endless). With Option 2 slot replication the client no
...
function ReduceDamage(out int Damage, Pawn Injured, Controller InstigatedBy, vector HitLocation, out vector Momentum, class<DamageType> DamageType, Actor DamageCauser, TraceHitInfo HitInfo)
{
    ...
    local int AmogusDmg;
    local ZTPlayerController PuppetPC;
    ...
    // *** REMOVE BEFORE SHIPPING *** (and mirror to ZTGameInfo_Endless).
    PuppetPC = ZTPlayerController(Injured.Controller);
    ...
    {
        foreach WorldInfo.AllControllers(class'ZTPlayerController', PuppetHumanPC)
        {
...
function Killed(Controller Killer, Controller KilledPlayer, Pawn KilledPawn, class<DamageType> DT)
{
    ...
    // *** REMOVE BEFORE SHIPPING *** (and mirror to ZTGameInfo_Endless).
    PuppetPC = ZTPlayerController(KilledPlayer);
...
// zeds. *** REMOVE/REVISIT BEFORE SHIPPING *** (mirrored in ZTGameInfo_Endless).
...
function WaveEnded(EWaveEndCondition WinCondition)
{
    ...
    // *** REMOVE/REVISIT BEFORE SHIPPING *** (mirrored in ZTGameInfo_Endless).
    foreach WorldInfo.AllControllers(class'ZTPlayerController', DKPC)
    {
...
defaultproperties
{
    ...
    GameReplicationInfoClass=class'ZedternalTempered.ZTGameReplicationInfo_AllWeapons'
    ...
    Name="Default__ZTGameInfo_Endless_AllWeapons"
}
