class ZTUpgrade_Perk_Artificer_Helper extends Info;
...
// to ZTHudWrapper via GetReaperHUD() (follows Gambit/Shapeshifter pattern).
...
//
// Sound Integration: Uses ZTSoundManager + ZTPlayerController.ClientPlayBuffSound()
...
// ===================================================================
// KILL TRACKING — Called from ZTUpgrade_Perk_Artificer.ModifyDamageGiven
// ===================================================================

// ===================================================================
// CONFIRMED KILL — Called from GameInfo.Killed() via ZTUpgrade_Perk_Artificer.NotifyZedKilled
...
function NotifyConfirmedKill(string NormName, bool bIsReforged)
{
    ...
    if (!WeaponKills[KillIdx].bReforgeUnlocked && !class'ZTUpgrade_Perk_Artificer'.static.HasReforgedVariant(NormName))
    {
    ...
    if (PerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level && bIsReforged)
    ...
    {
        Threshold = class'ZTUpgrade_Perk_Artificer'.static.GetReforgeThresholdForLevel(PerkLevel);
    ...
    if (PerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank1Level)
    {
...
function TrackWeaponKill(KFWeapon KFW, KFPawn_Monster KilledMonster)
{
    ...
    if (class'ZTUpgrade_Perk_Artificer'.static.IsHollowWeapon(KFW))
        return;

    // Normalize weapon name
    NormName = class'ZTUpgrade_Perk_Artificer'.static.NormalizeWeaponName(string(KFW.Class.Name));
    bIsReforged = class'ZTUpgrade_Perk_Artificer'.static.IsReforgedWeapon(KFW);
...
function TriggerReforgeUnlock(string NormName)
{
    ...
    DKGRI = ZTGameReplicationInfo(Owner.WorldInfo.GRI);
    if (DKGRI == None)
    {
        `log("ZR Artificer ERROR: GRI is not ZTGameReplicationInfo, cannot unlock reforged weapon");
        return;
    }
    ...
    if (PlayerPC != None)
        DKPRI = ZTPlayerReplicationInfo(PlayerPC.PlayerReplicationInfo);

    if (DKPRI == None)
    {
        `log("ZR Artificer ERROR: No ZTPlayerReplicationInfo for player, cannot unlock reforged weapon");
        return;
    }

    // Build the expected Reforged weapon class name from the normalized name
    // "AssaultRifle_AK12" -> "ZTWeap_AssaultRifle_AK12_Reforged"
    ...
    if (DKPRI.UnlockReforgedWeapon(BitIndex))
    {
        ...
        // Send to notification feed via ZTMessageManager
        if (PlayerPC != None)
            class'ZTMessageManager'.static.SendImportant(PlayerPC, "Artificer: Reforged" @ NormName @ "unlocked!");
    }
}
...
function CheckMasteryMilestone(string NormName, int KillCount)
{
    ...
    MilestoneKills = class'ZTUpgrade_Perk_Artificer'.default.MasteryMilestoneKills;
    if (MilestoneKills <= 0)
        return;
    ...
    RollCount = class'ZTUpgrade_Perk_Artificer'.static.GetMasteryRollCount(PerkLevel);
    BonusPerRoll = class'ZTUpgrade_Perk_Artificer'.default.MasteryBonusPerRoll;
    ...
    for (i = 0; i < NewMilestones; ++i)
    {
        ...
        // Send human-readable version to notification feed via ZTMessageManager
        if (PlayerPC != None)
            class'ZTMessageManager'.static.SendCritical(PlayerPC, "Artificer: Mastery #" $ (MilestonesWas + i + 1) @ "on" @ NormName @ "-" @ FeedStr);
    }
}
...
{
    return float(MasteredWeaponCount) * class'ZTUpgrade_Perk_Artificer'.default.ResonanceCrossBonus;
}
...
//   Server -> ClientRPC -> ZTHudWrapper (DIRECT, no intermediary)
//
...
function SendProgressUpdate(string NormName, int KillCount)
{
    ...
    if (!bReforged)
    {
        ...
        Phase = 0;
        KillTarget = class'ZTUpgrade_Perk_Artificer'.static.GetReforgeThresholdForLevel(PerkLevel);
    ...
    else
    {
        ...
        if (ReforgeKills <= 0 && KillIdx != INDEX_NONE && class'ZTUpgrade_Perk_Artificer'.static.HasReforgedVariant(NormName))
        {
            ReforgeKills = class'ZTUpgrade_Perk_Artificer'.static.GetReforgeThresholdForLevel(PerkLevel);
...
// ===================================================================
// RELIABLE CLIENT RPCs — Each calls ZTHudWrapper DIRECTLY
 ...
 *  Looks up via ZTSoundManager, plays via ZTPlayerController client RPC. */
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Perk_Artificer_Helper"
}
