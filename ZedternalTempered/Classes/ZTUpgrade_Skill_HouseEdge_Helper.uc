class ZTUpgrade_Skill_HouseEdge_Helper extends Info transient;
...
function OnEnemyKilled()
{
    ...
    SlotTrigger = class'ZTUpgrade_Skill_HouseEdge'.default.SlotMachineTrigger[UpgradeLevel - 1];
...
function TriggerSlotMachine()
{
    ...
    if (KFPC != None)
    {
        if (WinAmount > 0)
        {
            ...
            if (WinAmount >= 150)
            {
                class'ZTMessageManager'.static.SendCritical(KFPC, WinMessage);
            }
            else
            {
                class'ZTMessageManager'.static.SendImportant(KFPC, WinMessage);
            }
        }
        else
        {
            // Minor - no win notification
            class'ZTMessageManager'.static.SendMinor(KFPC, WinMessage);
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_HouseEdge_Helper"
}