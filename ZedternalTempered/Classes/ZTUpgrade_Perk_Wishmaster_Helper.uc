// ===================================================================
// ZTUpgrade_Perk_Wishmaster_Helper - the wish machine.
//
...
// ===================================================================
class ZTUpgrade_Perk_Wishmaster_Helper extends Actor;
...
// Wish outcome stings (loaded via DKMutator/ZTSoundManager)
...
function TraderWatch()
{
	...
	if (!bOpen && bWasTraderOpen)
	{
		...
		if (Stage == WM_HUD_PICK_WISH || Stage == WM_HUD_PICK_TARGET)
		{
			Stage = WM_HUD_HIDDEN;
			PushHUD();
			class'ZTMessageManager'.static.SendMinor(GetPC(), "Wishmaster: the wish window has closed.");
		}
...
function StartWishOffer()
{
	...
	OfferedCount = (PerkLevel >= 20)
		? class'ZTUpgrade_Perk_Wishmaster'.default.WishOffersL20
		: class'ZTUpgrade_Perk_Wishmaster'.default.WishOffersBase;
	...
	PushHUD();
	class'ZTMessageManager'.static.SendMinor(GetPC(), "Wishmaster: three wishes await. Comma cycles, Period confirms.");
}

// ===================================================================
// INPUT (from ZTPlayerController server RPCs)
// ===================================================================
...
function GrantWish(int WishID, KFPlayerController Target)
{
	...
	Chance = (PerkLevel >= 10)
		? class'ZTUpgrade_Perk_Wishmaster'.default.CorruptionChanceL10
		: class'ZTUpgrade_Perk_Wishmaster'.default.CorruptionChance;
	...
	if (bCorrupted)
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Wishmaster: THE WISH CORRUPTS!" @ ResultLine);
		if (Target != GetPC())
			class'ZTMessageManager'.static.SendMinor(Target, "A corrupted wish twists your fate:" @ ResultLine);
	}
	else
	{
		class'ZTMessageManager'.static.SendMinor(GetPC(), "Wishmaster: wish granted." @ ResultLine);
		if (Target != GetPC())
			class'ZTMessageManager'.static.SendMinor(Target, "A wish was granted upon you:" @ ResultLine);
	}
}
...
function ZTWish_Buff GetBuff(KFPawn_Human TP)
{
	local ZTWish_Buff B;

	foreach TP.ChildActors(class'ZTWish_Buff', B)
		return B;

	return TP.Spawn(class'ZTWish_Buff', TP);
}
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Wishmaster_Helper"
}
