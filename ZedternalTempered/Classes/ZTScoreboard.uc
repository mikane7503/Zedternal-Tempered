// ===================================================================
// ZTScoreboard - Custom scoreboard for Zedternal Tempered
// Extends WMGFxHudScoreBoard for full functionality:
//   - Resolution-aware font scaling
//   - Rounded rectangle styling (DrawRectBox)
//   - All columns: Perk, Player, Kills, Assists, Dosh, Health, Armor, Ping, Damage
//   - Live damage-based player ordering and per-column leader highlights
//   - Color-coded health/armor/ping
//
// Only overrides Draw() to change branding from ZedternalReborn
// to Zedternal Tempered with gold accent colors.
// ===================================================================
class ZTScoreboard extends WMGFxHudScoreBoard;

var transient float DamageXPos;

static function string GetPerkPowerHeader()
{
	return class'ZTLocalizationHelper'.static.TryLocalize(
		"ZedternalTempered", "ZTScoreboard", "PerkPowerString", "Perk Power Lv");
}

static function string GetDamageDealtHeader()
{
	return class'ZTLocalizationHelper'.static.TryLocalize(
		"ZedternalTempered", "ZTScoreboard", "DamageDealtString", "Damage Dealt");
}

static function string FormatCompactDamage(int Damage)
{
	if (Damage >= 10000)
		return string(Damage / 1000) $"k";

	return string(Max(0, Damage));
}

// Spectators/lobby players stay below active players. Active players are
// ordered by damage dealt, descending. Equal damage retains the previous
// scoreboard order so rows do not flicker unnecessarily.
function SortScoreboardByDamage(out array<WMPlayerReplicationInfo> Players)
{
	local int i, j;
	local WMPlayerReplicationInfo SwapPlayer;
	local bool bLeftInactive, bRightInactive, bShouldSwap;

	for (i = 0; i < Players.Length; ++i)
	{
		for (j = i + 1; j < Players.Length; ++j)
		{
			bLeftInactive = Players[i] == None || Players[i].bOnlySpectator || Players[i].bWaitingPlayer;
			bRightInactive = Players[j] == None || Players[j].bOnlySpectator || Players[j].bWaitingPlayer;
			bShouldSwap = (bLeftInactive && !bRightInactive)
				|| (bLeftInactive == bRightInactive && !bLeftInactive
					&& Players[j].DamageDealtOnTeam > Players[i].DamageDealtOnTeam);
...
event Draw(Canvas canvas)
{
	...
	local int MaxPerkPower, MaxKills, MaxAssists, MaxDosh, MaxHealth, MaxArmor, MaxDamage, MinPing;
	local ZTPlayerReplicationInfo DKPRI;
	local Color RankColor;
	local float RankXL, PerkScalar, PerkWidth;
	...
	SortScoreboardByDamage(WMPRIArray);
	PlayerIndex = INDEX_NONE;
	MinPing = 2147483647;
	for (i = 0; i < WMPRIArray.Length; ++i)
	{
		...
		if (DKPRI != None)
			MaxPerkPower = Max(MaxPerkPower, DKPRI.ScoreboardPerkLevel);
		else
			MaxPerkPower = Max(MaxPerkPower, WMPRI.PlayerLevel);
		MaxKills = Max(MaxKills, WMPRI.Kills);
		MaxAssists = Max(MaxAssists, WMPRI.Assists);
		MaxDosh = Max(MaxDosh, int(WMPRI.Score));
		MaxHealth = Max(MaxHealth, WMPRI.PlayerHealthInt);
		MaxArmor = Max(MaxArmor, WMPRI.PlayerArmorInt);
		MaxDamage = Max(MaxDamage, WMPRI.DamageDealtOnTeam);
		if (!WMPRI.bBot && WMPRI.UncompressedPing > 0)
			MinPing = Min(MinPing, WMPRI.UncompressedPing);
	}
	//////
	...
	if (PC.WorldInfo.NetMode != NM_Standalone)
		...
		S = " Zedternal Tempered" @class'KFCommon_LocalizedStrings'.default.DiscordSoloMatchString $" ";
	...
	PlayerXPos = Width * 0.140;
	KillsXPos = Width * 0.350;
	AssistXPos = Width * 0.430;
	CashXPos = Width * 0.510;
	HealthXPos = Width * 0.590;
	ArmorXPos = Width * 0.670;
	PingXPos = Width * 0.750;
	DamageXPos = Width * 0.840;

	// Header texts
	ScoreBoardCanvas.SetPos(XPos + PerkXPos, YPos);
	ScoreBoardCanvas.DrawText(GetPerkPowerHeader(), , FontScalar, FontScalar);
	...
	ScoreBoardCanvas.SetPos(XPos + DamageXPos, YPos);
	ScoreBoardCanvas.DrawText(GetDamageDealtHeader(), , FontScalar, FontScalar);
	//////
	...
	for (i = 0; i < WMPRIArray.length; ++i)
	{
		...
		DKPRI = ZTPlayerReplicationInfo(WMPRIArray[i]);
		if (DKPRI != None && DKPRI.PlayerRank > 0
			&& !WMPRIArray[i].bOnlySpectator && !WMPRIArray[i].bWaitingPlayer)
		{
			RankColor = class'ZedternalTempered.ZTRank'.static.GetTierColor(DKPRI.PlayerRank);
		...
		if (WMPRI.bOnlySpectator)
			...
			S = string(DKPRI.ScoreboardPerkLevel) $" (" $string(DKPRI.ScoreboardUpgradeCount + DKPRI.RoguelikeTotalUpgrades) $")";
		else
			S = string(WMPRI.PlayerLevel) $" (" $string(WMPRI.PlayerLevel) $")";

		if (!WMPRI.bOnlySpectator && !WMPRI.bWaitingPlayer
			&& ((DKPRI != None && DKPRI.ScoreboardPerkLevel == MaxPerkPower && MaxPerkPower > 0)
				|| (DKPRI == None && WMPRI.PlayerLevel == MaxPerkPower && MaxPerkPower > 0)))
			ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);

		// Keep three-digit power/upgrade totals inside the perk column.
		PerkScalar = FontScalar;
		PerkWidth = Width * (PlayerXPos / Width - PerkXPos / Width) - 8.0f;
		ScoreBoardCanvas.TextSize(S, XL, RankXL, PerkScalar, FontScalar);
		if (XL > PerkWidth && XL > 0.0f)
			PerkScalar *= PerkWidth / XL;
		ScoreBoardCanvas.DrawText(S, , PerkScalar, FontScalar);
		ScoreBoardCanvas.DrawColor = MakeColor(255, 255, 255, 255);
		//////
		...
		if (DKPRI != None && DKPRI.PlayerRank > 0 && !WMPRI.bOnlySpectator && !WMPRI.bWaitingPlayer)
		{
			...
			S = class'ZedternalTempered.ZTRank'.static.GetRankDisplayString(DKPRI.PlayerRank);
		...
		if (!WMPRI.bOnlySpectator && !WMPRI.bWaitingPlayer && MaxKills > 0 && WMPRI.Kills == MaxKills)
			ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
		...
		if (!WMPRI.bOnlySpectator && !WMPRI.bWaitingPlayer && MaxAssists > 0 && WMPRI.Assists == MaxAssists)
			ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
		...
		if (!WMPRI.bOnlySpectator && !WMPRI.bWaitingPlayer && MaxDosh > 0 && int(WMPRI.Score) == MaxDosh)
			ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
		...
		{
			if (MaxHealth > 0 && WMPRI.PlayerHealthInt == MaxHealth)
				ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
			else if (WMPRI.PlayerHealthInt >= 150)
		...
		{
			if (MaxArmor > 0 && WMPRI.PlayerArmorInt == MaxArmor)
				ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
			else if (WMPRI.PlayerArmorInt >= 150)
		...
		{
			if (WMPRI.UncompressedPing == MinPing)
				ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
			else if (WMPRI.UncompressedPing <= 100)
				ScoreBoardCanvas.DrawColor = MakeColor(0, 255, 0, 255);
		...
		////// Damage dealt (replaces the platform column)
		if (WMPRI.bOnlySpectator || WMPRI.bWaitingPlayer)
			S = "-";
		else
			S = FormatCompactDamage(WMPRI.DamageDealtOnTeam);
		if (!WMPRI.bOnlySpectator && !WMPRI.bWaitingPlayer && MaxDamage > 0 && WMPRI.DamageDealtOnTeam == MaxDamage)
			ScoreBoardCanvas.DrawColor = MakeColor(80, 255, 120, 255);
