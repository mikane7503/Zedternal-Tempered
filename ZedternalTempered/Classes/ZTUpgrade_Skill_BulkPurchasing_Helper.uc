class ZTUpgrade_Skill_BulkPurchasing_Helper extends Info transient;
...
function ApplyBulkDiscount()
{
    ...
    RefundRate = class'ZTUpgrade_Skill_BulkPurchasing'.default.RefundPercentage[UpgradeLevel - 1];
    TotalRefund = 0.0f;
    ...
    if (TotalRefund >= 1.0f)
    {
        ...
        if (KFPC != None)
        {
            ...
            if (KFPRI != None)
            {
                ...
                class'ZTMessageManager'.static.SendImportant(KFPC, "BULK PURCHASING DISCOUNT: +" $ int(TotalRefund) $ " Dosh refund (" $ RefundRate*100 $ "% off " $ (TotalPurchases-1) $ " additional items)");
            }
        }
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_BulkPurchasing_Helper"
}