class ZTUpgrade_Perk_Wendigo_Helper extends Info transient;
...
function UpdateStalkerTracking()
{
    ...
    if (CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank1Level && TimeSinceLastFire >= 10.0f && !bFirstShotReady)
    {
        bFirstShotReady = true;
    }
...
function ShowPerfectAmbushNotification()
{
    ...
    class'ZTMessageManager'.static.SendCritical(DKPC, "PERFECT AMBUSH: Prey eliminated!");
...
function ShowApexStalkerNotification()
{
    ...
    class'ZTMessageManager'.static.SendCritical(DKPC, "APEX STALKER: Ultimate predator awakened!");
    DKPC.ClientWendigoChainNotification("APEX STALKER!", "Untouchable hunter emerges", 4.0f);
}

// Resolve the owning player's ZTPlayerController (server-side).
function ZTPlayerController GetOwnerController()
{
    local Pawn P;

    P = Pawn(Owner);
    if (P != None)
        return ZTPlayerController(P.Controller);

    return None;
}

// Computes tracker states server-side and ships them to the owning client
// via ZTPlayerController.ClientUpdateWendigoTrackers.
...
// owning ZTPlayerController's replicated channel. The capstone level gates
// below read the server's ZTConfig_Capstone INI values, so the trackers
...
function UpdateStalkerDisplay(float TimeSinceLastFire, float TimeSinceLastDamage, optional bool bPerfectAmbushTriggered = false, optional bool bApexStalkerState = false)
{
    ...
    AmbushState = 0;
    if (CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank1Level)
    {
    ...
    ApexState = 0;
    if (CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level)
    {
...
function string GetStalkingSummary()
{
    ...
    if (bApexStalkerActive && CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level)
    {
        StatusText = "APEX STALKER ACTIVE (+35% damage)";
    }
    else if (TimeSinceLastDamage >= 25.0f && CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level)
    {
        StatusText = "Approaching Apex (" $ int(FMin(TimeSinceLastDamage, 30.0f)) $ "/30s)";
    }
    else if (bFirstShotReady && TimeSinceLastFire >= 10.0f && CurrentPerkLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank1Level)
    {
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Perk_Wendigo_Helper"
}
