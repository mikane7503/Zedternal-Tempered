// ===================================================================
// ZTUpgrade_Skill_TacticalFocus_Helper - Manages Tactical Focus ability state
// ===================================================================
class ZTUpgrade_Skill_TacticalFocus_Helper extends Info transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, ZTPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'ZTUpgrade_Skill_TacticalFocus'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'ZTUpgrade_Skill_TacticalFocus'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Tactical Focus", self, class'ZTUpgrade_Skill_TacticalFocus_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Tactical Focus: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Tactical Focus: Already active!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'ZTMessageManager'.static.SendMinor(DKPC, "Tactical Focus: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'ZTMessageManager'.static.SendImportant(DKPC, "TACTICAL FOCUS ACTIVATED! +40% reload speed for 10 seconds!");
	}
	else
	{
		class'ZTMessageManager'.static.SendImportant(DKPC, "TACTICAL FOCUS (DELUXE) ACTIVATED! +60% reload speed for 10 seconds!");
	}
...
function Deactivate()
{
	...
	class'ZTMessageManager'.static.SendMinor(DKPC, "Tactical Focus ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'ZTMessageManager'.static.SendImportant(DKPC, "Tactical Focus ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Skill_TacticalFocus_Helper"
}