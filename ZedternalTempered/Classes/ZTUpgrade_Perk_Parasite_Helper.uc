// ===================================================================
// ZTUpgrade_Perk_Parasite_Helper - Life Drain Tracking
// Handles: Siphoned enemy tracking, Blood Harvest, Hemorrhage Pulse
// ===================================================================
class ZTUpgrade_Perk_Parasite_Helper extends Info transient;
...
function Initialize(KFPawn_Human NewPlayer, int NewUpgradeLevel)
{
	...
	// Load sound effects via ZTMutator
	foreach Player.WorldInfo.AllActors(class'ZTMutator', Mutator)
	{
		`log("Parasite Helper: Found ZTMutator, requesting sounds...");
...
function ApplySiphon(KFPawn_Monster Monster)
{
	...
	for (i = 0; i < SiphonedEnemies.Length; i++)
	{
		...
		{
			// Refresh duration
			SiphonedEnemies[i].ExpirationTime = CurrentTime + class'ZTUpgrade_Perk_Parasite'.default.SiphonDuration;
			bFound = true;
			break;
		}
	}
	...
	if (!bFound && SiphonedEnemies.Length < class'ZTUpgrade_Perk_Parasite'.default.MaxSiphonedEnemies)
	{
		NewEntry.Monster = Monster;
		NewEntry.ExpirationTime = CurrentTime + class'ZTUpgrade_Perk_Parasite'.default.SiphonDuration;
		SiphonedEnemies.AddItem(NewEntry);
		
		// Apply bleeding DoT to the monster
		Monster.ApplyDamageOverTime(
			class'ZTUpgrade_Perk_Parasite'.default.SiphonDamagePerTick,
			Player.Controller,
			class'ZTDT_Siphon'
		);
	}
...
{
	return class'ZTUpgrade_Perk_Parasite'.default.MaxSiphonedEnemies;
}
...
function DamageSiphonedEnemies(int HealAmount)
{
	...
	DamagePerEnemy = Round(float(HealAmount) * class'ZTUpgrade_Perk_Parasite'.default.HealingDamageRatio);
...
function OnKillForBloodHarvest(KFPawn_Monster KilledMonster)
{
	...
	if (BloodHarvestProgress >= class'ZTUpgrade_Perk_Parasite'.default.BloodHarvestKillsRequired && !bHemorrhagePulseReady)
	{
	...
	`log("Parasite: Blood Harvest progress -" @ BloodHarvestProgress @ "/" @ class'ZTUpgrade_Perk_Parasite'.default.BloodHarvestKillsRequired);
}
...
function TriggerHemorrhagePulse()
{
	...
	RadiusSq = class'ZTUpgrade_Perk_Parasite'.default.HemorrhagePulseRadius ** 2;
	TotalDamage = 0;
	...
	foreach Player.WorldInfo.AllPawns(class'KFPawn_Human', Teammate)
	{
		...
		{
			DKPC = ZTPlayerController(Teammate.Controller);
	...
	foreach Player.WorldInfo.AllPawns(class'KFPawn_Monster', Monster)
	{
		if (Monster != None && Monster.IsAliveAndWell())
		{
			...
			{
				Monster.TakeDamage(
					class'ZTUpgrade_Perk_Parasite'.default.HemorrhagePulseDamage,
					Player.Controller,
					Monster.Location,
					vect(0,0,0),
					class'ZTDT_Siphon'
				);
				TotalDamage += class'ZTUpgrade_Perk_Parasite'.default.HemorrhagePulseDamage;
			}
		}
	}
	...
	DoubleLifeStealEndTime = Player.WorldInfo.TimeSeconds + class'ZTUpgrade_Perk_Parasite'.default.DoubleLifeStealDuration;
...
{
	local ZTPlayerController DKPC;
	local ZTHudWrapper ParasiteHUD;
	...
	{
		if (DebugUpdateCount % 50 == 1)
			`log("Parasite Helper CLIENT: GetALocalPlayerController returned None or not ZTPlayerController!");
		return;
	}
	
	if (DebugUpdateCount % 50 == 1)
		`log("Parasite Helper CLIENT: Got ZTPlayerController:" @ DKPC);
	
	// Get HUD using GetReaperHUD static function
	ParasiteHUD = class'ZTHudWrapper'.static.GetReaperHUD(DKPC);
	if (ParasiteHUD == None)
	{
		if (DebugUpdateCount % 50 == 1)
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Parasite_Helper"
}
