// ===================================================================
// ZTUpgrade_Skill_ScavengersLuck_Helper - Manages Scavenger's Luck ability state
// Instant reload + fire rate boost
// FIXED: Improved instant reload to properly handle weapon state
// ===================================================================
class ZTUpgrade_Skill_ScavengersLuck_Helper extends Info transient;
...
function Initialize(int InTier, KFPawn_Human InOwnerPawn, ZTPlayerController InDKPC)
{
	...
	if (InTier == 1)
		AbilityIcon = class'ZTUpgrade_Skill_ScavengersLuck'.default.UpgradeIcon[0];
	else
		AbilityIcon = class'ZTUpgrade_Skill_ScavengersLuck'.default.UpgradeIcon[1];
	
	if (!DKPC.RegisterAbility("Scavenger's Luck", self, class'ZTUpgrade_Skill_ScavengersLuck_Helper', AbilityIcon))
	{
...
function TryActivate()
{
	...
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Scavenger's Luck: Cannot activate while dead!");
		return;
	}
	
	if (bActive)
	{
		class'ZTMessageManager'.static.SendMinor(DKPC, "Scavenger's Luck: Already active!");
		return;
	}
	...
	if (bOnCooldown)
	{
		...
		class'ZTMessageManager'.static.SendMinor(DKPC, "Scavenger's Luck: On cooldown (" $ int(RemainingCooldown) $ "s remaining)");
		return;
	}
...
function Activate()
{
	...
	if (Tier == 1)
	{
		class'ZTMessageManager'.static.SendImportant(DKPC, "SCAVENGER'S LUCK! Instant reload + +30% fire rate for 10 seconds!");
	}
	else
	{
		class'ZTMessageManager'.static.SendImportant(DKPC, "SCAVENGER'S LUCK (DELUXE)! Instant reload + +50% fire rate for 10 seconds!");
	}
...
function Deactivate()
{
	...
	class'ZTMessageManager'.static.SendMinor(DKPC, "Scavenger's Luck ended. Cooldown: 60 seconds.");
...
function UpdateAbility()
{
	...
	else if (bOnCooldown)
	{
		...
		{
			bOnCooldown = false;
			class'ZTMessageManager'.static.SendImportant(DKPC, "Scavenger's Luck ready!");
			DKPC.ClientUpdateAbilityHUD(MySlotIndex, false, false, 0.0f, 0.0f);
		}
		else
		{
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Skill_ScavengersLuck_Helper"
}