// ===================================================================
// ZTMessageManager - prioritized message router with localization
// ===================================================================
...
// ===================================================================
class ZTMessageManager extends Object config(ZedternalTempered);
...
)
{
    ...
    if (KFPC.Role == ROLE_Authority && KFPC.RemoteRole != ROLE_None)
    {
        ...
        Replicator = class'ZTMessageReplicator'.static.GetReplicatorForPlayer(KFPC);
        if (Replicator != None)
        {
    ...
    else
    {
        ...
        CustomHUD = class'ZTHudWrapper'.static.GetReaperHUD(KFPC);
        if (CustomHUD != None)
        {
...
//   1. Active locale's ZedternalTempered.<lang> [DKMessages] KEY=...
//   2. ZedternalTempered.int [DKMessages] KEY=... (English fallback)
...
)
{
    ...
    Template = class'ZTLocalizationHelper'.static.TryLocalize(
        "ZedternalTempered",
        "DKMessages",
        string(MessageKey),
...
static function string GetEnglishFallback(name MessageKey)
{
    switch (MessageKey)
    {
        ...
        case 'ZURankPRIError':         return "PRI: None or not ZTPlayerReplicationInfo!";
...
// client via ZTMessageReplicator (sending key+args, NOT the formatted
...
defaultproperties
{
    Name="Default__ZTMessageManager"
}
