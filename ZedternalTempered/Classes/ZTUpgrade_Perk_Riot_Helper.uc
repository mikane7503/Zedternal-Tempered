class ZTUpgrade_Perk_Riot_Helper extends Info transient;
...
function ScanForNearbyEnemies()
{
	...
	RadiusSq = class'ZTUpgrade_Perk_Riot'.default.NearbyEnemyRadius * 
	           class'ZTUpgrade_Perk_Riot'.default.NearbyEnemyRadius;
	...
	foreach OwnerPawn.WorldInfo.AllPawns(class'KFPawn_Monster', Monster)
	{
		if (Monster.IsAliveAndWell())
		{
			...
			if (Distance <= RadiusSq)
			{
				...
				// Cap at max enemies
				if (Count >= class'ZTUpgrade_Perk_Riot'.default.MaxNearbyEnemies)
				{
					Count = class'ZTUpgrade_Perk_Riot'.default.MaxNearbyEnemies;
					break;
				}
			}
		}
	...
	if (UpgradeLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level)
	{
		if (Count >= class'ZTUpgrade_Perk_Riot'.default.MaxBonusThreshold)
		{
...
function UpdateLingerEffect(float CurrentTime)
{
	...
	if (TimeSinceHigh > class'ZTUpgrade_Perk_Riot'.default.RiotLinger &&
	    NearbyEnemyCount < class'ZTUpgrade_Perk_Riot'.default.MaxBonusThreshold)
	{
		bHadHighEnemyCount = false;
	}
}
...
function int GetNearbyEnemyCount()
{
	...
	if (UpgradeLevel >= class'ZTConfig_Capstone'.default.Capstone_Rank2Level && bHadHighEnemyCount)
	{
		CurrentTime = OwnerPawn.WorldInfo.TimeSeconds;
		TimeSinceHigh = CurrentTime - LastHighEnemyTime;
		...
		if (TimeSinceHigh <= class'ZTUpgrade_Perk_Riot'.default.RiotLinger)
		{
			return Max(NearbyEnemyCount, class'ZTUpgrade_Perk_Riot'.default.MaxBonusThreshold);
		}
	}
...
function UpdateHUD()
{
	...
	bMaxBonusActive = (DisplayCount >= class'ZTUpgrade_Perk_Riot'.default.MaxBonusThreshold);
...
defaultproperties
{
	...
	Name="Default__ZTUpgrade_Perk_Riot_Helper"
}