class ZTUpgrade_Perk_Cryophilite_Helper extends Info transient;
...
function ShowIcicleArrowNotification()
{
    ...
    if (KFPC != None)
    {
        KFPC.ClientMessage("ICICLE ARROW FIRED! (+200% Damage)", 'CriticalEvent');
    }
}
...
reliable client function UpdateKillDisplay(byte HeadshotKills, byte TotalKillsDisplay, bool bIcicleReady, bool bAbsoluteReady, optional int CurrentUpgradeLevel = 1)
{
    ...
    CryoHUD = class'ZTHudWrapper'.static.GetReaperHUD(KFPC);
    if (CryoHUD != None)
    {
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Perk_Cryophilite_Helper"
}
