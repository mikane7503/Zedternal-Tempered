...
function DrawMinigameEventHUD()
{
    ...
    ZTGRI = ZTGameReplicationInfo(KFGRI);
    if (ZTGRI == None)
        return;

    ZTPC = ZTPlayerController(PlayerOwner);
    NTS = ResScale;
    PadX = 10.0f * NTS;
    PadY = 5.0f * NTS;
    Canvas.Font = class'KFGameEngine'.Static.GetKFCanvasFont();
    ...
    if (ZTGRI.EventZoneLoc != CachedEventZoneLoc)
    {
        CachedEventZoneLoc = ZTGRI.EventZoneLoc;
    ...
    switch (EventWaveOverlayID)
    {
        case 28: // RED LIGHT, GREEN LIGHT - phase banner + red edges
            ...
            {
                class'ZTEventWave'.static.DrawEdgeVignette(Canvas, Canvas.SizeX, Canvas.SizeY, EventWaveAlpha, 255, 20, 20, 120.f, 5);
            }
        ...
        case 29: // THE FLOOR IS LAVA - zone marker + distance + countdown
            ...
            if (ZTGRI.EventSwapInterval > 0)
            {
                Remaining = int(float(ZTGRI.EventSwapInterval) - (WorldInfo.TimeSeconds - CachedZoneChangeTime));
                if (Remaining < 0)
                    Remaining = 0;
            }
        ...
        case 30: // BODYGUARD BOND - partner reminder
            if (ZTPC == None || ZTPC.EventPartnerName == "")
                return;

            Col = MakeColor(80, 200, 255, 255);
            Line = EventWave_HudPartner $ ": " $ ZTPC.EventPartnerName;
        ...
        case 31: // BOUNTY BOARD - personal quota + team progress
            if (ZTPC == None || ZTPC.EventQuotaNeeded <= 0)
                return;

            if (ZTPC.EventQuotaProgress >= ZTPC.EventQuotaNeeded)
                Col = MakeColor(60, 220, 80, 255);
            else
                Col = MakeColor(255, 200, 60, 255);

            Line = EventWave_HudQuota $ ": " $ ZTPC.EventQuotaProgress $ "/" $ ZTPC.EventQuotaNeeded @ ZTPC.EventQuotaText;
            Line2 = EventWave_HudTeamProgress $ ": " $ ZTGRI.EventDataA $ "/" $ ZTGRI.EventDataB;
...
//
// Pushed from ZTUpgrade_Perk_Goalkeeper_Helper via ClientGoalkeeperHUD.
...
//
// Pushed from ZTUpgrade_Perk_Fastball_Helper via ClientFastballHUD.
