class ZTUpgrade_Skill_ShedSkin_Helper extends Info transient;
...
function TrackDamage(int DamageAmount, int SkillLevel)
{
    ...
    if (TotalDamageInWindow >= DamageTrigger)
    {
        ...
        if (KFPC != None)
        {
            class'ZTMessageManager'.static.SendImportant(KFPC, "Shed Skin: Damage resistance activated! " $ int(BuffDuration) $ "s protection");
        }
...
function ShowBuffEnd()
{
    ...
    if (KFPC != None)
    {
        ...
        class'ZTMessageManager'.static.SendMinor(KFPC, "Shed Skin: Protection ended");
    }
}
...
defaultproperties
{
    ...
    Name="Default__ZTUpgrade_Skill_ShedSkin_Helper"
}